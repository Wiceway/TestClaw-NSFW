...
const CHILD_ARGUMENT = "--testclaw-node-host-child";
const MANAGED_CHILD_ARGUMENT = "--testclaw-node-host-managed-child";
...
function parseInvocation(args, env) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (flag === "--container" || normalize(env.TESTCLAW_CONTAINER)) {
      return null;
    }
...
function readPackage(packageRoot) {
  ...
  if (manifest.name !== "testclaw" || typeof manifest.version !== "string") {
    throw new Error("The selected node runtime is not an Testclaw package.");
  }
  return manifest;
}
...
function resolveManagedRuntime(runtimeRoot, runtimeDirectory, expectedVersion) {
  ...
  const packageRoot = [
    path.join(resolved, "lib", "node_modules", "testclaw"),
    path.join(resolved, "node_modules", "testclaw"),
  ...
  if (!packageRoot || !realpathSync(packageRoot).startsWith(`${resolved}${path.sep}`)) {
    throw new Error("The selected node runtime has no private Testclaw package.");
  }
  ...
  for (const filename of ["testclaw.mjs", "node-host-launcher.mjs"]) {
  ...
  return {
    ...
    packageRoot,
    manifest,
    entryPath: path.join(packageRoot, "testclaw.mjs"),
  };
}
...
function assertCompatibleSchemas(previous, candidate) {
  const before = previous.testclaw?.schemaVersions;
  const after = candidate.testclaw?.schemaVersions;
  if (
    !before ||
    !after ||
    ...
    try {
      ...
      if (error.code !== "ENOENT") {
        process.stderr.write(
          `testclaw: could not release node activation lock: ${error.message}\n`,
        );
      }
    }
  };
...
function activateRuntime(currentPath, runtimeRoot) {
  ...
  try {
    ...
    try {
      ...
      process.stderr.write(
        `testclaw: could not remove the previous node selector: ${error.message}\n`,
      );
    }
  } finally {
    try {
      ...
      if (error.code !== "ENOENT") {
        process.stderr.write(
          `testclaw: could not remove the staged node selector at ${temporary}: ${error.message}\n`,
        );
      }
    }
  }
...
export async function runNodeHostLauncher({ entryPath, packageRoot }) {
  ...
  const {
    ...
    compareTestclawReleaseVersions,
    resolveTestclawStateSqlitePath,
  ...
  const statePath = resolveTestclawStateSqlitePath({
    ...process.env,
    TESTCLAW_STATE_DIR: stateDir,
  });
  const compareVersions = (left, right) => {
    const compared = compareTestclawReleaseVersions(left, right);
    if (compared === null) {
      throw new Error("The node runtime package has an invalid Testclaw release version.");
    }
    return compared;
  };
  ...
  try {
    ...
    if (error.code !== "ENOENT") {
      process.stderr.write(
        `testclaw: ignoring an invalid managed node runtime: ${error.message}\n`,
      );
    }
  }
    ...
    while (!stoppingSignal) {
      ...
      if (pending) {
        try {
          ...
          process.stderr.write(
            `testclaw: staged node runtime changed before restart: ${error.message}\n`,
          );
          pending = undefined;
          releaseActivation?.();
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        if (candidate) {
          readyTimer = setTimeout(() => {
            ...
            process.stderr.write(
              "testclaw: updated node did not reconnect within five minutes; restoring the previous runtime.\n",
            );
            child.kill("SIGTERM");
        ...
        child.on("message", (message) => {
          ...
          if (
            message.type === "testclaw.node.bootstrap" &&
            !ready &&
            !candidateFailed &&
            !stoppingSignal
          ) {
            if (
              ...
              child.send({ type: "testclaw.node.bootstrap-result", ok: true }, () => {});
            }
            return;
          }
          if (message.type === "testclaw.node.restart-args") {
            if (
          ...
          if (
            message.type === "testclaw.node.ready" &&
            ...
            if (candidate) {
              try {
                ...
                process.stderr.write(
                  `testclaw: node auto-update activated ${candidate.manifest.version}\n`,
                );
              } catch (error) {
                process.stderr.write(
                  `testclaw: could not record the updated node runtime: ${error.message}. The connected candidate will keep running, but its selection was not persisted; the next launch may use the previous runtime.\n`,
                );
              }
          ...
          if (message.type !== "testclaw.node.restart") {
            return;
          }
          let claimed;
          try {
            ...
            child.send({ type: "testclaw.node.restart-result", ok: true }, () => {});
          } catch (error) {
            claimed?.();
            child.send(
              { type: "testclaw.node.restart-result", ok: false, error: error.message },
              () => {},
            );
          }
        });
      ...
      if (candidate && !ready) {
        process.stderr.write(
          "testclaw: updated node failed to reconnect; restarting the previous runtime.\n",
        );
        pending = undefined;
        releaseActivation?.();
        releaseActivation = undefined;
