...
export const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    ...
    process.stderr.write(
      `[testclaw] Failed to respawn launcher: ${
...
function readWindowsServiceScript(filename) {
  ...
  if (buffer[0] === 0xff && buffer[1] === 0xfe) {
    ...
    const marker = /^@rem testclaw-launcher-encoding=(\S+)\s*$/.exec(
      buffer.subarray(0, end < 0 ? buffer.length : end).toString("latin1"),
    );
    if (marker) {
      if (!preamble) {
  ...
  process.stderr.write(
    `testclaw: service script uses code page ${Number.isFinite(codePage) ? codePage : "unknown"}; not decodable here\n`,
  );
  return null;
}
...
function managedServiceNode(homeDir, env) {
  ...
  if (process.platform === "darwin") {
    ...
    const name =
      env.TESTCLAW_SYSTEMD_UNIT?.trim() || `testclaw-gateway${suffix ? `-${suffix}` : ""}`;
    ...
    const stateDir = resolveRecoveryPath(
      env.TESTCLAW_STATE_DIR?.trim() ||
        (homeDir && path.join(homeDir, `.testclaw${suffix ? `-${suffix}` : ""}`)),
      homeDir,
    );
    const filename = resolveRecoveryPath(
      env.TESTCLAW_TASK_SCRIPT?.trim() || (stateDir && path.join(stateDir, scriptName)),
      homeDir,
    );
...
export async function findUsableNodeRuntime({
  ...
  const recoveryHome = resolveRecoveryPath(
    homeDir ?? (env.TESTCLAW_HOME?.trim() || osHome),
    osHome,
  ...
  let nodePath = recoveryRoot
    ...
    : null;
  let reason = "cached Testclaw runtime";
  ...
  if (!nodePath && allowInstall && recoveryRoot) {
    ...
    reason = "private Testclaw runtime";
  }
...
export async function recoverNodeRuntime({
  ...
  if (
    ...
    env.TESTCLAW_NODE_UPDATE_RESPAWNED === "1" ||
    !process.argv[1] ||
  ...
  process.stderr.write(
    `testclaw: Retrying with ${JSON.stringify(nodePath)} (${reason}; current Node failed runtime admission).\n`,
  );
