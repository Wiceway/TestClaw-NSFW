import { isSupportedTestclawNodeVersion, parseNodeReleaseVersion } from "./node-version.mjs";
...
export function nodeRuntimeFailure(version, probe) {
  ...
  if (!release || release.major < 24) {
    return `${label}: testclaw requires Node >=24.16.0 <25, or >=26.1.0.`;
  }
  return null;
}

export function nodeRuntimeNote(version, probe) {
  return !nodeRuntimeFailure(version, probe) && !isSupportedTestclawNodeVersion(version)
