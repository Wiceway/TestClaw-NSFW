...
function listExtensionHealthChecksForDoctor(coreChecks, unavailablePlugins = []) {
	...
	for (const check of registeredChecks) {
		if (check.kind === "core") continue;
		const unavailable = unavailablePlugins.find((finding) => finding.source !== void 0 && finding.source === check.source);
