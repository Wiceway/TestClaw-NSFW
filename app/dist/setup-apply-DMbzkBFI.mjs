...
import { o as sameSetupConfiguredRoute, r as projectInferenceRoute, s as sameSetupInferenceRoute, t as assertSetupTarget } from "./inference-route-DTlrGGL8.mjs";
import { c as resolveOnboardingAgentTarget, u as resolveSystemAgentOnboardingTarget } from "./onboard-agent-target--U5T7XvY.mjs";
import { r as matchesLocalSetupWorkspace } from "./setup-recovery-l7UqkitP.mjs";
...
async function applySystemAgentSetup(params, hooks) {
	...
	const [{ readSetupConfigFileSnapshot, resolveQuickstartGatewayDefaults }, onboardHelpers, { applyLocalSetupWorkspaceConfig, resolveOnboardingWorkspaceConflict }, { transformConfigWithPendingPluginInstalls }] = await Promise.all([
		...
		import("./install-record-commit-AjIkCSWv.mjs")
	]);
	...
	let guardModules = expectedAgentId || expectedAgentDir || expectedModelRef ? await Promise.all([import("./agent-scope-CbPqWO_3.mjs"), import("./model-selection-De2pDgyb.mjs")]) : void 0;
	...
	if (startedWithoutAuthoredRoster) {
		...
		guardModules ??= await Promise.all([import("./agent-scope-CbPqWO_3.mjs"), import("./model-selection-De2pDgyb.mjs")]);
	...
	await runCommittedFollowUp(async () => {
		const { updateExecApprovals } = await import("./exec-approvals-DFTOVShU.mjs");
		beforePersistentApply?.();
	...
	if (surface === "cli") await runCommittedFollowUp(async () => {
		const { ensureGatewayServiceForOnboarding } = await import("./setup.finalize-CCW3UCa5.mjs");
		beforePersistentApply?.();
		...
		if (gateway.status === "failed") lines.push(`Gateway service: ${gateway.error}`);
			...
			const probe = await onboardHelpers.waitForGatewayReachable({
				...
				password: settings.authMode === "password" ? await (await import("./setup.secret-input-B_Z1cBJl.mjs")).resolveSetupSecretInputString({
					config: nextConfig,
