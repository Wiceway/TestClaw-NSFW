...
import { o as supportsNativeExecApprovalClient, r as listNativeExecApprovalClientLabels, t as describeNativeExecApprovalClientSetup } from "./exec-approval-surface-FKkZmrLs.mjs";
...
function buildGenericNativeExecApprovalFallbackText(params) {
	...
	let manualRecovery = "Print the Control UI URL with `testclaw dashboard --no-open`, open it in a browser, then use the approval inbox.";
	if (params?.host === "node") {
		const nodeId = normalizeOptionalString(params.nodeId) ?? "<id|name|ip>";
		manualRecovery += ` Inspect the node's effective exec policy with \`testclaw approvals get --node ${nodeId}\`.`;
	}
	return clients ? `Approve it from the Web UI or terminal UI, or enable a native chat approval client such as ${clients}. ${manualRecovery} If those accounts already know your owner ID via allowFrom or owner config, Assistant can often infer approvers automatically.` : `Approve it from the Web UI or terminal UI. ${manualRecovery}`;
}
