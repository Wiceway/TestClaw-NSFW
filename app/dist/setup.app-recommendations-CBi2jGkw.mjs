...
import { E as string, d as array, l as _enum, x as object } from "./schemas-qz0osXyE.mjs";
import { h as resolveOfficialExternalPluginLabel, l as listOfficialExternalPluginCatalogEntries, p as resolveOfficialExternalPluginInstall, s as listOfficialExternalChannelCatalogEntries, u as listOfficialExternalProviderCatalogEntries } from "./official-external-plugin-catalog-BxZhHmRY.mjs";
...
import { c as fetchClawHubSkillVerification, u as searchClawHubSkills } from "./clawhub-skills-uDrjQHSa.mjs";
import { i as resolveClawHubSkillVerificationTarget } from "./clawhub-status-DKS0FANL.mjs";
import { n as t } from "./i18n-DrbABx94.mjs";
import "./setup-inference-GP_M4P1b.mjs";
import { t as completeSetupInference } from "./setup-inference-turn-DVFg2qoq.mjs";
import { t as scanInstalledApps } from "./installed-apps-Dhe8aPRY.mjs";
import { t as ensureOnboardingPluginInstalled } from "./onboarding-plugin-install-BxBy4LVS.mjs";
import { t as installSkillFromClawHub } from "./clawhub-Bvm28cuk.mjs";
...
function officialCandidate(entry, source) {
	...
	return {
		...
		summary: entry.description?.trim() || "Official Assistant plugin",
		source
	};
}
...
function buildMatcherPrompt(groups) {
	...
	return [
		"Match installed applications to genuinely related Assistant plugins or skills.",
...
async function setupAppRecommendations(params) {
	...
	const storedRecord = await (params.deps?.readStored ?? store.read)();
	...
	if (hasLegacyClawHubId && storedRecord) {
		if (!await clearPendingStored({ expected: storedRecord })) return unchangedOutcome(params.config);
	}
	...
	let recordResult;
	const commitStoredResult = async (retryMatches) => {
		...
		const updated = retryMatches.length === 0 ? await acknowledgeStored({ expected }) : await updatePendingStored({
	...
	return {
		config: next,
		commitResult: hasDeferredOfficialResult ? () => recordResult(retryMatches) : async () => void 0
	};
}
//#endregion
