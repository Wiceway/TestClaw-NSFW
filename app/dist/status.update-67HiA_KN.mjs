...
import { n as compareSemverStrings, t as checkUpdateStatus } from "./update-check-iRYMjMOk.mjs";
...
async function getUpdateCheckResult(params) {
	...
	if (update.installKind === "git" && update.git && !params.fetchGit) {
		const stale = await import("./update-run-ledger-D1vR-znz.mjs").then(({ getLatestUpdateFetchFailure }) => getLatestUpdateFetchFailure()).catch(() => void 0);
		if (stale) update.git = {
			...update.git,
			stale,
			countsCached: true
