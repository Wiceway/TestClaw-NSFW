...
async function resolveRouteRpcOptions(args, deps) {
	...
	const config = await (deps.readNonObservingHealthConfig ?? (await import("./health-rSp49rDE.mjs")).readNonObservingHealthConfig)();
	return {
		...args.rpc,
...
async function runGatewayHealthJsonRoute(args, runtime, deps = {}) {
	...
	try {
		...
		const callGateway = deps.callGateway ?? (await import("./gateway-rpc-DL0RWIot.mjs")).callGatewayFromCliWithTransport;
		...
		if (!rpc) throw error;
		const [healthModule, callModule] = await Promise.all([deps.emitReachableGatewayAuthDiagnostic && deps.readNonObservingHealthConfig ? void 0 : import("./health-rSp49rDE.mjs"), deps.formatGatewayAuthErrorJson && deps.formatGatewayClientRequestErrorJson && deps.formatGatewayTransportErrorJson ? void 0 : import("./call-BFsjnbnk.mjs")]);
