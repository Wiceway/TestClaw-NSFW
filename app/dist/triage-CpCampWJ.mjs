...
import { a as withInstallationTarget, i as resolveInstallationTarget, r as installationTargetEnv } from "./installation-target-context-ybuwJO_0.mjs";
...
import { n as readTriageUpdateFailure, r as sanitizeTriageUpdateFailure, t as readPendingTriageUpdateFailure } from "./triage-update-Bun224Jd.mjs";
...
import { t as acceptTriageContinuation } from "./triage-continuation-BfjN4tvG.mjs";
...
async function collectTriageBundle(skipExport, redaction) {
	...
	try {
		...
		const [{ writeDiagnosticSupportExport }, { gatherDaemonStatus }] = await Promise.all([import("./diagnostic-support-export-CsNuQrvg.mjs"), import("./status.gather-BS8W3y34.mjs")]);
		return {
			kind: "available",
...
async function triageCommand(runtime, options = {}, automatic) {
	...
	if (!deferDiagnostics) try {
		const { collectDoctorFindings } = await import("./doctor-lint-BF2PEtd2.mjs");
	...
	if (!runEmbedded) {
		...
		if (handoff.agent === "claude" && !automatic) {
			const { probeClaudeSafeMode } = await import("./triage-claude-DOO7wF2U.mjs");
		...
		try {
			if (automatic) {
				const { runUtf8CommandWithTimeout } = await import("./exec-DTHFtwAf.mjs");
	...
	const { runUpdateRepairLoop } = await import("./update-repair-agent-N2O9gFYA.mjs");
		...
		validate: async (signal) => {
			try {
				const validateDoctor = async () => {
					const { validateTriageDoctor } = await import("./triage-doctor-BDF7RrTG.mjs");
				...
				const { validateTriageUpdateResolution } = await import("./update-triage-resolution-BiXyF35L.mjs");
