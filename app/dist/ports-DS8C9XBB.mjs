...
import { a as shouldLogVerbose, n as info, s as warn, t as danger } from "./globals-CUJhO5PM.mjs";
...
async function describePortOwner(port) {
	const { inspectPortUsage } = await import("./ports-inspect-Bbr63XHq.mjs");
...
async function handlePortError(err, port, context, runtime = defaultRuntime) {
	if (err instanceof PortInUseError || isErrno(err) && err.code === "EADDRINUSE") {
		...
		if (details) {
			...
			if (/testclaw|src\/index\.ts|dist\/index\.js/.test(details)) runtime.error(warn("It looks like another Assistant instance is already running. Stop it or pick a different port."));
		}
