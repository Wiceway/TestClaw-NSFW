...
import "./browser-network-o8BMjpgh.mjs";
import { n as resolveSandboxScope, t as resolveSandboxDockerEnv } from "./config-contract-CFOz6uqW.mjs";
import { p as DEFAULT_SANDBOX_WORKSPACE_ROOT, t as DEFAULT_SANDBOX_BROWSER_AUTOSTART_TIMEOUT_MS } from "./constants-DLwYYb7M.mjs";
...
const DEFAULT_SANDBOX_SSH_WORKSPACE_ROOT = "/tmp/testclaw-sandboxes";
...
function resolveSandboxBrowserDockerCreateConfig(params) {
	...
	const base = {
		...params.docker,
		network: browserNetwork || "testclaw-sandbox-browser",
...
function resolveSandboxDockerConfig(params) {
	...
	return {
		image: agentDocker?.image ?? globalDocker?.image ?? "testclaw-sandbox:bookworm-slim",
		containerPrefix: agentDocker?.containerPrefix ?? globalDocker?.containerPrefix ?? "testclaw-sbx-",
...
function resolveSandboxBrowserConfig(params) {
	...
	return {
		...
		image: agentBrowser?.image ?? globalBrowser?.image ?? "testclaw-sandbox-browser:bookworm-slim",
		containerPrefix: agentBrowser?.containerPrefix ?? globalBrowser?.containerPrefix ?? "testclaw-sbx-browser-",
		network: agentBrowser?.network ?? globalBrowser?.network ?? "testclaw-sandbox-browser",
