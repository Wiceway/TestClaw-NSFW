...
import "./config-cli-path-CJ3CYGsf.mjs";
import { i as formatSystemAgentPersistentPlan } from "./operations-parse-D2_r8HP6.mjs";
import { a as sameDefaultInferenceRoute, n as projectDefaultInferenceRoute, r as projectInferenceRoute } from "./inference-route-DTlrGGL8.mjs";
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-CLVFcCHs.mjs");
	if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
	const listPlugins = deps?.listChannelSetupPlugins ?? (await import("./setup-registry-DHPt8SRf.mjs")).listChannelSetupPlugins;
	const resolveEntries = deps?.resolveChannelSetupEntries ?? (await import("./discovery-CgWyTXm7.mjs")).resolveChannelSetupEntries;
	const isConfigured = deps?.isChannelConfigured ?? (await import("./channel-configured-shared-CnE01POs.mjs")).isStaticallyChannelConfigured;
	const { shouldShowChannelInSetup } = await import("./discovery-CgWyTXm7.mjs");
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-daSbsV4P.mjs");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-DqegeOVK.mjs")).applySystemAgentSetup;
			const surface = ctx.deps?.setupSurface ?? "cli";
			const recovery = surface === "cli" ? await (await import("./setup-recovery-nPALUWyN.mjs")).loadLocalSetupRecovery(operation.workspace) : void 0;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-model-selection-DthnADDc.mjs");
...
async function isPluginBackingDefaultInferenceRoute(pluginId) {
	...
	const { resolveModelRuntimePolicy } = await import("./model-runtime-policy-BVXCJkVF.mjs");
	...
	const { resolveOwningPluginIdsForProviderRef } = await import("./providers-DbX0qnZY.mjs");
