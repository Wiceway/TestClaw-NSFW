import { b as runWithGatewayDetachedWorkAdmission, f as isGatewayWorkAdmissionClosed } from "./gateway-work-admission-DeFm4gyw.mjs";
	...
	const owner = {
		...
		run: () => {
			...
			inFlight = runWithGatewayDetachedWorkAdmission(async () => {
				await previousDrain;
				assertCurrent();
				const { runSessionColdStorageMaintenance } = await import("./session-cold-storage-Ckzw8iTJ.mjs");
				assertCurrent();
