...
import { f as shouldDeferConfiguredPluginInstallRepair, s as isLegacyParentWritableUpdateDoctorPass } from "./update-phase-ygr4tw0u.mjs";
import { t as emitDoctorNotes } from "./emit-notes-BF8NdCNQ.mjs";
import { t as scrubDoctorErrorMessage } from "./doctor-error-message-CBAGGfGW.mjs";
import { c as throwIfDoctorStateMigrationRefused, t as DoctorStateMigrationRefusalError } from "./state-migrations.messages-Q7LpdcwR.mjs";
...
import { n as noteBackupDoctorHint } from "./backup-health-CEeSSg7Y.mjs";
import { t as resolveDoctorWorkspaceSuggestionScopes } from "./doctor-workspace-suggestion-scopes-DIFZQOOu.mjs";
import { n as normalizeHealthCheck } from "./health-check-adapter-BrC920MN.mjs";
import { n as resolveDoctorMode, r as resolveDoctorWorkspaceDir, t as isUpdateDoctorRun } from "./doctor-health-contribution-utils-n0tZCxfB.mjs";
import { n as recordDoctorHealthWarnings, t as createDoctorHealthContribution } from "./doctor-health-contribution-fh6TmQ3x.mjs";
import { n as runCoreHealthFindingNote, r as runStructuredHealthRepairs, t as runCoreContributionHealth } from "./doctor-health-contribution-core-Cb6GL9X4.mjs";
import { a as runWriteConfigHealth, n as runFinalConfigValidationHealth, r as runInitialConfigWriteHealth, t as collectWriteConfigHealthFindings } from "./doctor-health-contribution-runners.config-DpRyTO5h.mjs";
import { a as runGatewayDaemonHealth, c as runHostDesktopHealth, d as runStartupChannelMaintenanceHealth, f as runWebFetchProxyHealth, i as runDevicePairingHealth, l as runOpenAIOAuthTlsHealth, n as runClaudeCliHealth, o as runGatewayServicesHealth, p as runWhatsappResponsivenessHealth, r as runCommandOwnerHealth, s as runGitHubProjectHealth, t as runBrowserHealth, u as runSecurityHealth } from "./doctor-health-contribution-runners.gateway-tXIlFnOk.mjs";
...
async function runAuthProfileMigration(ctx) {
	...
	const { repairAuthProfileMigration } = await import("./auth-profile-repair-DCEOTnMx.mjs");
	const { maybeRepairLegacyOAuthProfileIds } = await import("./doctor-auth-legacy-oauth-DGNE8KCY.mjs");
	const { maybeRepairLegacyOAuthSidecarProfiles } = await import("./doctor-auth-oauth-sidecar-Bl0HnpmY.mjs");
	const { maybeMigrateLegacyPluginModelCatalogs } = await import("./doctor-plugin-model-catalog-Bvz_uvrW.mjs");
	const { buildGatewayConnectionDetails } = await import("./call-BFsjnbnk.mjs");
	...
	const { maybeMigrateModelCatalogCredentials } = await import("./doctor-model-catalog-credentials-DsRQkdso.mjs");
	...
	if (ctx.configResult.retiredAuthProfileCleanupPlans?.length || ctx.configResult.openAICodexAuthProfileIdMap?.size) {
		const { runRetiredAuthProfileCleanup, runWriteConfigHealth } = await import("./doctor-health-contribution-runners.config-DTrPFnWb.mjs");
...
async function runAuthProfileDiagnostics(ctx) {
	const { noteAuthProfileHealth, noteCopilotAmbientToken, noteLegacyCodexProviderOverride, noteSharedAuthStoreStatus } = await import("./doctor-auth-DfdTOWDb.mjs");
...
const loadDoctorStateIntegrityModule$1 = async () => await import("./doctor-state-integrity-BIKGm836.mjs");
...
async function collectWorkspaceStatusPluginVersionReadiness(params) {
	...
	try {
		const { gatherDaemonStatus } = await import("./status.gather-BS8W3y34.mjs");
		...
		if (status.pluginVersionRestartReadiness?.status === "resolved") {
			const { resolvePluginVersionDriftTargets } = await import("./plugin-version-drift-XAjCBTr9.mjs");
			return {
...
async function runWorkspaceStatusHealth(ctx) {
	...
	const { noteWorkspaceStatus } = await import("./doctor-workspace-status-XvqTZVmi.mjs");
...
async function runWorkspaceAliasHealth(ctx) {
	const { collectRepointedWorkspaceAliasFindings } = await import("./doctor-workspace-alias-BORenBQy.mjs");
...
async function runSkillsHealth(ctx) {
	const { maybeRepairSkillReadiness } = await import("./doctor-skills-DORSftVS.mjs");
...
async function runBootstrapSizeHealth(ctx) {
	const { noteBootstrapFileSize } = await import("./doctor-bootstrap-size-BAxewan0.mjs");
	await noteBootstrapFileSize(ctx.cfg);
}
async function runHeartbeatCadenceMigrationHealth(ctx) {
	const { maybeMigrateHeartbeatCadenceToCron } = await import("./doctor-heartbeat-cadence-migration-C4wB3DMx.mjs");
...
async function runHeartbeatScratchMigrationHealth(ctx) {
	const { maybeMigrateHeartbeatFilesToScratch } = await import("./doctor-heartbeat-scratch-migration-DznqNyUb.mjs");
...
async function runToolsMdMigrationHealth(ctx) {
	const { maybeMigrateToolsMd } = await import("./doctor-tools-md-migration-DRKDHgpu.mjs");
...
async function runHeartbeatTaskMigrationHealth(ctx) {
	const { maybeMigrateHeartbeatTasksToCron } = await import("./doctor-heartbeat-task-migration-BGGqdxag.mjs");
...
async function runMemorySearchHealthContribution(ctx) {
	const { maybeRepairMemoryRecallHealth, noteMemoryRecallHealth, noteMemorySearchHealth } = await import("./doctor-memory-search-By6VWT3q.mjs");
...
async function collectMemorySearchHealthFindings(ctx) {
	const { noteMemorySearchHealth } = await import("./doctor-memory-search-By6VWT3q.mjs");
	const notes = [];
	await noteMemorySearchHealth(ctx.cfg, {
		env: ctx.env,
...
async function runWorkspaceSuggestionsHealth(ctx) {
	...
	const { MEMORY_SYSTEM_PROMPT, shouldSuggestMemorySystem } = await import("./doctor-workspace-DCKAHima.mjs");
	const { note } = await import("./terminal-core/note.js");
	...
	return [
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				async detect(ctx) {
					const { collectHostDesktopHealthFindings } = await import("./doctor-host-desktop-GGElz3Au.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectMissingDefaultAccountBindingWarnings, collectMissingExplicitDefaultAccountWarnings } = await import("./default-account-warnings-CvXI8a8L.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				async detect(ctx) {
					...
					const { collectBundledChannelPackageStateLoadFailures } = await import("./package-state-probes-Cf1M8oj9.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: [{
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { channelPluginBlockerHitToHealthFinding, scanConfiguredChannelPluginBlockers } = await import("./channel-plugin-blockers-BZjfcKNN.mjs");
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectChannelPreviewWarningHealthFindings } = await import("./doctor-startup-channel-maintenance-DkJlOr7n.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectWorkspaceStatusHealthFindings } = await import("./doctor-workspace-status-XvqTZVmi.mjs");
					const pluginVersionReadiness = await collectWorkspaceStatusPluginVersionReadiness({
						cfg: ctx.cfg,
						options: {
							nonInteractive: true,
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: true,
				async detect(ctx) {
					const { collectHeartbeatCadenceMigrationFindings } = await import("./doctor-heartbeat-cadence-migration-C4wB3DMx.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: true,
				async detect(ctx) {
					const { collectHeartbeatScratchMigrationFindings } = await import("./doctor-heartbeat-scratch-migration-DznqNyUb.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: true,
				async detect(ctx) {
					const { collectToolsMdMigrationFindings } = await import("./doctor-tools-md-migration-DRKDHgpu.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: true,
				async detect(ctx) {
					const { collectHeartbeatTaskMigrationFindings } = await import("./doctor-heartbeat-task-migration-BGGqdxag.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectWhatsappResponsivenessHealthFindings } = await import("./doctor-whatsapp-responsiveness-DJYxKKsQ.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectDevicePairingHealthFindings } = await import("./doctor-device-pairing-BQF-EQEZ.mjs");
...
const loadDoctorStateIntegrityModule = async () => await import("./doctor-state-integrity-BIKGm836.mjs");
async function runLegacyPluginManifestHealth(ctx) {
	const { maybeRepairLegacyPluginManifestContracts } = await import("./doctor-plugin-manifests-4-e2qhu4.mjs");
...
async function runPluginRegistryHealth(ctx) {
	const { maybeRepairPluginRegistryState } = await import("./doctor-plugin-registry-B6XMTpEq.mjs");
...
async function runLegacyPluginSourceCapturesHealth(ctx) {
	const { noteLegacyPluginSourceCaptures } = await import("./doctor-plugin-source-captures-CZfLDL3e.mjs");
...
async function runReleaseConfiguredPluginInstallsHealth(ctx) {
	...
	const { maybeRunConfiguredPluginInstallReleaseStep } = await import("./release-configured-plugin-installs-CwAGycky.mjs");
	const { note } = await import("./terminal-core/note.js");
...
async function runDiskSpaceHealth() {
	const { noteDiskSpace } = await import("./doctor-disk-space-C2FcCHBW.mjs");
	noteDiskSpace();
}
async function runDatabaseBloatHealth() {
	const { noteSqliteDatabaseBloat } = await import("./doctor-db-bloat-B3fQj6kq.mjs");
	await noteSqliteDatabaseBloat();
}
async function runAgentMemorySchemaHealth(ctx) {
	const { noteDoctorAgentMemorySchemaHealth } = await import("./doctor-agent-memory-schema-BcIka_ZQ.mjs");
...
async function runChannelIngressDeadLettersHealth() {
	const { noteChannelIngressDeadLetters } = await import("./doctor-channel-ingress-BWcHKEOR.mjs");
	noteChannelIngressDeadLetters();
}
async function runStateIntegrityHealth(ctx) {
	const { noteDoctorAgentDatabasePathHealth } = await import("./doctor-agent-database-paths-B440Ql3V.mjs");
	...
	const { noteBackupScratchHealth } = await import("./doctor-backup-scratch-BnhboUCQ.mjs");
	await noteBackupScratchHealth(ctx.env ?? process.env, ctx.prompter.shouldRepair);
}
async function runCodexSessionRouteHealth(ctx) {
	const { maybeRepairCodexSessionRoutes } = await import("./codex-route-warnings-4p0g02Hs.mjs");
	const { note } = await import("./terminal-core/note.js");
...
async function runSessionTranscriptHeadersHealth(ctx) {
	const { noteSessionTranscriptHeaderHealth } = await import("./doctor-session-transcript-headers-CJhozXn5.mjs");
...
async function runSessionTranscriptLabelsHealth(ctx) {
	const { noteSessionTranscriptLabelHealth } = await import("./doctor-session-transcript-labels-qhsl-Jxw.mjs");
...
async function runSessionSnapshotsHealth(ctx) {
	const { noteSessionSnapshotHealth } = await import("./doctor-session-snapshots-C-iN5RNc.mjs");
...
async function runConfigAuditScrubHealth(ctx) {
	const { maybeRepairLegacyRuntimeFiles } = await import("./doctor-usage-cost-cache-D_qclq1R.mjs");
	await maybeRepairLegacyRuntimeFiles(ctx.prompter.shouldRepair, ctx.env);
}
async function runLegacyCronHealth(ctx) {
	const { maybeRepairLegacyCronStore, noteLegacyWhatsAppCrontabHealthCheck } = await import("./cron-CqjGZqoo.mjs");
...
async function runSandboxHealth(ctx) {
	const { maybeRepairSandboxImages, maybeRepairSandboxRegistryFiles, noteSandboxScopeWarnings } = await import("./doctor-sandbox-DF_7G_Ae.mjs");
...
async function runStaleRuntimeBuildHealth(ctx) {
	const { runCoreContributionHealth } = await import("./doctor-health-contribution-core-DPiBI82M.mjs");
	...
	return [
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				async detect(ctx) {
					const { evaluateAgentDatabaseAdmissions } = await import("./agent-database-admission-BYxwaEpV.mjs");
		...
		createDoctorHealthContribution({
			...
			async run(ctx) {
				const { runCoreHealthFindingNote } = await import("./doctor-health-contribution-core-DPiBI82M.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectAuthProfileHealthFindings } = await import("./doctor-auth-DfdTOWDb.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				async detect(ctx) {
					const { collectNodeHostingPreconditionFindings } = await import("./doctor-node-hosting-preconditions-B7pneB7n.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect() {
					const { detectSessionTranscriptHealthIssues, sessionTranscriptIssueToHealthFinding } = await import("./doctor-session-transcripts-B_2X1kjn.mjs");
					return (await detectSessionTranscriptHealthIssues()).map(sessionTranscriptIssueToHealthFinding);
				},
				repair: legacyOwnedRepair(async () => {
					const { detectSessionTranscriptHealthIssues, sessionTranscriptIssueToRepairEffect } = await import("./doctor-session-transcripts-B_2X1kjn.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectLegacyPluginManifestContractMigrations, legacyPluginManifestContractMigrationToHealthFinding } = await import("./doctor-plugin-manifests-4-e2qhu4.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect() {
					const { collectStalePluginRuntimeSymlinkHealthFindings } = await import("./plugin-runtime-symlinks-C2v45FN4.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { detectConfiguredPluginInstallHealthIssues, configuredPluginInstallIssueToHealthFinding } = await import("./missing-configured-plugin-install-CH62redj.mjs");
				...
				repair: legacyOwnedRepair(async (ctx) => {
					const { detectConfiguredPluginInstallHealthIssues, configuredPluginInstallIssueToRepairEffect } = await import("./missing-configured-plugin-install-CH62redj.mjs");
					return (await detectConfiguredPluginInstallHealthIssues({
						cfg: ctx.cfg,
						env: process.env
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { detectPluginRegistryHealthIssues, pluginRegistryIssueToHealthFinding } = await import("./doctor-plugin-registry-B6XMTpEq.mjs");
				...
				repair: legacyOwnedRepair(async (ctx) => {
					const { detectPluginRegistryHealthIssues, pluginRegistryIssueToRepairEffect } = await import("./doctor-plugin-registry-B6XMTpEq.mjs");
					return (await detectPluginRegistryHealthIssues({
						config: ctx.cfg,
						env: process.env,
						prompter: { shouldRepair: false }
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect() {
					const { collectDiskSpaceHealthFindings } = await import("./doctor-disk-space-C2FcCHBW.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { collectProjectCloneShapeHealthFindings } = await import("./doctor-project-clone-shape-CNLFeb_X.mjs");
					return await collectProjectCloneShapeHealthFindings(ctx.cfg);
				}
			},
			async run(ctx) {
				const { noteProjectCloneShape } = await import("./doctor-project-clone-shape-CNLFeb_X.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { detectStateIntegrityHealthIssues, stateIntegrityIssueToHealthFinding } = await import("./doctor-state-integrity-BIKGm836.mjs");
				...
				repair: legacyOwnedRepair(async (ctx) => {
					const { detectStateIntegrityHealthIssues, stateIntegrityIssueToRepairEffect } = await import("./doctor-state-integrity-BIKGm836.mjs");
					return detectStateIntegrityHealthIssues(ctx.cfg, {
						configPath: ctx.configPath,
						env: ctx.env ?? process.env
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect(ctx) {
					const { detectSessionSnapshotHealthIssues, sessionSnapshotIssueToHealthFinding } = await import("./doctor-session-snapshots-C-iN5RNc.mjs");
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				defaultEnabled: false,
				async detect() {
					const { configAuditScrubToHealthFinding, detectConfigAuditScrubIssue } = await import("./doctor-config-audit-scrub-NChIp3Ch.mjs");
					const result = await detectConfigAuditScrubIssue();
					return result.rewritten > 0 ? [configAuditScrubToHealthFinding(result)] : [];
				},
				repair: legacyOwnedRepair(async () => {
					const { configAuditScrubToRepairEffect, detectConfigAuditScrubIssue } = await import("./doctor-config-audit-scrub-NChIp3Ch.mjs");
					const result = await detectConfigAuditScrubIssue();
		...
		createDoctorHealthContribution({
			...
			healthChecks: {
				...
				async detect() {
					const { detectLegacySandboxRegistryFileIssues, legacySandboxRegistryInspectionToHealthFinding } = await import("./doctor-sandbox-DF_7G_Ae.mjs");
					return (await detectLegacySandboxRegistryFileIssues()).map(legacySandboxRegistryInspectionToHealthFinding);
				},
				repair: legacyOwnedRepair(async () => {
					const { detectLegacySandboxRegistryFileIssues, legacySandboxRegistryInspectionToRepairEffect } = await import("./doctor-sandbox-DF_7G_Ae.mjs");
...
const loadCommandFormatModule = async () => await import("./command-format-OzrSP4gw.mjs");
const loadNoteModule = async () => await import("./terminal-core/note.js");
const loadOnboardHelpersModule = async () => await import("./onboard-helpers-ByjrA4hV.mjs");
const loadSecretTypesModule = async () => await import("./types.secrets-DieHS7xT.mjs");
...
async function reportDeferredLegacyState(ctx) {
	...
	const [{ detectLegacyStateMigrations }, { prepareLegacySessionSurfaces }] = await Promise.all([import("./state-migrations.doctor-BOeyWLNE.mjs"), import("./legacy-session-surfaces-rVUtwmOE.mjs")]);
...
async function runGatewayConfigHealth(ctx) {
	...
	const { hasAmbiguousGatewayAuthModeConfig } = await import("./auth-mode-policy-CqbXjw5e.mjs");
	const { note } = await loadNoteModule();
...
async function runGatewayAuthHealth(ctx) {
	...
	const { detectGatewayAuthHealth } = await import("./doctor-gateway-auth-DVqr_GIt.mjs");
	...
	if (gatewayTokenRef) {
		if (gatewayTokenRef.source === "store" && finding.requirement === "SECRET_REF_REDACTED_VALUE" && (ctx.options.generateGatewayToken === true || ctx.options.repair === true || ctx.options.yes === true)) {
			const { isRedactedSecretValue } = await import("./redact-sentinel-CV0uM3Dl.mjs");
			const { readSecretStoreValue, writeSecretStoreEntryWithRollback } = await import("./secret-store-B8Q8hsMv.mjs");
			const { createVerifiedSqliteSnapshot } = await import("./sqlite-snapshot-Bbx3GSor.mjs");
			const { resolveAssistantStateSqlitePath } = await import("./testclaw-state-db.paths-ISNX87Xn.mjs");
...
async function runLegacyStateHealth(ctx) {
	const { detectLegacyStateMigrations, runLegacyStateMigrations } = await import("./state-migrations.doctor-BOeyWLNE.mjs");
	const { note } = await loadNoteModule();
	...
	const { repairObsoleteGeneratedExecApprovals } = await import("./exec-approvals-generated-migration-D-gXGJi9.mjs");
	const { ExecApprovalsMigrationRequiredError } = await import("./exec-approvals-migration-gate-B4oXDj6h.mjs");
...
async function hasUserScopedSystemdGatewayService(env) {
	const { findInstalledSystemdGatewayScope } = await import("./systemd-B6ATIqbo.mjs");
...
async function runSystemdLingerHealth(ctx) {
	...
	const { readGatewayServiceState, resolveGatewayService } = await import("./service-DPsoBeZn.mjs");
...
async function detectSystemdLingerFindings(ctx) {
	...
	const { readGatewayServiceState, resolveGatewayService } = await import("./service-DPsoBeZn.mjs");
	if ((await readGatewayServiceState(resolveGatewayService(), { env: process.env })).loadState.status !== "loaded") return [];
	const { isSystemdUserServiceAvailable, readSystemdUserLingerStatus, resolveSystemdUserServiceAccount } = await import("./systemd-B6ATIqbo.mjs");
...
async function runShellCompletionHealth(ctx) {
	const { doctorShellCompletion } = await import("./doctor-completion-hw2E8rcW.mjs");
...
async function runGatewayHealthChecks(ctx) {
	...
	const { checkGatewayHealth, probeGatewayMemoryStatus } = await import("./doctor-gateway-health-o0Lj5X5O.mjs");
...
async function resolveDoctorContributionHealthChecks() {
	const { createCoreHealthChecks } = await import("./doctor-core-checks-CitN1WQH.mjs");
