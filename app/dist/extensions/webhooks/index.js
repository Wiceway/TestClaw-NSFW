...
import { E as string, T as strictObject, _ as literal, b as number, f as boolean, h as json, k as union, l as _enum, m as discriminatedUnion, w as record, x as object } from "../../schemas-qz0osXyE.mjs";
import { H as ZodIssueCode } from "../../zod-schema.core-v-bbtNf8.mjs";
...
import { f as withResolvedWebhookRequestPipeline, n as normalizeWebhookPath, u as resolveWebhookTargetWithAuthOrRejectSync } from "../../webhook-targets-BFLfUbJU.mjs";
import "../../api-UL1Zrowr.mjs";
...
//#endregion
//#region extensions/webhooks/src/http-request-schema.ts
const jsonValueSchema = json();
const nullableStringSchema = string().trim().min(1).nullable().optional();
const createFlowRequestSchema = strictObject({
...
const getFlowRequestSchema = strictObject({
	action: literal("get_flow"),
	flowId: string().trim().min(1)
});
const listFlowsRequestSchema = strictObject({ action: literal("list_flows") });
const findLatestFlowRequestSchema = strictObject({ action: literal("find_latest_flow") });
const resolveFlowRequestSchema = strictObject({
	action: literal("resolve_flow"),
	token: string().trim().min(1)
});
const getTaskSummaryRequestSchema = strictObject({
	action: literal("get_task_summary"),
	flowId: string().trim().min(1)
});
const setWaitingRequestSchema = strictObject({
...
const resumeFlowRequestSchema = strictObject({
...
const finishFlowRequestSchema = strictObject({
	action: literal("finish_flow"),
	flowId: string().trim().min(1),
	expectedRevision: number().int().nonnegative(),
	stateJson: jsonValueSchema.nullable().optional()
});
const failFlowRequestSchema = strictObject({
...
const requestCancelRequestSchema = strictObject({
	action: literal("request_cancel"),
	flowId: string().trim().min(1),
	expectedRevision: number().int().nonnegative()
});
const cancelFlowRequestSchema = strictObject({
	action: literal("cancel_flow"),
	flowId: string().trim().min(1)
});
const runTaskRequestSchema = strictObject({
...
function extractSharedSecret(req) {
	...
	const sharedHeader = req.headers["x-testclaw-webhook-secret"];
...
async function executeWebhookAction(params) {
	...
	switch (action.action) {
		...
		case "list_flows": return { flows: (await target.taskFlow.list()).map(toFlowView) };
		...
		case "get_task_summary": return { summary: await target.taskFlow.getTaskSummary(action.flowId) ?? null };
		case "set_waiting": return mapFlowMutationResult(await target.taskFlow.setWaiting({
		...
		case "resume_flow": return mapFlowMutationResult(await target.taskFlow.resume({
			flowId: action.flowId,
			expectedRevision: action.expectedRevision,
			status: action.status,
			currentStep: action.currentStep,
		...
		case "finish_flow": return mapFlowMutationResult(await target.taskFlow.finish({
			flowId: action.flowId,
			expectedRevision: action.expectedRevision,
			stateJson: action.stateJson
		}));
		case "fail_flow": return mapFlowMutationResult(await target.taskFlow.fail({
			flowId: action.flowId,
			expectedRevision: action.expectedRevision,
			stateJson: action.stateJson,
			blockedTaskId: action.blockedTaskId,
		...
		case "request_cancel": return mapFlowMutationResult(await target.taskFlow.requestCancel({
			flowId: action.flowId,
			expectedRevision: action.expectedRevision
		}));
		case "cancel_flow": {
...
var webhooks_default = definePluginEntry({
	id: "webhooks",
	name: "Webhooks",
	description: "Authenticated inbound webhooks that bind external automation to Assistant TaskFlows.",
	register(api) {
