...
const MAX_EDIT_PREDICTION_RESPONSE_BYTES = 131072;
...
var Editor = class {
	...
	*/
	...
	__syncRenderView(props) {
		...
		for (const el of shadowRoot.querySelectorAll("[data-code]")) {
			...
			for (const child of el.children) {
				...
				if (gutter != null) {
					if (isDeletions) deletionsGutterEl = child;
					else gutterEl = child;
				} else if (content != null) {
					if (isDeletions) deletionsContentEl = child;
		...
		this.#globalEventDisposes = [
			...
			addEventListener(document, "selectionchange", () => {
				...
				if (this.#isContentMouseDown) {
					if (this.#selectionStart !== void 0) selection = createSelectionFrom(this.#selectionStart, selection);
	...
	#runCommand(command) {
		...
		switch (command) {
			...
			case "expandSelectionDocEnd":
				...
				break;
			case "undo":
			case "redo": this.#applyHistoryChange(command);
		}
	}
	...
	#handleInput(inputType, data) {
		switch (inputType) {
			...
			case "insertTranspose":
				...
				break;
			default: console.warn(`[diffs] Unknown input type: ${inputType}`, data);
		}
	}
	...
	#deleteSoftLineBackward() {
		...
		const textDocument = this.#editSession?.document;
		if (selections === void 0 || textDocument === void 0) return;
		const getSoftLineStart = this.#isWrap ? (line, character) => {
			const wrapOffsets = this.#wrapLineTextOrWholeLine(line);
		...
		const { nextSelections, change } = applyDeleteSoftLineBackwardToSelections(textDocument, selections, getSoftLineStart, this.#lineAnnotations);
		if (change !== void 0) this.#applyChange(change, nextSelections, this.#applyChangeToLineAnnotations(change));
	}
	#deleteWordBackward() {
		const selections = this.#selections;
