...
import { isHighlighterLoaded, preloadHighlighter } from "../highlighter/shared_highlighter.js";
...
const SCROLL_REBASE_TARGET_BOTTOM = 1e7;
...
var CodeView = class CodeView {
	...
	reset() {
		...
		const teardownSessions = [...this.itemEditors.values()].map((record) => {
			...
			if (item != null) record.state.removedItem = item.item;
			return {
				record,
				instance: item?.instance
			};
		...
		let failed = false;
		let failure;
		for (const { record, instance } of teardownSessions) try {
	...
	*/
	appendItemsInternal(inputs, render = true) {
		...
		this.scrollDirty = true;
		if (render) {
			if (this.canSkipRenderForAppend(appendedTop)) this.syncContainerHeight();
			else this.render();
		}
	}
	...
	isReady() {
		...
		if (workerManager == null || workerManager.getStats().workersFailed) return this.isSharedHighlighterReady();
	...
	clearReadySubscription() {
		...
		this.pendingHighlighterTheme = void 0;
	}
	isSharedHighlighterReady() {
		const theme = this.workerManager?.getFileRenderOptions().theme ?? this.options.theme ?? DEFAULT_THEMES;
		if (isHighlighterLoaded() && areThemesAttached(theme)) {
			this.clearReadySubscription();
			return true;
		}
		if (!areThemesEqual(this.pendingHighlighterTheme, theme)) this.clearReadySubscription();
		this.isReadySubscription ??= (() => {
	...
	attachItemEditor(item) {
		...
		try {
			if (record == null) {
				...
				if (state == null) throw new Error("CodeView.attachItemEditor: instance options are missing their state");
			...
			} else if (item.type === "diff") record.dispose = record.editor.edit(item.instance);
			else record.dispose = record.editor.edit(item.instance);
	...
	syncItemEditors(removedItems) {
		...
		for (const [id, record] of this.itemEditors) {
			...
			if (removedItem == null && item != null) {
				...
				if (edit) {
					if (collapsed && this.attachedEditors.delete(id)) record.editor.cleanUp("recycle");
					continue;
				}
			} else if (removedItem != null) record.state.removedItem = removedItem.item;
		...
		for (const record of completions) {
			...
			const item = this.idToItem.get(record.state.id);
			if (item != null) this.markItemLayoutDirty(item);
		}
		this.render();
		if (failed) throw failure;
	}
	requireItemFromState(state) {
		const item = this.idToItem.get(state.id)?.item ?? state.removedItem;
		if (item == null) throw new Error(`CodeView.onItemEditComplete: unknown item "${state.id}"`);
		return item;
	}
	/**
	...
	*/
	completeFileItemEdit(state, event) {
		const item = this.requireItemFromState(state);
		if (item.type !== "file") throw new Error(`CodeView.onItemEditComplete: item "${state.id}" is not a file item`);
		const nextItem = {
			...item,
			file: event.file,
		...
		if ((this.options.onItemEditComplete?.(event, item, nextItem) ?? "reject") === "reject") return "reject";
		if (this.idToItem.has(item.id)) this.updateItem(nextItem);
		return "accept";
	}
	/**
	...
	*/
	completeDiffItemEdit(state, event) {
		const item = this.requireItemFromState(state);
		if (item.type !== "diff") throw new Error(`CodeView.onItemEditComplete: item "${state.id}" is not a diff item`);
		const nextItem = {
			...item,
	...
	createFileOptionsPrototype() {
		...
		defineItemOption(prototype, "onEditComplete", (receiver) => {
			if (this.options.onItemEditComplete == null) return;
			const state = getItemOptionsState(receiver);
			if (state == null) return;
			return (event) => this.completeFileItemEdit(state, event);
		});
	...
	createDiffOptionsPrototype() {
		...
		defineItemOption(prototype, "onEditComplete", (receiver) => {
			...
			const state = getItemOptionsState(receiver);
			if (state == null) return;
			return (event) => this.completeDiffItemEdit(state, event);
		});
