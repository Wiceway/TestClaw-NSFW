...
const DEFAULT_WORKER_INITIALIZATION_TIMEOUT = 1e4;
...
var WorkerPoolManager = class {
	...
	async setRenderOptions({ theme = this.renderOptions.theme ?? DEFAULT_THEMES, useTokenTransformer = this.renderOptions.useTokenTransformer ?? false, lineDiffType = this.renderOptions.lineDiffType ?? "word-alt", maxLineDiffLength = this.renderOptions.maxLineDiffLength ?? 1e3, tokenizeMaxLineLength = this.renderOptions.tokenizeMaxLineLength ?? 1e3 }) {
		...
		try {
			...
			if (!areThemesEqual(newRenderOptions.theme, this.renderOptions.theme)) {
				if (hasResolvedThemes(themeNames)) resolvedThemes = getResolvedThemes(themeNames);
	...
	async initializeWorkers(resolvedThemes, resolvedLanguages) {
		...
		for (let i = 0; i < (this.options.poolSize ?? 8); i++) {
			...
			worker.addEventListener("error", (error) => this.handleWorkerError(managedWorker, error));
		...
		const timeoutMs = this.options.workerInitializationTimeout ?? DEFAULT_WORKER_INITIALIZATION_TIMEOUT;
		let timeoutId;
		try {
			await Promise.race([Promise.all(initPromises), new Promise((_, reject) => {
				timeoutId = setTimeout(() => {
					reject(/* @__PURE__ */ new Error(`WorkerPoolManager: worker initialization timed out after ${timeoutMs}ms`));
				}, timeoutMs);
			})]);
		} finally {
	...
	async resolveLanguagesAndExecuteTask(availableWorker, task, langs) {
		try {
			...
			if (mainThreadLangs.length > 0) getSharedHighlighter({
			...
			if (workerMissingLangs.length > 0) {
				if (hasResolvedLanguages(workerMissingLangs)) task.request.resolvedLanguages = getResolvedLanguages(workerMissingLangs);
...
function normalizeWorkerError(error) {
	...
	if (error != null && typeof error === "object") {
		if ("error" in error && error.error instanceof Error) return error.error;
