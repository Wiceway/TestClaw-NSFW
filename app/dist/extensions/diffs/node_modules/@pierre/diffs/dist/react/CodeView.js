...
import { assignRef } from "./utils/assignRef.js";
...
function CodeViewInner(props, ref) {
	...
	const createEditor = useStableCallback((editorType, options, editStateKey) => {
		...
		return contextCreateEditor(editorType, resolvedOptions, editStateKey);
	});
		...
		try {
			...
			if (controlled) {
				if (controlledItems !== prevItems) {
					if (areItemListsEqual(prevItems, controlledItems)) cachedDataRef.current.items = controlledItems;
