...
function FileDiffSSR({ prerenderedHTML, annotations, className, style, renderAnnotation }) {
	...
	if (htmlObjectRef.current === null) {
		const classAttr = className !== void 0 ? ` class="${escapeHtml(className)}"` : "";
		const styleAttr = style !== void 0 ? ` style="${serializeStyle(style)}"` : "";
		const annotationSlots = annotations !== void 0 && annotations.length > 0 && renderAnnotation != null ? annotations.map((annotation) => {
			return `<div slot="${getLineAnnotationName(annotation)}">${renderToString(renderAnnotation(annotation))}</div>`;
		}).join("") : "";
		htmlObjectRef.current = { __html: `<${DIFFS_TAG_NAME}${classAttr}${styleAttr}><template shadowrootmode="open">${prerenderedHTML}</template>${annotationSlots}</${DIFFS_TAG_NAME}>` };
	}
	useEffect(() => {
