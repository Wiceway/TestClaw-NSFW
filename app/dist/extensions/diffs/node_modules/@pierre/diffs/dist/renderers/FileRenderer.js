...
var FileRenderer = class {
	...
	*/
	beginEditSession(file, externalFile) {
		...
		if (wasAlreadyActive && renderCache.file === file) return;
		const { options } = this.getRenderOptions(file);
		const cacheBelongsToSession = renderCache.file === file;
		const cacheBelongsToExternal = externalFile != null && areFileTargetsEqual(renderCache.file, externalFile);
		const { result } = renderCache;
		if (!renderCache.highlighted || result == null || !areFileRenderOptionsEqual(renderCache.options, options) || !cacheBelongsToSession && !cacheBelongsToExternal) {
		...
		if (lineCache != null && externalFile != null && isLineCacheForFile(lineCache, externalFile)) this.lineCache = {
			cacheKey: void 0,
			file,
			sourceContents: file.contents,
			lines: lineCache.lines
		};
		else this.lineCache = void 0;
	}
	/**
	...
	*/
	endEditSession(settledFile) {
		this.editSessionActive = false;
		this.pendingHighlightResult = void 0;
		const { renderCache } = this;
		if (settledFile == null || renderCache == null || renderCache.file === settledFile || !areFileTargetsEqual(renderCache.file, settledFile)) return;
	...
	*/
	...
	getRenderOptions(file) {
		...
		if (!areFileTargetsEqual(file, renderCache.file) || !areFileRenderOptionsEqual(options, renderCache.options)) return {
			options,
			forceHighlight: true
		};
		return {
	...
	*/
	getFileForNextRender(file) {
		const { options } = this.getRenderOptions(file);
		if (this.getReadyRenderResult(file, options) != null) return file;
		const { renderCache } = this;
		if (renderCache == null) return file;
		if (areFileTargetsEqual(renderCache.file, file)) return renderCache.file;
		const lines = linesFromFileContents(file.contents);
		const forcePlainText = file.contents.length === 0 || isFilePlainText(file) || isFileMassive(lines.length, this.getTokenizeMaxLength());
		return this.canRenderFile(file, options, forcePlainText) ? file : renderCache.file;
	}
	canRenderFile(file, options, forcePlainText) {
		const { renderCache } = this;
		if (renderCache == null || areFileTargetsEqual(renderCache.file, file)) return true;
	...
	updateRenderCache(dirtyLines, themeType, lineCountChangeInFlight = false) {
		...
		for (const [line, tokens] of dirtyLines) {
			if (pendingStructuralRows == null && lineCache != null && line < lineCache.lines.length) {
	...
	renderFile(file = this.file, renderRange = DEFAULT_RENDER_RANGE) {
		this.file = file;
		if (file == null) {
			this.pendingHighlightResult = void 0;
			return;
		...
		if (!this.editSessionActive && this.workerManager?.isWorkingPool() === true) {
			...
			if (canRenderFile && !preserveHydratedContent && (forcePlainText || this.renderCache.result == null || !this.renderCache.highlighted && (newContent || newRenderRange))) {
			...
			if (canRenderFile && this.highlighter != null && hasThemes && (forceHighlight || forcePlainText || !this.renderCache.highlighted && canHighlight || this.renderCache.result == null)) {
	...
	processFileResult(file, renderRange, { code, themeStyles, baseThemeType }) {
		...
		if (fileLevelAnnotations != null) {
			...
			contentArray.push(createAnnotationElement({
				...
				hunkIndex: -1,
				lineIndex: -1,
				annotations: fileLevelAnnotations.map((annotation) => this.annotationSlotName(annotation))
			}));
			rowCount++;
		}
		for (let lineIndex = renderRange.startingLine; lineIndex < endLine; lineIndex++) {
			...
			if (annotations != null) {
				...
				contentArray.push(createAnnotationElement({
					...
					annotations: annotations.map((annotation) => this.annotationSlotName(annotation))
				}));
				rowCount++;
			}
		}
	...
	applyHighlightResult(file, result, options, highlighted = true) {
		...
		if (currentFile == null || renderCache == null || !areFileTargetsEqual(file, currentFile) || !areFileRenderOptionsEqual(options, this.getRenderOptions(currentFile).options)) return;
		if (!(renderCache.result == null || !renderCache.highlighted || !areFileRenderOptionsEqual(renderCache.options, options) || !areFileTargetsEqual(renderCache.file, currentFile))) return;
	...
	getReadyRenderResult(file, options) {
		const { pendingHighlightResult } = this;
		if (pendingHighlightResult != null && areFileTargetsEqual(pendingHighlightResult.file, file) && areFileRenderOptionsEqual(pendingHighlightResult.options, options)) return pendingHighlightResult;
		const workerCache = this.getMatchingWorkerResultCache(file, options);
		if (workerCache == null || this.hasHighlightedRenderCache(file, options)) return;
		return {
			file,
			highlighted: true,
			...workerCache
	...
	hasHighlightedRenderCache(file, options) {
		const { renderCache } = this;
		return renderCache?.result != null && renderCache.highlighted && areFileTargetsEqual(file, renderCache.file) && areFileRenderOptionsEqual(options, renderCache.options);
	}
