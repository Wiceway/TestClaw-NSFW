...
const NO_GHOST_TEXT_ROWS = /* @__PURE__ */ new Map();
let instanceId = -1;
var VirtualizedFileDiff = class extends FileDiff {
	...
	cache = {
		...
		ghostTextRowsByIndex: /* @__PURE__ */ new Map()
	};
	isVisible = false;
	isSetup = false;
	virtualizer;
	...
	syncLineAnnotations(lineAnnotations) {
		if (lineAnnotations == null || !this.isNewAnnotations(lineAnnotations) || lineAnnotations.length === 0 && this.getLatestAnnotations().length === 0) return false;
	...
	hasFileAnnotations(fileDiff) {
		const lineAnnotations = this.getLatestAnnotations();
		if (!includesFileAnnotations(lineAnnotations)) return false;
	...
	resetLayoutCache({ forceSimpleRecompute = false, includeEstimatedHeights = false, resetRenderRange = true } = {}) {
		...
		if (this.cache.ghostTextRowsByIndex.size > 0) this.cache.ghostTextRowsByIndex.clear();
	...
	applyGhostTextRows(fileDiff, ghostTextRows) {
		const { cache: { heightDeltas, ghostTextRows: folded, ghostTextRowsByIndex: previous }, metrics: { lineHeight } } = this;
		if (folded === ghostTextRows) return false;
		...
		let changed = false;
		for (const rowIndex of /* @__PURE__ */ new Set([...previous.keys(), ...next.keys()])) {
			const delta = ((next.get(rowIndex) ?? 0) - (previous.get(rowIndex) ?? 0)) * lineHeight;
			if (delta === 0) continue;
			const nextDelta = (heightDeltas.get(rowIndex) ?? 0) + delta;
			if (nextDelta === 0) heightDeltas.delete(rowIndex);
			else heightDeltas.set(rowIndex, nextDelta);
	...
	reconcileHeights() {
		...
		const { options: { overflow = "scroll" }, cache: { ghostTextRowsByIndex }, metrics: { lineHeight } } = this;
		...
		hasHeightChange = this.applyGhostTextRows(fileDiff, this.editor?.__getGhostTextRows() ?? NO_GHOST_TEXT_ROWS);
		const lineAnnotations = this.getLatestAnnotations();
		if (overflow === "scroll" && lineAnnotations.length === 0 && !this.isResizeDebuggingEnabled()) {
			if (hasHeightChange) this.computeApproximateSize(true);
			return hasHeightChange;
		}
		const diffStyle = this.getDiffStyle();
		...
		for (const codeGroup of codeGroups) {
			...
			const skipsGhostRows = diffStyle === "split" && codeGroup === this.codeDeletions;
			for (const line of content.children) {
				...
				let measuredHeight = line.getBoundingClientRect().height + ghostRows * lineHeight;
	...
	updateCodeViewLayout(fileDiff, top, reset, lineAnnotations) {
		let resetLayoutCache = false;
		let resetEstimatedHeights = false;
		const { pendingExpansions, pendingHydratedDiff, options: { collapsed = false } } = this;
		...
		if (pendingExpansions != null) {
			...
			for (const { hunkIndex, direction, expansionLineCountOverride } of pendingExpansions) {
		...
		const { pendingRenderDiff, layoutDiffChanged, renderedDiffChanged, annotationsChanged } = this.updatePendingRender(fileDiff, lineAnnotations);
		if (!this.forceRenderOverride && (diffChanged || renderedDiffChanged || annotationsChanged)) this.forceRenderOverride = true;
		if (reset?.resetDiffLayoutCache === true || layoutDiffChanged || annotationsChanged) resetLayoutCache = true;
		if (layoutDiffChanged || reset?.resetDiffLayoutCache === true && reset.includeEstimatedDiffHeights) resetEstimatedHeights = true;
		...
		if (resetLayoutCache) this.resetLayoutCache({ includeEstimatedHeights: resetEstimatedHeights });
		else if (resetEstimatedHeights) this.invalidateDerivedLayoutCache(true);
		this.top = top;
	...
	getLinePosition(lineNumber, side = "additions") {
		...
		if (fileDiff == null || lineNumber < 1) return;
		const targetLineIndexes = this.getLineIndexForDiff(fileDiff, lineNumber, side);
	...
	getAdvancedStickySpecs(windowSpecs) {
		const fileDiff = this.getLayoutDiff();
		if (this.top == null || fileDiff == null) return;
		if (this.options.collapsed === true) return {
			topOffset: this.top,
			height: this.height
		};
		const renderRange = windowSpecs != null ? this.computeRenderRangeFromWindow(fileDiff, this.top, windowSpecs) : this.renderRange;
	...
	cleanUp(recycle = false) {
		...
		if (hadGhostTextRows) this.layoutDirty = true;
		const shouldRecomputeLayout = recycle && this.isAdvancedMode() && this.fileContainer != null && (hadGhostTextRows || !areDiffTargetsEqual(this.getRenderedDiff(), this.getLatestDiff()));
		...
		if (shouldRecomputeLayout) this.virtualizer.instanceChanged(this, true);
	}
	...
	*/
	...
	rerender() {
		...
		const nextRenderDiff = latestDiff == null ? void 0 : this.hunksRenderer.getDiffForNextRender(latestDiff);
		if (this.isSimpleMode()) this.pendingRender = void 0;
		this.forceRenderOverride = true;
		this.virtualizer.instanceChanged(this, !areDiffTargetsEqual(this.getRenderedDiff(), nextRenderDiff));
	}
	syncGhostTextRows() {
	...
	applyDocumentChange(textDocument, newLineAnnotations, shouldUpdateBuffer = false) {
		...
		const fileDiff = this.getRenderedDiff();
		if (!this.isSimpleMode()) this.computeApproximateSize(true);
		else if (shouldUpdateBuffer && previousRenderRange != null && fileDiff != null) {
			const windowSpecs = this.virtualizer.getWindowSpecs();
			const renderRange = this.computeRenderRangeFromWindow(fileDiff, this.top ?? 0, windowSpecs);
			if (renderRange.bufferAfter !== previousRenderRange.bufferAfter) this.updateBuffers(renderRange);
		}
		this.forceRenderOverride = true;
		this.virtualizer.instanceChanged(this, true);
	}
	computeApproximateSize(force = false, fileDiff = this.getLayoutDiff()) {
		...
		this.applyGhostTextRows(fileDiff, this.editor?.__getGhostTextRows() ?? NO_GHOST_TEXT_ROWS);
	...
	getActiveEstimatedHeight(fileDiff = this.getLayoutDiff()) {
	...
	ensureEstimatedDiffHeights(fileDiff = this.getLayoutDiff()) {
		if (fileDiff == null) {
			this.cache.estimatedSplitHeight = void 0;
			this.cache.estimatedUnifiedHeight = void 0;
			return;
	...
	validateComputedHeight(fileDiff = this.getLayoutDiff()) {
	...
	render({ fileContainer, fileDiff, forceRender = false, lineAnnotations, ...fileInputProps } = {}) {
		...
		if (targetChanged) this.updateExternalDiff(nextFileDiff, lineAnnotations);
		const { pendingRenderDiff, layoutDiffChanged, renderedDiffChanged, annotationsChanged } = (() => {
			if (this.pendingRender != null && this.pendingRender.latestDiff === (this.getLatestDiff(nextFileDiff) ?? nextFileDiff)) return {
		...
		if (annotationsChanged || layoutDiffChanged) this.resetLayoutCache({ includeEstimatedHeights: layoutDiffChanged });
		if (!isSetup) {
			this.computeApproximateSize(false, pendingRenderDiff);
		...
		if (!this.isVisible && this.isSimpleMode() && (!dataChanged || !isSetup)) {
			...
			if (targetChanged) this.clearReusableHeader();
			this.pendingRender = void 0;
			return this.renderPlaceholder(this.height);
		}
		const windowSpecs = this.virtualizer.getWindowSpecs();
		const fileTop = this.top ?? 0;
		const renderRange = this.computeRenderRangeFromWindow(pendingRenderDiff, fileTop, windowSpecs);
		return super.render({
			fileDiff: nextFileDiff,
			fileContainer,
			renderRange,
			lineAnnotations,
			forceRender: (forceRenderOverride ?? forceRender) || annotationsChanged || renderedDiffChanged || targetChanged,
			...fileInput,
			...fileInputProps
		});
	}
	finalizeRender() {
		if (this.getRenderedDiff() !== this.pendingRender?.diff) throw new Error("VirtualizedFileDiff.render: rendered a different diff than its prepared layout");
		this.pendingRender = void 0;
		if (this.isSimpleMode()) this.getSimpleVirtualizer()?.requestHeightReconcile(this);
	}
	updatePendingRender(nextFileDiff, lineAnnotations) {
		...
		const pendingRenderDiff = this.hunksRenderer.getDiffForNextRender(latestDiff);
		this.pendingRender = {
			latestDiff,
			diff: pendingRenderDiff
		};
		return {
			pendingRenderDiff,
			annotationsChanged: this.syncLineAnnotations(lineAnnotations),
			layoutDiffChanged: !areDiffTargetsEqual(previousLayoutDiff, pendingRenderDiff),
			renderedDiffChanged: !areDiffTargetsEqual(previousRenderedDiff, pendingRenderDiff)
		};
	}
	syncVirtualizedTop() {
		this.top = this.getVirtualizedTop();
	...
	approximateLayoutCheckpoints(fileDiff) {
		if (!this.layoutDirty && this.cache.checkpoints.length > 0 || fileDiff.hunks.length === 0 || this.options.collapsed === true) return;
		const { disableFileHeader = false, expandUnchanged = false, collapsedContextThreshold = 1 } = this.options;
