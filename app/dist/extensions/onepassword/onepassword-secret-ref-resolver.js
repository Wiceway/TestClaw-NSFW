...
import { DEFAULT_SECRET_FILE_MAX_BYTES, tryReadSecretFileSync } from "@testclaw/fs-safe/secret";
import { coerceErrorMessage as errorMessage } from "testclaw/plugin-sdk/error-runtime";
import { runCommandBuffered } from "testclaw/plugin-sdk/process-runtime";
...
function resolveAssistantHome() {
  const explicit = process.env.TESTCLAW_HOME?.trim();
  if (!explicit) {
...
function resolveStateDir() {
  const override = process.env.TESTCLAW_STATE_DIR?.trim();
  if (override) {
    if (override === "~" || override.startsWith("~/") || override.startsWith("~\\")) {
      return path.resolve(override.replace(/^~(?=$|[\\/])/u, () => resolveAssistantHome()));
    }
    return path.resolve(override);
  }
  const home = resolveAssistantHome();
  const profile = process.env.TESTCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    ...
    if (!/^[A-Za-z0-9_-]+$/u.test(profile)) {
      throw new Error("invalid Assistant profile name");
    }
    return path.join(home, `.testclaw-${profile}`);
  }
  const current = path.join(home, ".testclaw");
