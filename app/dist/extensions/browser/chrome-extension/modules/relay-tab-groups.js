import { TESTCLAW_TAB_GROUP_TITLE } from "./relay-core.js";

async function isAssistantGroupId(groupId) {
  if (!Number.isInteger(groupId) || groupId < 0) {
    return false;
  }
  try {
    const group = await chrome.tabGroups.get(groupId);
    return group.title === TESTCLAW_TAB_GROUP_TITLE;
  } catch {
    return false;
  }
}
...
export async function isTabSelected(tab) {
  return await isAssistantGroupId(tab?.groupId);
}

export async function addTabToAssistantGroup(tabId, { chromeApi, getGroupColor, created }) {
  ...
  const groups = await chromeApi.tabGroups
    .query({ title: TESTCLAW_TAB_GROUP_TITLE })
    .catch(() => []);
  assertCurrent();
  ...
  if (!group) {
    ...
    await chromeApi.tabGroups.update(groupId, { title: TESTCLAW_TAB_GROUP_TITLE, color });
    assertCurrent();
  }
}
