...
// Assistant extension service worker.
//
// Thin transport between the Assistant extension relay (loopback WebSocket) and
...
// The Assistant tab group is the ACL in selected mode and an ownership marker
// in all-tabs mode.
import {
...
const RELAY_WATCHDOG_ALARM = "testclaw-relay-watchdog";
const RELAY_OPENING_DEADLINE_ALARM = "testclaw-relay-opening-deadline";
...
async function removeTabFromAssistantGroup(tabId) {
  try {
...
function failRelayAuthentication(ws, error) {
  ...
  relayStatusHint =
    "Relay authentication v2 failed. Update Assistant, or re-pair after a relay key rotation.";
  try {
    closeRelaySocket(
      4001,
...
function handleRelayOpeningDeadline() {
  ...
  setBadge("error");
  relayStatusHint = "Relay authentication v2 timed out. Make sure Assistant is up to date.";
  scheduleReconnect();
}
...
const handlePopupMessage = createPopupMessageHandler({
  ...
  setBadge,
  detachDebugger,
  removeTabFromAssistantGroup,
  addTabToAssistantGroup: (tabId) => tabAccessPolicy.addTabToGroup(tabId),
  scheduleTabsSync,
  pauseTab,
});
