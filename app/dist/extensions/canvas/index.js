...
import { a as selectDefaultNodeFromList } from "../../nodes-utils-D51B-c_T.mjs";
import { t as definePluginEntry } from "../../plugin-entry-JBPHePKD.mjs";
import "../../agent-harness-runtime-BiNT4re-.mjs";
import { n as isCanvasHostEnabled, t as canvasConfigSchema } from "../../config-DpwtO6c6.mjs";
import { t as A2UI_PATH } from "../../a2ui-shared-B_B0azg7.mjs";
import { i as CanvasToolSchema, n as isEligibleCanvasNode, t as CANVAS_PRESENT_COMMAND } from "../../node-eligibility-B4WWUSAG.mjs";
...
ZodPipeline.create;
//#endregion
//#region node_modules/.pnpm/@a2ui+web_core@0.10.7/node_modules/@a2ui/web_core/src/v0_9/schema/common-types.js
...
markChildRef(unionType([arrayType(ComponentIdSchema).describe("A static list of child component IDs."), objectType({
...
const A2UI_V08_BUNDLE_PATH = "/__testclaw__/a2ui/a2ui.bundle.js";
const A2UI_V09_BUNDLE_PATH = "/__testclaw__/a2ui/a2ui-v0.9.bundle.js";
function escapeHtmlAttribute(value) {
	return value.replaceAll("&", "&amp;").replaceAll("\"", "&quot;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
}
function bootScript(messages, promptGranted) {
	return `<script>globalThis.testclawA2UIBoot=${JSON.stringify({
		messages,
...
function resourceScript(path, url) {
	...
	return `<script>(()=>{const match=location.pathname.match(/^\\/__testclaw__\\/cap\\/[^/]+/u);const script=document.createElement("script");script.src=(match?.[0]??"")+${JSON.stringify(path).replaceAll("<", "\\u003c")};document.head.appendChild(script);})();<\/script>`;
}
...
const canvasA2UIBoardWidgetKind = {
	...
	resources: {
		...
		readPublicResource: async (resourcePath) => {
			if (resourcePath !== A2UI_V08_BUNDLE_PATH && resourcePath !== A2UI_V09_BUNDLE_PATH) return;
			const { readPublicA2uiResource } = await import("../../a2ui-VrATzxSZ.mjs");
	...
	composeDocument({ source, resourceUrls, promptGranted }) {
		...
		return `${bootScript(parsed.messages, promptGranted)}<style>html,body{height:100%;overflow:hidden;background:transparent}testclaw-a2ui-host{display:block;height:100%}</style><testclaw-a2ui-host></testclaw-a2ui-host>${resourceScript(bundlePath, bundleUrl)}`;
	}
};
//#endregion
...
function createLazyCanvasTool(agentSessionKey) {
	const loadTool = createLazyRuntimeModule(() => import("../../tool-CffRTVxR.mjs").then(({ createCanvasTool }) => createCanvasTool({ agentSessionKey })));
	return {
		label: "Canvas",
		name: "canvas",
...
var canvas_default = definePluginEntry({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			...
			const loadRenderer = createLazyRuntimeModule(() => import("../../a2ui-VrATzxSZ.mjs"));
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-B7lpfPMI.mjs");
