...
import { g as onInternalDiagnosticEvent } from "./diagnostic-events-C4sEV9aC.mjs";
...
function getDiagnosticStabilityState() {
	...
	globalStore["__testclawDiagnosticStabilityState"] ??= createState();
	return globalStore["__testclawDiagnosticStabilityState"];
}
...
function sanitizeDiagnosticEvent(event) {
	...
	switch (event.type) {
		...
		case "diagnostic.gc":
		case "diagnostic.child_process.spawn": break;
		case "model.usage":
		...
		case "model.call.error":
			...
			record.timeToFirstByteMs = event.timeToFirstByteMs;
			record.failureKind = event.failureKind;
			record.memory = event.memory ? { ...event.memory } : void 0;
			assignReasonCode(record, event.errorCategory);
			break;
		case "log.record":
			record.level = event.level;
...
function resetDiagnosticStabilityRecorderForTest() {
	...
	const globalStore = globalThis;
	globalStore["__testclawDiagnosticStabilityState"] = next;
}
//#endregion
