...
import { c as isPathInside, d as pathExists } from "./fs-safe-B1VkXzpu.mjs";
import { l as pathExists$1 } from "./utils-Dy46mFy2.mjs";
...
import { i as runCommandWithTimeout, t as runCommandBuffered } from "./exec-BddaUUYf.mjs";
...
import { n as readPackageName, r as readPackageVersion, t as readPackageManagerSpec } from "./package-json-CKAceI3K.mjs";
...
import { n as applyPosixNpmScriptShellEnv, r as createNpmFreshnessBypassArgs, t as applyNpmFreshnessBypassEnv } from "./npm-install-env-B-e0Nn5f.mjs";
import { t as applyPathPrepend } from "./path-prepend-CRp372y2.mjs";
import { a as readPackageDistInventoryIfPresent, r as collectPackageDistInventory, t as PACKAGE_DIST_INVENTORY_RELATIVE_PATH } from "./package-dist-inventory-DnyC80Ez.mjs";
import { t as collectGitRuntimeErrors } from "./update-git-runtime-B4Gs4MzG.mjs";
...
async function collectInstalledGlobalPackageErrors(params) {
	...
	if (params.expectedGitCheckout) {
		...
		if (installedRoot !== params.expectedGitCheckout.root) errors.push(`expected checkout ${params.expectedGitCheckout.root}, found ${installedRoot ?? "<missing>"}`);
			...
			if (!await pathExists$1(path.join(installedRoot, "testclaw.mjs"))) errors.push(`missing ${path.join(installedRoot, "testclaw.mjs")}`);
		}
...
async function collectCriticalInstalledPackageDistPaths(packageRoot) {
	...
	await Promise.all(BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
		...
		if (await pathExists$1(path.join(packageRoot, pluginRoot, "package.json")) || await pathExists$1(path.join(packageRoot, pluginRoot, "testclaw.plugin.json"))) expectedFiles.add(relativePath);
	}));
