...
import { E as readTaskBackingInstance, q as sameTaskRunScope } from "./task-registry.store.kernel-CTpG8C0S.mjs";
import { C as taskRegistryLog, S as taskProgressBatches } from "./task-registry-state-C_3mxHYW.mjs";
...
import { a as prepareTaskBackingRead } from "./task-backing-authority-3puosIoc.mjs";
import { s as resolveTaskDeliveryOwner } from "./task-notification-routing-QKMzdln2.mjs";
import { a as recordRequesterTaskProgress, n as flushTaskProgressBatch, r as getTaskProgressBatchesForRuns, s as scheduleYieldedSubagentRunProgress, t as completeTaskProgressBatch } from "./task-registry-progress-BvGy2sM8.mjs";
...
async function withTaskProgressRequesterContinuation(params, run) {
	...
	let unsubscribe;
	try {
		const [{ normalizeAgentPlanSteps }, { readPreparedTaskActivityItem }] = await Promise.all([import("./streaming-DK-5jeTk.mjs"), import("./task-registry-activity-7FV3ahwr.mjs")]);
...
function createPreparedTaskProgressContinuation(params, read) {
	...
	return {
		adopt: async (receipt) => {
			...
			used = true;
			try {
				const { adoptTaskProgressMessage } = await import("./task-registry-progress-runtime-CLlIVZlt.mjs");
				assertCurrent();
				...
				if (params.onAdopted) params.onAdopted({ operationId });
					...
					const { attachRequesterProgressPresentation } = await import("./subagent-registry-BuC6YZFG.mjs");
					assertCurrent();
