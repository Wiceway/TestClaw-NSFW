...
import { l as resolveAgentIdFromSessionKey, m as scopeLegacySessionKeyToAgent, o as classifySessionKeyShape, s as isUnscopedSessionKeySentinel } from "./session-key-C_bfgyCp.mjs";
...
import { a as READ_SCOPE, t as ADMIN_SCOPE } from "./operator-scopes-D-CL26h0.mjs";
...
import { _ as assertGatewayCliMessageContext, g as randomIdempotencyKey, m as isGatewayExplicitAuthRequiredError, o as callGateway, p as isGatewayCredentialsRequiredError } from "./call-Cm2P5Cd6.mjs";
...
import { t as GatewayProtocolRequestError } from "./protocol-request-BMUN1re6.mjs";
import { a as isGatewayTransportError } from "./transport-error-BJi8D8Qw.mjs";
import { c as recordCliGatewayRunFailure, s as readCliGatewayRunFailure } from "./failure-output-BDwPHdmu.mjs";
...
async function startOneShotDiagnosticsExporters(params) {
	...
	const [{ acquirePluginRegistryForInspection }, { startPluginServices }] = await Promise.all([import("./plugins/loader.js"), import("./services-B8rkIUs1.mjs")]);
...
const embeddedAgentCommandLoader = createLazyPromiseLoader(() => import("./agent-zmLuKkLm.mjs").then((module) => module.agentCommand), { cacheRejections: true });
const agentSessionModuleCache = createLazyPromiseLoader(() => import("./session.runtime.js"), { cacheRejections: true });
const runtimeConfigModuleLoader = createLazyPromiseLoader(() => import("./io-CExODfn_.mjs"), { cacheRejections: true });
const embeddedStateLockModuleLoader = createLazyPromiseLoader(() => import("./embedded-state-lock-B2m8HT_I.mjs"), { cacheRejections: true });
...
async function runEmbeddedAgentCommand(opts, runtime, deps, diagnosticsOptions) {
	...
	if (isExecutionIdentityCollectionEnabled(config)) try {
		const { startAgentLocalAuditWriter } = await import("./agent-local-audit-CbN7Ox2r.mjs");
