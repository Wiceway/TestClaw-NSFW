...
import { i as resolveCanvasHttpPathToLocalPath, r as resolveCanvasDocumentsDir } from "./documents-CW9SXtfW.mjs";
import { t as buildBoardWidgetContentSecurityPolicy } from "./board-sandbox-BeHh5xeC.mjs";
...
async function handleCanvasDocumentHttpRequest(req, res) {
	...
	try {
		...
		if (mime === "text/html") {
			...
			if (await resolveDocumentSandbox(root$1, relativePath) === "scripts") {
				res.setHeader("Content-Security-Policy", buildBoardWidgetContentSecurityPolicy({
					grantState: "none",
