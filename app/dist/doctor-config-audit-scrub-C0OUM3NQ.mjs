import { h as scrubConfigAuditLog, p as resolveLegacyConfigAuditLogPath } from "./io.audit-C8k3Ku7p.mjs";
...
function configAuditScrubToHealthFinding(result) {
	return {
		...
		fixHint: "Run `testclaw doctor --fix` to rewrite argv/execArgv fields through the current redactor."
	};
}
...
async function maybeScrubConfigAuditLog(params) {
	...
	try {
		if (params.shouldRepair) {
			...
			if (result.aborted) {
				note("Config audit scrub was aborted because new entries were appended to config-audit.jsonl during the rewrite. No records were modified. Stop the gateway (or wait until it is idle) and rerun `testclaw doctor --fix`.", NOTE_TITLE);
				return;
			}
		...
		if (preview.rewritten > 0) {
			const fixCommand = params.doctorFixCommand ?? "testclaw doctor --fix";
