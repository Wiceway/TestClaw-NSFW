...
import { a as getPreparedModelRuntimeAuthMaterializations, c as loadPreparedModelRuntimeAuth } from "./prepared-model-runtime-auth-Cnc45sWs.mjs";
...
import { t as createPreparedGatewayModelCatalog } from "./server-model-catalog-view-muuNaRwE.mjs";
...
async function resolveLoader(params) {
	...
	const { loadPublishedPreparedModelCatalogOwnerSnapshot } = await import("./prepared-model-catalog-C1J9Stvc.mjs");
...
async function readPreparedGatewayModelCatalogBatch(agentIds, params) {
	...
	const { getPreparedModelCatalogOwnerSnapshot, withPreparedModelRuntimeReadBatch } = await import("./prepared-model-catalog-C1J9Stvc.mjs");
...
async function readPreparedGatewayModelCatalogOwnerSnapshot(params) {
	const { getPublishedPreparedModelCatalogOwnerSnapshot, materializePreparedModelCatalogOwner } = await import("./prepared-model-catalog-C1J9Stvc.mjs");
	const config = (params?.getConfig ?? getRuntimeConfig)();
