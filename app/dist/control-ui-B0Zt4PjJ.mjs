...
import { E as getSubagentSessionListReadSnapshotIdentity, P as prepareSubagentSessionListReadCache } from "./subagent-registry-read-PBgGP-fW.mjs";
...
import { a as loadSessionEntriesForTarget } from "./sessions-shared-BEFWecy7.mjs";
import { t as resolveAgentIdOrRespondError } from "./agent-id-shared-DePOhdT-.mjs";
import { a as resolveControlUiSessionPrTarget, i as prepareControlUiSessionPrRead, n as parseControlUiSessionPullRequestsSubscribeParams, r as withControlUiSessionPrSource } from "./control-ui-session-pr-subscriptions-tKRw7Hpe.mjs";
...
const loadSessionCheckDetails = async (params, deps) => {
	deps.assertCurrent();
	const { loadControlUiSessionPullRequestChecks } = await import("./control-ui-session-pr-check-details-DsaVopBg.mjs");
	const { loadControlUiSessionPullRequests } = await import("./control-ui-session-prs-y6YoblcY.mjs");
...
function createControlUiHandlers(loadGitHubPreview = (...args) => gitHubPublicApi.loadControlUiGitHubPreview(...args), loadSessionPreview = loadControlUiSessionPreview, loadChecks = loadSessionCheckDetails) {
	return {
		"controlUi.linkPreview": async ({ params, context, respond, signal }) => {
			...
			const { parseControlUiLinkPreviewUrl, loadControlUiLinkPreview } = await import("./control-ui-link-preview-CVUi6ag7.mjs");
