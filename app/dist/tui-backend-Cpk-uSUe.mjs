...
import { r as agentSessionKeysMatchByRequestKey } from "./session-key-C_bfgyCp.mjs";
...
import { i as loadSystemAgentOverview, r as formatSystemAgentStartupMessage } from "./overview-DU-Fj9NZ.mjs";
import { o as resolveSystemAgentVerifiedInferenceState } from "./verified-inference-CP3RX57B.mjs";
import { n as SystemAgentChatEngine, t as buildOnboardingWelcome } from "./onboarding-welcome-oHNjPHQf.mjs";
import { randomUUID } from "node:crypto";
//#region src/system-agent/tui-backend.ts
async function loadHostedSetupForTui() {
	const [{ createClackPrompter }, hostedSetup] = await Promise.all([import("./clack-prompter-D4W0DvqS.mjs"), import("./hosted-setup.runtime.js")]);
	return {
		createClackPrompter,
		hostedSetup
	};
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	await (opts.runChannelsAdd ?? (await import("./add-7MA2XwgH.mjs")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
...
async function runSystemAgentTui(opts, runtime) {
	...
	for (;;) {
		...
		const runTui = boundOpts.runTui ?? (await import("./tui-wi2LuHey.mjs")).runTui;
		try {
			await runTui({
				local: true,
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (verified) {
			...
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-C1J9Stvc.mjs"), import("./model-thinking-default-C0I2QO4I.mjs")]);
