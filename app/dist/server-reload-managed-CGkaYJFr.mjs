...
import { O as setRuntimeConfigAppliedHash, c as getRuntimeConfigSnapshotMetadata, f as hashRuntimeConfigValue, u as getRuntimeConfigSourceSnapshot, w as resolveConfigWriteFollowUp } from "./runtime-snapshot-Dti8jFIP.mjs";
...
import { r as getRuntimeConfig, w as getRuntimeConfigWriteApplication } from "./io.runtime-DIHH_X2V.mjs";
...
import { S as runWithGatewayIndependentRootWorkAdmission, a as getActiveGatewayRootWorkCount } from "./gateway-work-admission-DeFm4gyw.mjs";
import { l as reloadSessionMcpRuntimes } from "./agent-bundle-mcp-manager-api-BNGMxdv8.mjs";
...
import { g as hasSameSecretReloadContract, i as clearSecretsRuntimeSnapshotState, l as getActiveSecretsRuntimeSnapshotRevisionState, p as hasActiveSecretsRuntimeSnapshotLineage, u as getActiveSecretsRuntimeSnapshotState, x as setSecretsRuntimeSourceSnapshotIfCurrent, y as restoreSecretsRuntimeSourceSnapshotIfLineageCurrent } from "./runtime-state-p_Glf0Cm.mjs";
import { s as publishOperatorRoleConfigChange } from "./operator-role-policy-BqKjnbl9.mjs";
import { u as runOutsideSetupCredentialAccess } from "./order-BnTFWeei.mjs";
import { _ as rejectPendingPreparedModelRuntimeReplacement, d as markPreparedModelRuntimeSnapshotsStale, g as refreshPreparedModelRuntimeSnapshots, s as advancePreparedModelRuntimeConfig } from "./prepared-model-runtime-DkA7Mb_L.mjs";
import { n as runOutsidePluginLifecycleLease, r as withPluginLifecycleLease } from "./plugin-lifecycle-lease-CkmD-Hkp.mjs";
...
import { _ as isCurrentGatewayReloadGeneration, d as setGatewayRestartPolicy, g as abortPendingChannelReloads, r as deferGatewayRestartUntilIdle, v as isGatewayReloadGenerationAborted, y as nextGatewayReloadGeneration } from "./restart-BLOXglMD.mjs";
...
import { w as requestActiveCronJobCancellationByDeclarationKeyPrefix } from "./active-jobs-CdxG5_Ri.mjs";
...
import { i as refreshContextWindowCache } from "./context-CkigEvA0.mjs";
import { l as resetSkillSnapshotConfigFingerprintCache } from "./workspace-skill-loader-DFNdZEeN.mjs";
import { t as bumpSkillsSnapshotVersion } from "./refresh-state-CK1eR8r9.mjs";
import { t as commitHooksConfigReload, y as resolveHooksConfig } from "./hooks-De9-SwUZ.mjs";
import { i as getPluginRuntimeGeneration, n as PluginRuntimeApplicationError } from "./lifecycle-kyMYqai_.mjs";
import { a as resolvePluginInstallReloadMetadata, n as isNoopGatewayReloadPlan, o as diffConfigPaths, r as listConfigReloadRefinementPrefixes, s as diffGatewayReloadPaths, t as buildGatewayReloadPlan } from "./config-reload-plan-WY_GBKpc.mjs";
import { n as resolveReloadAgentIds, t as refreshModelRuntimeAfterHotReload } from "./server-reload-model-runtime-scope-B4lkpCSw.mjs";
import { t as invalidateConfigGetResponseCache } from "./config-get-response-Qw9gFMd0.mjs";
...
import { r as startGatewayCronWithLogging } from "./server-runtime-services-BQ-gpFSI.mjs";
	...
	const applySnapshot = async (sourceSnapshot, candidate, initialEpoch = sourceObservation.epoch, { pluginLifecycle, onRuntimeCommitted, assertInvokerOwned: pluginInvokerGuard } = {}) => {
		...
		if (pluginLifecycle?.expectedInstallHashes && Object.keys(pluginLifecycle.expectedInstallHashes).length > 0 && completed && completed.runtime.generation === getPluginRuntimeGeneration() && !opts.hasOutstandingGatewayRestart?.() && sourceSnapshot.hash === completed.snapshot.hash && diffConfigPaths(nextSourceConfig, completed.snapshot.sourceConfig).length === 0 && isDeepStrictEqual(sourceSnapshot.includedPaths, completed.snapshot.includedPaths) && isDeepStrictEqual(sourceSnapshot.includeProvenance, completed.snapshot.includeProvenance) && isDeepStrictEqual(serializeConfigResolutionFacts(nextSourceConfig), serializeConfigResolutionFacts(completed.snapshot.sourceConfig)) && isDeepStrictEqual(nextPluginInstallRecords, completed.installRecords) && Object.entries(pluginLifecycle.expectedInstallHashes).every(([id, hash]) => nextPluginInstallRecords[id] && hashStableJson(nextPluginInstallRecords[id]) === hash)) {
			...
			if (entries.length === Object.keys(expected).length) {
				const { inspectPluginGenerationSources } = await import("./plugin-generation-source-inspection-28krS13f.mjs");
				await checkpoint();
				assertCurrent();
...
function createGatewayReloadHandlers(params) {
	...
	const applyHotReload = async (plan, nextConfig, publication) => {
		...
		const internalHooks = plan.reloadInternalHooks || plan.reloadPlugins ? await (await import("./loader-CvUs2Aex.mjs")).prepareInternalHooks(nextConfig, tryResolveConfiguredAgentWorkspaceDir(nextConfig, candidateEnv) ?? resolveDefaultAgentWorkspaceDir(candidateEnv)) : void 0;
		...
		if (plan.restartCron) {
			const { buildGatewayCronService } = await import("./server-cron-sf6qzHhX.mjs");
		...
		if (plan.restartGmailWatcher) {
			...
			try {
				...
				if (!restartAbortController.signal.aborted) {
					const [{ stopGmailWatcher }, { startGmailWatcherWithLogs }] = await Promise.all([import("./gmail-watcher-4hFSgMzD.mjs"), import("./gmail-watcher-lifecycle-D0GTRXy8.mjs")]);
...
async function restoreSecretsRuntimeSnapshotIfCurrent(snapshot, expectedRevision, ownedSnapshot, options) {
	if (!(await import("./runtime-xnvnIymm.mjs")).restoreSecretsRuntimeSnapshotIfCurrent(snapshot, expectedRevision, ownedSnapshot, { runtimeSourceConfig: options?.runtimeSourceConfig })) return false;
	options?.onActivated?.();
	return true;
}
...
function startManagedGatewayConfigReloader(params) {
	...
	const { applyHotReload, getDeferredChannelReloads, acceptRestartConfig, beginGatewayRestartLifecycle, hasOutstandingGatewayRestart, hasConfigCandidatePending, pauseGatewayRestartForConfigCandidate, publishAppliedConfigHash, publishAcceptedRestartTarget, publishDeferredAppliedConfigHash, recordAcceptedRestartTarget, requestGatewayRestart, restoreConservativeRestartDebt, stopRestartRetries } = createGatewayReloadHandlers({
		...
		assertRestartReady: (config) => import("./testclaw-database-preflight-CXmB84Lv.mjs").then(({ assertAssistantDatabasesReady }) => assertAssistantDatabasesReady({
			env: process.env,
