...
import { T as StateDatabaseCoordinatorContentionError, o as acquireGatewayMaintenanceCoordinator, s as acquireStateDatabaseCoordinator } from "./sqlite-source-handle-CDYF24uv.mjs";
import { n as createAssistantDatabaseMaintenanceScope } from "./testclaw-state-db-async-lifecycle-Bn4ZDDk6.mjs";
import { s as resolveAssistantStateSqlitePath } from "./testclaw-state-db.paths-DStyAtQk.mjs";
import { n as openDoctorStateSchemaReadAdmission } from "./testclaw-state-db-doctor-schema-C3u4Dvax.mjs";
import { c as readStateLeaseProcessOwnerStatus } from "./testclaw-state-lease-store-D0QbC1Ab.mjs";
...
import { a as findServiceOwnershipRefusal, s as hasGatewayServiceStopUnsafeError } from "./service-inspection-error-DLyDrlb3.mjs";
import { i as UpdateDoctorError, t as DoctorMaintenanceRefusalError } from "./update-doctor-result-Dn-wRvxN.mjs";
import { o as readGatewayOwnerLease } from "./windows-port-pids-PRvzp9PM.mjs";
import { c as readLockPayload, d as resolveGatewayOwnerStatus, n as GatewayLockError, o as readActiveGatewayLockIdentity, u as resolveGatewayLockPaths } from "./gateway-lock-CMKMxZa9.mjs";
...
import { l as withCommandProcessScope, n as resolveCommandProcessSignal } from "./exec-spawn-D7QjtIL9.mjs";
import { n as GatewayServiceAuthorityError } from "./service-update-authority-p1W3yDaI.mjs";
...
import { m as recordUpdateRunRepairContinuation } from "./update-run-ledger-CWjgjkCi.mjs";
import { s as listUpdateRuns } from "./update-run-reader-DcJAE2Qr.mjs";
...
import { s as inspectGatewayRestart } from "./restart-health-DODQwMtT.mjs";
...
import { c as resolveUpdateParentGatewayActivation, l as shouldManageGatewayService, n as assertDoctorServiceSelection, o as isServiceRepairExternallyManaged } from "./doctor-service-repair-policy-BIWEOO8f.mjs";
import { n as classifyDoctorMaintenanceRefusal, t as assertDoctorMaintenanceInspection } from "./doctor-maintenance-inspection-CAmoifeK.mjs";
import { n as resolveDoctorRepairMode, t as isDoctorUpdateRepairMode } from "./doctor-repair-mode-CSVsSPsZ.mjs";
import { n as recordUpdateDoctorRefusal, r as resolveUpdateDoctorGitRecovery, t as formatUpdateDoctorServiceStopRefusal } from "./doctor-update-refusal-CibZ7MUg.mjs";
...
async function preflightExternalDoctorAgentLease(env) {
	const { readActiveAssistantAgentDatabaseLeasesReadOnly } = await import("./testclaw-agent-db-lease-B0jD95B7.mjs");
...
async function assertDoctorAgentLeaseAdmission(env) {
	const { assertNoAssistantAgentDatabaseLeasesReadOnly, AssistantAgentDatabaseLeaseActiveError } = await import("./testclaw-agent-db-lease-B0jD95B7.mjs");
	try {
	...
	const finish = async (initialConfig, assertCustody, writeConfig, assertRestoreAdmission = assertUpdateAdmissionCurrent) => {
		...
		if (!before.stopped) {
			...
			if (verdict?.kind === "owned" && verdict.requiresInstallRootRefresh) {
				const { inspectGatewayServiceInstallationDrift } = await import("./service-layout-CJ3iNnXQ.mjs");
				const drift = await inspectGatewayServiceInstallationDrift({ packageRootReal: verdict.root }, root);
				if (drift) {
					const { formatGatewayServiceInstallationDrift } = await import("./shared-rHhCNyaN.mjs");
		...
		try {
			...
			const [{ restoreDoctorGatewayService }, { resolveUpdatedGatewayRestartPort }, { renderRestartDiagnostics, waitForGatewayHealthyRestart }] = await Promise.all([
				import("./doctor-maintenance-restoration-Dxb-Z-0e.mjs"),
				import("./update-command-service-plan-DtIHd7Kk.mjs"),
				import("./restart-health-CRBateWR.mjs")
			]);
			...
			if (error instanceof GatewayServiceAuthorityError) {
				const { formatDaemonServiceInstallCommand } = await import("./shared-rHhCNyaN.mjs");
				const serviceEnv = before.serviceEnv;
	...
	try {
		await settle(async () => {
			...
			if (params.root && isDefaultInstallIdentity(env) && !externallyManaged && await shouldManageGatewayService(env)) {
				serviceMaintenance = await import("./update-command-service-maintenance-B0P3o2-a.mjs");
				...
				if (inspection.serviceUpdateVerdict?.kind === "owned") {
					...
					if (parentActivation === void 0 || staleReplacement) {
						...
						const root = params.root;
						const { withGatewayServiceOperationLock } = await import("./service-operation-lock-BZb17rs_.mjs");
