import { F as resolveTimerTimeoutMs, g as isFutureDateTimestampMs } from "./number-coercion-CLj0HTDM.mjs";
import { n as isTruthyEnvValue } from "./env-DiCPcdkM.mjs";
import { t as createSubsystemLogger } from "./subsystem-Bmu9GF-b.mjs";
import { a as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-DnqLKLPL.mjs";
//#region src/gateway/channel-health-monitor.ts
const log = createSubsystemLogger("gateway/health-monitor");
const DEFAULT_CHECK_INTERVAL_MS = 3e5;
	...
	async function runCheckWork() {
		try {
			...
			for (const [channelId, accounts] of Object.entries(snapshot.channelAccounts)) {
				if (!accounts || snapshot.reloadingChannels?.has(channelId)) continue;
				...
				for (const [accountId, status] of Object.entries(accounts)) {
					if (stopped) return;
					if (!status) continue;
					if (!channelManager.isAccountListed(channelId, accountId)) continue;
...
function startGatewayChannelHealthMonitor(params) {
	...
	if (isTruthyEnvValue(env.TESTCLAW_SKIP_CHANNELS) || isTruthyEnvValue(env.TESTCLAW_SKIP_PROVIDERS)) return null;
