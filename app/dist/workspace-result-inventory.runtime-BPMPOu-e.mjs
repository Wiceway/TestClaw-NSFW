import { n as MAX_RECONCILIATION_FILE_BYTES, o as parseWorkerWorkspaceManifest } from "./workspace-manifest-Ify_1Ssa.mjs";
...
import { c as stagedWorkspaceEntryBytes, o as requireWorkerResultStorageRef, s as resolveStagedWorkspaceReadEntry } from "./workspace-result-inventory-Yxwk0T_A.mjs";
import { r as reconciliationEntries } from "./workspace-reconcile-derived-paths-jO1JAiFj.mjs";
import "./workspace-fs-C0S-KsR8.mjs";
import { t as WORKSPACE_RESULT_GIT_TIMEOUT_MS } from "./workspace-result-git-D1KJKlev.mjs";
...
async function collectStagedWorkerArtifacts(input) {
	...
	const changes = [...snapshot.changedEntries.map((entry) => ({
		path: entry.path,
		status: basePaths.has(entry.path) ? "modified" : "added",
		additions: 0,
		deletions: 0
	})), ...snapshot.base.entries.filter((entry) => !currentPaths.has(entry.path)).map((entry) => ({
		path: entry.path,
		status: "deleted",
		additions: 0,
		deletions: 0
	...
	const preview = selected?.type === "file" && selected.size <= 262144 ? await readStagedWorkerWorkspaceEntries({
		root: input.root,
...
//#endregion
export { collectStagedWorkerArtifacts, loadStagedWorkerWorkspace, readStagedWorkerWorkspaceEntries };
