...
async function collectStalePluginRuntimeSymlinks(packageRoot = resolveAssistantPackageRootSync({
...
function stalePluginRuntimeSymlinkToHealthFinding(item) {
	return {
		...
		fixHint: "Run `testclaw doctor --fix` to remove stale plugin-runtime symlinks."
	};
}
async function collectStalePluginRuntimeSymlinkHealthFindings(params = {}) {
	return (await collectStalePluginRuntimeSymlinks(params.packageRoot, params)).map(stalePluginRuntimeSymlinkToHealthFinding);
}
...
async function noteStalePluginRuntimeSymlinks(packageRoot, options = {}) {
	...
	const lines = [
		...
		`  ${PLUGIN_RUNTIME_DEPS_MARKER} directories from a previous Assistant install.`,
	...
	lines.push("- Repair: run `testclaw doctor --fix` to remove the dangling symlinks.");
...
async function inspectCandidate(fullPath, fsApi) {
