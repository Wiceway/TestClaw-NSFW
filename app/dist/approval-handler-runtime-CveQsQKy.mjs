...
import { n as buildPendingApprovalView, r as buildResolvedApprovalView, t as buildExpiredApprovalView } from "./approval-view-model-Bf3ZrZpf.mjs";
...
function createChannelApprovalNativeRuntimeAdapter(spec) {
	return {
		...
		...spec.observe ? { observe: {
			...
			...spec.observe.onFinalized ? { onFinalized: (params) => {
				return spec.observe?.onFinalized?.(params);
			} } : {}
		} } : {}
	};
}
