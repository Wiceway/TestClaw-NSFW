const CACHE_PREFIX = "testclaw-control-";
const EMBEDDED_CACHE_VERSION = "2026.9.5-2026-09-23T17-40-55.196Z";
const URL_CACHE_VERSION = new URL(self.location.href).searchParams
  .get("v")
  ?.replace(/[^a-zA-Z0-9._-]/g, "-");
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__TESTCLAW_CONTROL_UI_BUILD_ID__"
...
async function reportControlUiHttpFailure(event) {
  ...
  try {
    ...
    if (client?.type === "window" && controlUiPathname(new URL(client.url)) !== null) {
      client.postMessage({ type: "testclaw-http-request-failed" }, []);
    }
  } catch {
...
self.addEventListener("fetch", (event) => {
  ...
  const cacheable = !(
    pathname.startsWith("/__testclaw__/") ||
...
self.addEventListener("push", (event) => {
  ...
  const options = {
    ...
    tag: data.tag || "testclaw-notification",
