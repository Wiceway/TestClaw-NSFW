...
import { r as resolveAgentConfig, t as AgentSelectionRequiredError } from "./agent-scope-config-Dm8T0OhW.mjs";
import "./session-key-C_bfgyCp.mjs";
import { n as normalizeAccountId, t as DEFAULT_ACCOUNT_ID } from "./account-id-B1bfbA5J.mjs";
import { n as listAgentEntries, r as listAgentEntriesWithSource } from "./agent-roster-Cl9s4QHb.mjs";
import { i as normalizeChatChannelId } from "./ids-CiKpeSuq.mjs";
import { i as resolveProviderToolPolicyEntry, n as normalizeToolProviderPolicyKey, r as resolveProviderToolPolicy } from "./provider-tool-policy-Yi_RXS5Z.mjs";
import { d as resolveToolProfilePolicy } from "./tool-policy-shared-dUIuMpQR.mjs";
import { o as isToolAllowedByPolicies, s as isToolAllowedByPolicyName } from "./tool-policy-match-DDlVID1U.mjs";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-ByO2bSww.mjs"));
...
async function resolveDoctorChannelPreviewConfig(params) {
	const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] = await Promise.all([import("./command-secret-gateway-Bs62oD5H.mjs"), import("./command-secret-targets-CT-IPLu-.mjs")]);
...
async function collectDoctorPreviewNotes(params) {
	...
	const channelPluginRuntime = await import("./channel-plugin-blockers-BZjfcKNN.mjs");
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-BFyikb5T.mjs");
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-Dt1Q1JNc.mjs");
	...
	const { collectCodexRouteWarnings } = await import("./codex-route-warnings-4p0g02Hs.mjs");
	...
	if (hasPluginConfig) {
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-CL7KV88b.mjs");
	...
	if (hasSubagentAllowlistConfig(params.cfg)) {
		const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } = await import("./stale-subagent-allowlist-CzVcu8A-.mjs");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-D0QfTEKi.mjs");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-DbIlx-fa.mjs");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-BkiZ94aP.mjs");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-Bh_l6H1c.mjs");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-CGfhcZf8.mjs");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-CCtZidvK.mjs");
	...
	const { collectStaleConfiguredAuthOrderWarnings } = await import("./stale-auth-order-Zm9M4Qlu.mjs");
	warnings.push(...collectStaleConfiguredAuthOrderWarnings({
		cfg: params.cfg,
		doctorFixCommand: params.doctorFixCommand,
		env
	}));
	const { repairMergedGatewayOwnerProfile } = await import("./user-profiles-owner-migration-B4PVHYtf.mjs");
