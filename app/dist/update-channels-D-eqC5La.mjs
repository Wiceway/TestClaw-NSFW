...
import { i as normalizeLegacyDotBetaVersion, n as compareValidSemver } from "./semver-BiH_OTew.mjs";
...
/**
* Env var carrying the *effective* update channel into `testclaw update finalize`
...
* channel split (`TESTCLAW_UPDATE_POST_CORE_CHANNEL` vs `…_REQUESTED_CHANNEL`).
*/
const UPDATE_EFFECTIVE_CHANNEL_ENV = "TESTCLAW_UPDATE_EFFECTIVE_CHANNEL";
...
/** Maps an Assistant update channel to the npm dist-tag used for package lookups. */
...
function selectNpmChannelVersion(beta, latest) {
	if (!latest.version) return beta;
	if (!beta.version) return latest;
	const comparison = compareAssistantReleaseVersions(beta.version, latest.version) ?? compareValidSemver(normalizeLegacyDotBetaVersion(beta.version), normalizeLegacyDotBetaVersion(latest.version));
...
function hasExtendedStablePatch(parsed) {
	return parsed !== null && parsed.major >= 1e3 && parsed.major <= 9999 && parsed.minor >= 1 && parsed.minor <= 12 && parsed.patch >= 33;
}
...
function isExtendedStableReleaseVersion(version) {
	...
	return hasExtendedStablePatch(parsed) && parsed.build.length === 0 && parsed.prerelease.length === 0;
}
...
function isStableTag(tag) {
	return !hasExtendedStablePatch(parse(tag.trim())) && !isPrereleaseTag(tag);
}
...
function resolveEffectiveUpdateChannel(params) {
	...
	if (params.currentVersion && isBetaTag(params.currentVersion)) return {
		channel: "beta",
	...
	if (params.installKind === "git") {
		...
		if (tag) {
			if (isExtendedStableReleaseVersion(tag)) return {
				channel: "extended-stable",
				source: "git-tag"
			};
			return {
...
//#endregion
export { channelToNpmTag as a, normalizeUpdateChannel as c, resolveRegistryUpdateChannel as d, resolveUpdateChannelDisplay as f, UPDATE_EFFECTIVE_CHANNEL_ENV as i, resolveDevUpstreamRefs as l, DEV_BRANCH as n, isBetaTag as o, selectNpmChannelVersion as p, EXTENDED_STABLE_TAG_UNSUPPORTED_REASON as r, isStableTag as s, DEFAULT_PACKAGE_CHANNEL as t, resolveEffectiveUpdateChannel as u };
