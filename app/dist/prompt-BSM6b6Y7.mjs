...
import { H as STRUCTURED_MEMORY_FAILURE_STATUSES, V as STRUCTURED_MEMORY_EMPTY_STATUSES, W as TIMEOUT_BOILERPLATE_PATTERNS, o as ACTIVE_MEMORY_PLUGIN_TAG, r as ACTIVE_MEMORY_CONTEXT_HEADER, z as NO_RECALL_VALUES } from "./types-DgEaE0BK.mjs";
import { a as extractTextContentParts } from "./query-CxEf9w2F.mjs";
//#region extensions/active-memory/prompt.ts
const ACTIVE_MEMORY_RECALL_OUTCOME_TEXT = {
	"skipped-no-recall-intent": "Active Memory intentionally skipped deep recall because this turn did not ask for past context.",
	unavailable: "Active Memory could not retrieve memory for this turn. Do not assume that no relevant memory exists."
};
...
function normalizeActiveSummary(rawReply) {
	...
	if (!singleLine || normalizeNoRecallValue(singleLine) || isTimeoutBoilerplateSummary(singleLine) || ASSISTANT_CHITCHAT_PATTERNS.some((pattern) => pattern.test(singleLine))) return null;
	return singleLine;
}
...
function buildRecallOutcomePrefix(outcome) {
	return buildPromptPrefix(ACTIVE_MEMORY_RECALL_OUTCOME_TEXT[outcome]);
}
//#endregion
export { normalizeActiveSummary as a, readStructuredMemoryFailure as c, buildRecallPrompt as i, readStructuredMemoryFailureFromContent as l, buildPromptPrefix as n, readExplicitMemoryEvidence as o, buildRecallOutcomePrefix as r, readStructuredMemoryEvidenceFromContent as s, buildMetadata as t, truncateSummary as u };
