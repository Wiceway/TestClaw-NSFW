...
import { n as getActiveManagedProxyTlsOptions, r as getActiveManagedProxyUrl } from "./active-proxy-state-Cv_uMpbF.mjs";
import { once } from "node:events";
import tls from "node:tls";
import { openProxyConnectTunnel } from "@testclaw/proxyline";
...
function normalizeConnectProxyUrl(proxyUrl) {
	...
	try {
		...
		const detail = err instanceof URIError ? "URI malformed" : err instanceof Error ? err.message : String(err);
	...
	try {
		return await new Promise((resolve, reject) => {
			...
			const request = session.request({
				...
				authorization: "bearer intentionally.invalid.testclaw.proxy.validation",
				"apns-topic": "ai.testclaw.ios",
			...
			request.end(JSON.stringify({ aps: { alert: "Assistant APNs proxy validation" } }));
		});
	} finally {
