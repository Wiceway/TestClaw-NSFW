...
import { h as readToolStringParam } from "./common-C3p8rD5A.mjs";
...
import { a as isPersistentSystemAgentOperation, n as SYSTEM_AGENT_OPERATOR_NAVIGATION_HANDOFF, s as validateSystemAgentPluginInstallSpec, t as SYSTEM_AGENT_OPERATOR_APPROVAL_HANDOFF } from "./operations-parse-D2_r8HP6.mjs";
...
import { n as hashSystemAgentOperation } from "./operator-approval-Ch9Cihoi.mjs";
			...
			if (isPersistentSystemAgentOperation(operation)) {
				...
				if (!(params.approved === true && options.approvalArmed === true && options.proposalRef?.current === operationHash)) {
					...
					if (operation.kind === "plugin-activate-artifact") {
						signal?.throwIfAborted();
						const { prepareSystemAgentPluginArtifact } = await import("./plugin-artifact-C3coYSsn.mjs");
			...
			try {
				await executeSystemAgentOperation(operation, capture, {
					...
					deps: {
						...
						loadOverview: async () => (await import("./overview-CkQtJenK.mjs")).loadSystemAgentOverview({ agentId: options.agentId })
					}
				});
			} catch (error) {
...
//#endregion
export { resolveSystemAgentDirectiveTransition as n, resolveSystemAgentProposalTransition as r, createSystemAgentTool as t };
