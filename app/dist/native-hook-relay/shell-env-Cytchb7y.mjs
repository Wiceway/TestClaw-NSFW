...
import { A as resolveTimerTimeoutMs, E as parseStrictNonNegativeInteger } from "./utils-CTn0cI82.mjs";
import { et as pruneMapToMaxSize } from "./redact-CTzbrAJ7.mjs";
import { c as isTruthyEnvValue, i as sanitizeHostExecEnv } from "./host-env-security-DhI6-aX4.mjs";
...
function createLoginShellEnvCacheKey(params) {
	const startupEnvEntries = Object.entries(params.execEnv).filter(([key]) => {
		...
		return key.startsWith("XDG_") || key.startsWith("TESTCLAW_");
...
function loadShellEnvFallback(opts) {
	...
	if (!probe.ok) {
		logger.warn(`[testclaw] shell env fallback failed: ${probe.error}`);
		return {
			ok: false,
			error: probe.error,
			applied: []
...
function shouldEnableShellEnvFallback(env) {
	return isTruthyEnvValue(env.TESTCLAW_LOAD_SHELL_ENV);
}
function shouldDeferShellEnvFallback(env) {
	return isTruthyEnvValue(env.TESTCLAW_DEFER_SHELL_ENV_FALLBACK);
}
function resolveShellEnvFallbackTimeoutMs(env) {
	const raw = env.TESTCLAW_SHELL_ENV_TIMEOUT_MS?.trim();
