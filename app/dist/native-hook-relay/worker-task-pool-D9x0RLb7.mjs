...
import { i as resolveRuntimeWorkerThreadExecArgv, t as createCpuTrackedWorker } from "./worker-cpu-Bem1mHsf.mjs";
...
const CANCELLED = 1;
const SECTION = 2;
resolveGlobalSingleton(Symbol.for("testclaw.workerTaskNativeSection"), () => new AsyncLocalStorage());
		...
		const worker = runInWorkerPoolContext(() => {
			...
			if (temporaryDirectory) {
				...
				slot.releaseResources = async () => {
					try {
						const { removeTemporaryArtifacts } = await import("./temp-artifact-cleanup-CF39QI12.mjs");
