...
import { n as readResponseWithLimit, t as cancelUnreadResponseBody } from "./http-body-QkS0Ooqs.mjs";
import { c as isOfficialExternalPluginCatalogFeed, d as resolveHostedCatalogFeedSource, g as MANIFEST_KEY, h as shouldRequireManifestInstallSourceRef, i as filterOfficialExternalPluginCatalogEntriesBySourceRefs, l as parseOfficialExternalPluginCatalogEntries, n as HostedCatalogSignedFeedMonotonicityError, r as dedupeOfficialExternalPluginCatalogEntries, u as parseOfficialExternalPluginCatalogTimestamp } from "./official-external-plugin-catalog-source-D8QR4Q8K.mjs";
import { n as listOfficialExternalPluginCatalogEntries } from "./official-external-plugin-catalog-NW7yIJOC.mjs";
...
async function parseHostedCatalogFeedBody(params) {
	...
	if (params.verification?.mode === "signed") {
		const { verifyOfficialExternalPluginCatalogSignedEnvelope } = await import("./official-external-plugin-catalog-envelope-DjOeFbwP.mjs");
...
async function resolveHostedCatalogSnapshotStore(params) {
	...
	const { createSqliteHostedOfficialExternalPluginCatalogSnapshotStore } = await import("./official-external-plugin-catalog-snapshot-store-mXogLTpz.mjs");
...
async function loadHostedOfficialExternalPluginCatalogEntries(params) {
	...
	let release;
	try {
		const { fetchWithSsrFGuard } = await import("./fetch-guard-CVMHcUY0.mjs");
