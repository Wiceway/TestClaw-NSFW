import { h as executeWithCachedStatement } from "./testclaw-state-db.paths-Bcv76PAw.mjs";
import { F as normalizeLowercaseStringOrEmpty, I as normalizeNullableString } from "./utils-CTn0cI82.mjs";
import { c as redactRegisteredSecretValues, s as hasRegisteredSecretValuesForRedaction, y as resolveStateDir } from "./redact-CTzbrAJ7.mjs";
import { d as resolveSqliteDatabaseFilePaths } from "./testclaw-state-worker-error-D40PX6nT.mjs";
import { n as runAssistantStateWriteTransaction, t as openAssistantStateDatabase } from "./testclaw-state-db-D-j7_f5b.mjs";
import { o as normalizeUniqueStringEntries } from "./string-normalization-Cwp6Gnf_.mjs";
import { $t as sha256Hex, G as executeSqliteQuerySync, It as runSqliteImmediateTransactionSync, K as executeSqliteQueryTakeFirstSync, Kt as applyPrivateModeSync, W as compileSqliteQueryBindings, Y as configureSqliteConnectionPragmas, Z as registerSqliteCacheExitClose, j as migrateSqliteSchemaToStrict, q as getNodeSqliteKysely, qt as openNodeSqliteDatabase, s as retainAssistantStateDatabaseForIdle } from "./testclaw-state-db-cache-BihpKglZ.mjs";
import { t as isHeadersLike } from "./fetch-headers-DD03wtQj.mjs";
import { n as withResponseBodyTimeout } from "./http-response-body-timeout-DpKABGqT.mjs";
...
var DebugProxyCaptureKernel = class {
	...
	persistPayload(data, contentType) {
		...
		if (this.capturePathBased) {
			fsSync.mkdirSync(this.capturePathBased.blobDir, {
				recursive: true,
				mode: 448
			});
			const outputPath = path.join(this.capturePathBased.blobDir, `${blobId}.bin.gz`);
			if (!fsSync.existsSync(outputPath)) fsSync.writeFileSync(outputPath, gzipSync(data), { mode: 384 });
	...
	readBlob(blobId) {
		if (this.capturePathBased) {
			...
			return fsSync.existsSync(blobPath) ? gunzipSync(fsSync.readFileSync(blobPath)).toString("utf8") : null;
		}
	...
	purgeAll() {
		...
		if (this.capturePathBased) {
			...
			let blobs = 0;
			if (fsSync.existsSync(this.capturePathBased.blobDir)) for (const entry of fsSync.readdirSync(this.capturePathBased.blobDir)) {
				fsSync.rmSync(path.join(this.capturePathBased.blobDir, entry), { force: true });
				blobs += 1;
			}
			return {
	...
	deletePathBasedSessions(sessionIds) {
		...
		for (const blobId of candidateBlobIds) {
			...
			if (fsSync.existsSync(blobPath)) {
				fsSync.rmSync(blobPath, { force: true });
				blobs += 1;
			}
		}
		return {
...
function hardenLegacyDatabaseFiles(dbPath) {
	for (const candidate of resolveSqliteDatabaseFilePaths(dbPath)) if (fsSync.existsSync(candidate)) applyPrivateModeSync(candidate, 384);
}
function openPathBasedDebugProxyCaptureStore(dbPath, blobDir) {
	...
	if (fileBackedPath) {
		fsSync.mkdirSync(path.dirname(fileBackedPath), {
			recursive: true,
			mode: 448
		});
		if (!fsSync.existsSync(fileBackedPath)) fsSync.closeSync(fsSync.openSync(fileBackedPath, "a", 384));
	}
