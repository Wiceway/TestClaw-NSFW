import { L as normalizeOptionalLowercaseString } from "./utils-CTn0cI82.mjs";
import { c as sortUniqueStrings } from "./string-normalization-Cwp6Gnf_.mjs";
import { AsyncLocalStorage } from "node:async_hooks";
//#region src/shared/lazy-promise.ts
/** Returns the cached promise for a key, creating and storing it when absent. */
function getOrCreatePromise(cache, key, create, options = {}) {
	...
	if (options.evictOnSettled === true) created.then(evict, evict);
	else if (options.cacheRejections === false) created.catch(evict);
	return created;
}
/**
...
createLazyPromise(() => import("./subsystem-5vt5YuG6.mjs").then(({ createSubsystemLogger }) => createSubsystemLogger("env")), { cacheRejections: true });
...
function isNoPagerOverride(key, value) {
	return (key.toUpperCase() === "GIT_PAGER" || key.toUpperCase() === "PAGER") && (value === "" || value === "cat");
}
...
//#endregion
export { SUBAGENT_EXEC_ENV_VAR as a, isTruthyEnvValue as c, getOrCreatePromise as d, sanitizeHostExecEnv as i, normalizeZaiEnv as l, isDangerousHostEnvVarName as n, markAssistantExecEnv as o, normalizeEnvVarKey as r, expandEnvNormalizationKeys as s, isDangerousHostEnvOverrideVarName as t, resolveEnvNormalizationKeys as u };
