...
import { i as normalizeChannelId, n as getLoadedChannelPlugin, t as getChannelPlugin } from "./registry-DcRiLbUb.mjs";
...
import { r as parseOptionalDelimitedEntries } from "./helpers-DJWU8tX8.mjs";
...
import { n as resolveChannelSetupCliOptionMetadata } from "./cli-add-options-DkqOYkbq.mjs";
...
import { i as withCommandPluginMetadata, r as requireValidConfigForWrite } from "./config-validation-DQS8Bqjh.mjs";
import { t as normalizeExternalChannelSetupConfig } from "./config-compatibility-CA15Ojxq.mjs";
import { l as shouldUseWizard } from "./shared-D0fdYAaw.mjs";
...
const loadChannelSetupPluginInstall = createLazyPromise(() => import("./plugin-install-CXo7kCco.mjs"));
const loadOnboardChannels = createLazyPromise(() => import("./onboard-channels-CxCC4CyW.mjs"));
...
async function resolveCatalogChannelEntry(raw, cfg, resolveWorkspaceDir) {
	...
	if (!trimmed) return;
	return (await import("./trusted-catalog-BfaCFENo.mjs").then(({ listTrustedChannelPluginCatalogEntries }) => listTrustedChannelPluginCatalogEntries({
		cfg,
...
async function configureChannelAccount(writeSnapshot, opts, runtime, params) {
	...
	if (shouldUseWizard(params)) {
		const { resolveInitialWizardChannelTarget, runChannelsAddWizardFlow, selectChannelSetupOwner } = await import("./add-wizard-CZx8NGBa.mjs");
	...
	if (catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-CgWyTXm7.mjs");
