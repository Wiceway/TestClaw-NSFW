...
import { s as withExistingAssistantStateDatabaseArtifactPreservingReadOnly, t as executeExistingAssistantStateRead } from "./testclaw-state-db-readonly-L2ePyI_M.mjs";
...
import { C as ensureRepositoryGitHubPublicationSchema, S as ensurePersonalGitHubPublicationSchema, c as runAssistantStateWriteTransaction, r as openAssistantStateDatabase, v as ensureGitHubPublicationSchema } from "./testclaw-state-db-BXFT1fUC.mjs";
import { r as getRuntimeConfig } from "./io.runtime-DIHH_X2V.mjs";
import "./config-DqAgdhnz.mjs";
import { t as runTasksWithConcurrency } from "./run-with-concurrency-Dtu208ef.mjs";
import { s as WORKER_ADMISSION_DEADLINE_MS } from "./worker-process-protocol-DAgOy1yj.mjs";
import { k as NODE_WORKER_WORKSPACE_RETAIN_COMMAND } from "./node-commands-BLhGKTZa.mjs";
...
import { m as readOpenIncognitoAgentDatabaseGeneration } from "./testclaw-agent-db-lifecycle-BQsqjh85.mjs";
import { d as retainAssistantAgentDatabaseReadCandidates } from "./testclaw-agent-db-DAdiee0a.mjs";
import { a as prepareAssistantAgentDatabaseRegistrySnapshotRead } from "./testclaw-agent-db-registry-listing-bY5cRH88.mjs";
import { o as readUserGitHubConnection, s as resolvePersonalGitHubOwner } from "./user-github-connections-8PXv_bZt.mjs";
import { r as GatewayOperatorAccessUnavailableError } from "./operator-access-policy-FuVg7FFf.mjs";
import { n as captureSessionStoreReadCandidate } from "./session-store-read-candidates-AxJeGVjE.mjs";
import { c as resolveUnsuffixedSqliteTargetFromSessionStorePath } from "./session-sqlite-target-CBM31t7c.mjs";
import { o as normalizeSqliteSessionKey } from "./session-accessor.sqlite-scope-kTo4D2H6.mjs";
import { n as emitSessionLifecycleEvent, r as onSessionIdentityMutation } from "./session-lifecycle-events-BReqLf0S.mjs";
import { o as captureCanonicalSessionReaderContinuation } from "./session-canonical-key-D8rnGIu3.mjs";
import { Z as getSessionRepositoryWorkspaceStore } from "./session-accessor.sqlite-entry-store-Ct1v5fIu.mjs";
import { S as runExclusiveSessionStoreWrite, _ as startSessionWorkAdmissionInterruption, f as interruptSessionWorkAdmissions, g as runExclusiveSessionLifecycleMutation, h as isSessionWorkAdmissionActive, i as closeSessionWorkAdmissions, n as beginSessionWorkAdmission, t as SESSION_WORK_ADMISSION_DRAIN_TIMEOUT_MS } from "./session-lifecycle-admission-8OlXMJli.mjs";
...
import { c as appendSessionTranscriptReport, u as readLatestSessionTranscriptReport } from "./session-accessor-CtBBLApI.mjs";
import { i as resolveSessionStoreKey, n as resolveSessionStoreAgentId } from "./session-store-key-GnE6aqPA.mjs";
import { n as readSessionIdentityEvidenceBatch } from "./session-accessor.sqlite-entry-availability-CAcfV00q.mjs";
...
import { a as recoverWorkerWorkspaceReconciliation, t as applyStagedWorkerWorkspace } from "./workspace-reconcile-BBxtIEQ4.mjs";
import { o as isCurrentWorkerWorkspacePendingResultOwner, t as ActiveTurnClaimError } from "./placement-turn-claims-BGedi3bm.mjs";
import { t as boundedWorkerError } from "./worker-error-ylcLWUlF.mjs";
import { t as SessionWorkspaceReservationBusyError } from "./placement-workspace-reservation-BkrdaFda.mjs";
import { a as projectWorkspaceResultConflict, i as formatWorkspaceConflictSummary, n as WORKSPACE_CONFLICT_TRANSCRIPT_TYPE, r as WORKSPACE_RECOVERY_FAILURE_TRANSCRIPT_TYPE, t as WORKSPACE_CONFLICT_CLEARED_TRANSCRIPT_TYPE } from "./workspace-conflicts-CQVfTI04.mjs";
import { l as resolveNodeCommandAllowlist } from "./node-command-policy-DonFHl1h.mjs";
...
import { N as readWorkerPlacementIdentity, j as createWorkerPlacementRunnerAvailabilityReader } from "./session-row-prepared-read-B632AuGn.mjs";
...
import { p as resolveConfiguredGitHubToolIdentity, u as preparePersonalGitHubPublicationIdentity } from "./github-tool-identity-CY0H5w86.mjs";
import { t as clearSessionQueues } from "./cleanup-EJ6UuSTU.mjs";
import { n as resolveWorkerPlacementDestination } from "./placement-destination-2QVG7nzc.mjs";
import { n as installSessionPlacementAdmissionProvider } from "./session-placement-admission-CKM9VBQb.mjs";
...
import { n as resolveGitHubRepositoryTarget } from "./github-repository-target-Dzw_8VNK.mjs";
import { a as readProjectCheckoutRemoteHead, i as ProjectCloneError, t as materializeProjectClone } from "./project-clone-DIwIPQLC.mjs";
import { t as prepareSessionWorktree } from "./session-worktree-preparation-CHLumVPR.mjs";
import { t as parseGitHubRemoteUrl } from "./github-remote-CloOx2kv.mjs";
import { n as WorkerRunnerUnavailableError, r as WorkerTunnelOwnerDisconnectedError, t as WorkerRunnerCapacityError } from "./tunnel-contract-C4XjTxW-.mjs";
import { a as withSessionRepositoryCheckpoint, i as stageSessionRepositoryCheckpoint } from "./session-repository-checkpoints-Bhyie6gb.mjs";
import { a as isUnavailableEnvironment, c as createDevicePlacementAuthority, i as isExactAttachedEnvironment, l as resolveDevicePlacementEligibility, n as createPlacementFailureActions, o as workerDisappearanceError, r as isCurrentActiveWorkerEnvironment, s as DevicePlacementUnavailableError } from "./placement-dispatch-failure-BZdCLSED.mjs";
import { i as supportsCurrentWorkerLaunch, n as StaleWorkerBuildError, o as verifyWorkerAdmissionHandshake } from "./admission-xe_2NmwN.mjs";
import { t as matchesWorkerPlacementTarget } from "./placement-reclaim-contract-BNCBrypU.mjs";
import { a as recoverGitHubPublicationBranchAndIndex, i as assertGitHubPublicationRefCasCompleted, n as restoreGitHubPublicationRequester, o as updateGitHubPublicationBranchAndIndex, r as GitHubPublicationRecoveryPendingError } from "./github-publication-requester-DYakdKk8.mjs";
...
const table$1 = "github_personal_publication_requests";
...
function readPersonalGitHubPublication(owner, request) {
	...
	let selection = query$1(db).selectFrom(table$1).selectAll().where("owner_profile_id", "=", owner);
...
function insertPersonalGitHubPublication(row, lifecycleRevision, assertCurrent) {
	return runAssistantStateWriteTransaction(({ db }) => {
		...
		executeSqliteQuerySync(db, query$1(db).insertInto(table$1).values(row));
...
function claimPersonalGitHubPublication(row, instanceId, assertCurrent) {
	...
	const claimed = runAssistantStateWriteTransaction(({ db }) => {
		assertCurrent();
		assertOwner(row.owner_profile_id);
		if (executeSqliteQuerySync(db, query$1(db).updateTable(table$1).set({
			status: "publishing",
	...
	const write = (values, requireAction) => runAssistantStateWriteTransaction(({ db }) => {
		...
		const current = executeSqliteQueryTakeFirstSync(db, query$1(db).selectFrom(table$1).selectAll().where("owner_profile_id", "=", row.owner_profile_id).where("request_id", "=", row.request_id));
		...
		const updated = executeSqliteQueryTakeFirstSync(db, query$1(db).updateTable(table$1).set({
			...values,
			updated_at_ms: Date.now()
...
function requirePersonalGitHubPublicationConfirmation(instanceId) {
	...
	if (!tableExists(database.db, table$1)) return;
	runAssistantStateWriteTransaction(({ db }) => {
		executeSqliteQuerySync(db, query$1(db).updateTable(table$1).set({
			status: "needs_confirmation",
			updated_at_ms: Date.now()
...
function listUnreportedPersonalGitHubPublications() {
	const db = openAssistantStateDatabase().db;
	if (!tableExists(db, table$1)) return [];
	return executeSqliteQuerySync(db, query$1(db).selectFrom(table$1).selectAll().where("status", "in", ["published", "failed"]).where("reported_at_ms", "is", null).orderBy("updated_at_ms")).rows.map((row) => {
...
function markPersonalGitHubPublicationReported(requestId) {
	...
	if (!tableExists(database.db, table$1)) return;
	runAssistantStateWriteTransaction(({ db }) => {
		executeSqliteQuerySync(db, query$1(db).updateTable(table$1).set({ reported_at_ms: Date.now() }).where("request_id", "=", requestId).where("status", "in", ["published", "failed"]));
...
const loadWorkerPlacementSessionRuntimeModule = createLazyRuntimeModule(async () => {
	const [placementSessionRuntime, { managedWorktrees }, sessionUtils] = await Promise.all([
		import("./placement-session-runtime-BFnht2aw.mjs"),
		import("./service-C0yVLDnU.mjs"),
...
const loadWorkerTurnExecution = createLazyRuntimeModule(() => import("./worker-turn-execution-6ZN1FmOY.mjs"));
const loadRemoteExecTurn = createLazyRuntimeModule(() => import("./workspace-result-finalize-C4bMyXgd.mjs"));
const loadPlacementSandbox = createLazyRuntimeModule(() => import("./placement-sandbox-BjeXgvBb.mjs"));
...
const loadWorkerWorkspacePreflight = createLazyRuntimeModule(async () => {
	const { preflightWorkerWorkspace } = await import("./workspace-sync-preflight-CpV2_Pjy.mjs");
