import { D as withPluginCache, a as createPluginCache, o as getPluginCache } from "./plugin-cache-CTGtP6hf.mjs";
...
import { t as migrateLegacyConfig } from "./legacy-config-migrate-Ccd6TXfs.mjs";
import { t as addDoctorLegacyIssues } from "./legacy-config-issues-dZ7YhAnj.mjs";
import { t as completeDoctorPluginMetadataSnapshot } from "./plugin-metadata-snapshot-scope-U6ZRCRVe.mjs";
...
const loadInstalledPluginIndexStoreWrite = createLazyRuntimeModule(() => import("./installed-plugin-index-store-write-B6KJNTYH.mjs"));
...
function createDoctorRehearsalSnapshotPreparation(report) {
	...
	const prepareSnapshot = async (snapshot) => {
		if (completed) return;
		const { completeUpdateCandidatePluginRehearsal } = await import("./update-candidate-plugin-repair-D-aPS56G.mjs");
