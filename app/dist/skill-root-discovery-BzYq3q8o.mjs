...
import { c as findContainingAllowedSkillSymlinkTarget, i as compactSkillPath, u as tryRealpath } from "./local-loader-CT9lL3Xc.mjs";
...
function listChildDirectories(dir, opts) {
	...
	if (budget) budget.remainingDirectoryScans -= 1;
	const maxRawEntriesToScan = resolveRawEntryScanLimit(opts.maxCandidateDirs);
	const scan = walkDirectorySync(dir, {
		maxDepth: 1,
		maxEntries: budget ? Math.min(maxRawEntriesToScan, budget.remainingRawEntries) : maxRawEntriesToScan,
		symlinks: opts.followSymlinks === false ? "skip" : "include",
		include: (entry) => (entry.kind === "directory" || entry.kind === "symlink") && !entry.name.startsWith(".") && entry.name !== "node_modules"
	});
	const dirs = scan.entries.flatMap((entry) => {
		try {
			return entry.kind === "directory" || fs.statSync(entry.path).isDirectory() ? [entry.name] : [];
		} catch (error) {
			opts.onDiagnostic?.({
				kind: "read",
				path: entry.path,
	...
	for (const failure of scan.failedDirs) if (!isMissingPathError(failure.error)) opts.onDiagnostic?.({
		kind: "read",
		path: failure.path,
...
function resolveRawEntryScanLimit(maxCandidateDirs) {
	if (maxCandidateDirs <= 0) return 0;
	return Math.min(DEFAULT_MAX_RAW_ENTRIES_PER_DIRECTORY_SCAN, Math.max(DEFAULT_MIN_RAW_ENTRIES_PER_DIRECTORY_SCAN, maxCandidateDirs * 10));
}
...
function buildEscapedSkillPathReason(params) {
	...
	if (params.source === "testclaw-bundled" && candidateIsSymlink) return {
	...
	if (params.source === "testclaw-bundled") return {
...
function resolveNestedSkillsRoot(dir, maxEntriesToScan) {
	...
	if (!hasSkillFileCandidate(dir) && containsDiscoverableSkill(dir, {
	...
	for (const candidate of queue) {
		if (hasSkillFileCandidate(candidate.dir)) return nested;
		if (candidate.depth >= MAX_GROUPED_SKILL_SCAN_DEPTH) continue;
		const childDirs = listChildDirectories(candidate.dir, {
			budget: discoveryBudget,
			followSymlinks: false,
...
function shouldEnforceConfiguredSkillRootContainment(source) {
	return source !== "testclaw-managed" && source !== "agents-skills-personal";
}
function shouldUseConfiguredSymlinkTargets(source) {
	return source === "testclaw-workspace" || source === "testclaw-extra" || source === "agents-skills-project";
}
...
function resolveSkillFilePath(params) {
	...
	return resolved || tryRealpath(params.candidatePath) ? resolved : path.resolve(params.candidatePath);
}
function discoverSkillCandidates(params) {
	...
	try {
		...
		if (!isMissingPathError(error)) params.onDiagnostic?.({
			kind: "read",
			path: rootDir,
			message: String(error)
		});
	...
	const rootIsContainer = params.source === "testclaw-workshop";
	const baseDir = rootIsContainer ? rootDir : resolveNestedSkillsRoot(params.dir, params.limits.maxCandidatesPerRoot);
	...
	if (!rootIsContainer && hasSkillFileCandidate(baseDir)) return {
		candidates: resolveSkillFilePath({
			source: params.source,
			skillDir: baseDir,
	...
	const limitedChildren = maxSkillsLoadedPerSource === 0 ? [] : childDirs.slice(0, maxCandidatesPerRoot);
	if (maxSkillsLoadedPerSource > 0 && childDirs.includes("skills") && !limitedChildren.includes("skills")) limitedChildren.push("skills");
	if (childDirScan.truncated) skillsLogger.warn("Skills root looks suspiciously large, truncating discovery.", {
		dir: params.dir,
		baseDir,
	...
	for (const candidate of scanQueue) {
		...
		const maxGroupedDepth = params.source === "testclaw-extra" && !baseDirIsNestedSkillsRoot && !baseDirLooksLikeSkillsRoot && candidatePath !== nestedSkillsRootPath && !isPathInside(nestedSkillsRootPath, candidatePath) ? MAX_CONFIGURED_ROOT_GROUPED_SKILL_SCAN_DEPTH : MAX_GROUPED_SKILL_SCAN_DEPTH;
	...
	if (discoveryBudget.truncated) {
		params.onDiagnostic?.({
			kind: "invalid",
			path: rootDir,
			message: "Skill inventory reached its discovery limit; inspect the remaining skills separately."
		});
...
function discoverPluginSkills(params) {
	...
	if (!allowedRoots.some(({ realPath }) => realPath !== null)) return [];
	...
	for (const name of childDirs) {
		...
		const pluginRoot = skillDirRealPath && allowedRoots.find(({ realPath }) => realPath !== null && isPathInside(realPath, skillDirRealPath));
		...
		candidates.push({
			skillDir,
			skillDirRealPath,
			rejectHardlinks: pluginRoot.rejectHardlinks
		});
	}
	return candidates;
}
//#endregion
export { resolveSkillDiscoveryLimits as a, isSymlinkPath as i, discoverPluginSkills as n, discoverSkillCandidates as r, canonicalSkillDirForSource as t };
