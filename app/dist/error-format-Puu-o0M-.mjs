...
function formatUnknownChannelMessage(params) {
	...
	const listCommand = params.listCommand ?? "testclaw channels list --all";
...
function formatUnsupportedChannelActionMessage(params) {
	const inspectCommand = params.inspectCommand ?? `testclaw channels capabilities --channel ${params.channel}`;
...
function formatStrictJsonParseFailure(params) {
	...
	return [
		...
		`${cause}.`,
		`Use valid JSON. For structured changes, use a JSON5 config patch object file with ${formatInlineCliCommand("testclaw config patch --file <path> --dry-run")}.`,
...
function formatGatewayCommandFailure(params) {
	const message = (params.error instanceof Error ? params.error.message : String(params.error)).replace(/\s*Run [`"]?testclaw doctor[`"]? for diagnostics\.?/gi, "").replace(/\s+Gateway target:\s+.*$/isu, "").replace(/\s+/g, " ").trim().replace(/[.。]+$/u, "");
	const inspectCommand = params.inspectCommand ?? "testclaw gateway status --deep";
...
function formatMissingPluginMessage(params) {
	const listCommand = params.listCommand ?? "testclaw plugins list";
	const searchHint = params.includeSearch ? `, or ${formatInlineCliCommand("testclaw plugins search " + params.id)} to look for installable plugins` : "";
