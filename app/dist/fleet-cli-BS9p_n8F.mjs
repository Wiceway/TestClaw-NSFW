...
import { y as validateDiskSize } from "./cell-profile-YWHBU0uF.mjs";
import { InvalidArgumentError } from "commander";
//#region src/cli/fleet-cli/register.ts
const fleetRuntimeLoader = createLazyImportLoader(() => import("./commands.runtime-Da8xHMER.mjs"));
...
function registerFleetCli(program) {
	...
	fleet.command("create").description("Create an isolated tenant cell").argument("<tenant>", "Tenant slug").option("--image <ref>", "Container image", "ghcr.io/testclaw/testclaw:latest").option("--runtime <runtime>", "Container runtime (docker or podman)", parseContainerRuntime, "docker").option("--port <port>", "Host loopback port (default: allocate from 19100)", parsePort).option("--memory <limit>", "Container memory limit", "2g").option("--cpus <count>", "Container CPU limit", parseCpus, "2").option("--disk <size>", "Cap the container writable layer (requires overlay2+XFS pquota, btrfs, or zfs)", parseDisk).option("--network <mode>", "Container egress network (bridge or internal)", parseNetwork, "bridge").option("--pids-limit <count>", "Container process limit", (value) => parseStrictPositiveIntOption(value, "--pids-limit"), 512).option("--env <KEY=VAL>", "Pass an environment variable to the cell", collectOption, []).option("--gateway-token <token>", "Use an existing Gateway token").option("--no-start", "Create the container without starting it").option("--json", "Output JSON", false).action(async (tenant, options) => {
	...
	fleet.command("logs").description("Stream tenant cell container logs").argument("<tenant>", "Tenant slug").option("--follow", "Follow log output", false).option("--timestamps", "Show timestamps", false).option("--tail <count>", "Number of lines to show", (value) => parseStrictPositiveIntOption(value, "--tail")).option("--since <value>", "Show logs since a duration or timestamp").action(async (tenant, options) => {
