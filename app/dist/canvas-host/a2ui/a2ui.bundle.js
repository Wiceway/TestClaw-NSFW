...
(() => {
    ...
    `];
		...
		render() {
			...
			if (this.thickness !== null && this.thickness !== void 0) {
				if (this.axis === "vertical") dynamicStyle["width"] = `${this.thickness}px`;
...
var AssistantA2UIHost = class extends i$7 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      position: relative;
      box-sizing: border-box;
      padding: var(--testclaw-a2ui-inset-top, 0px) var(--testclaw-a2ui-inset-right, 0px)
        var(--testclaw-a2ui-inset-bottom, 0px) var(--testclaw-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--testclaw-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--testclaw-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--testclaw-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--testclaw-a2ui-empty-top, var(--testclaw-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["testclaw:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	...
	disconnectedCallback() {
		...
		if (this.#statusListener) {
			for (const eventName of ["testclaw:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis["__testclawLastA2UIAction"] = userAction;
		const boardApi = globalThis.testclaw;
		if (boardApi?.state?.emit) {
			const request = globalThis.testclawA2UIBoot?.actionTier === "prompt" && boardApi.prompt?.send ? boardApi.prompt.send(Object.keys(context).length ? `A2UI action ${name}: ${JSON.stringify(context)}` : `A2UI action ${name}`) : boardApi.state.emit({
				eventType: "a2ui.action",
				action: userAction
			});
...
if (!customElements.get("testclaw-a2ui-host")) customElements.define("testclaw-a2ui-host", AssistantA2UIHost);
