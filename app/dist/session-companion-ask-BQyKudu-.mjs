...
async function defaultRun(params) {
	...
	const { prepareInternalSessionEffectsSession, removeInternalSessionEffectsSession } = await import("./internal-session-effects-B4NidmOi.mjs");
	...
	try {
		const [{ SessionManager }, { runEmbeddedAgent }] = await Promise.all([import("./sessions-CwuDo0tw.mjs"), import("./embedded-agent-Dj7EUuO_.mjs")]);
