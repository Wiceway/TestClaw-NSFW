...
import { d as pathExists, o as ensureAbsoluteDirectory, w as root } from "./fs-safe-B1VkXzpu.mjs";
import { d as resolveAgentWorkspaceDir, g as resolveDefaultAgentId, r as resolveAgentConfig } from "./agent-scope-config-Dm8T0OhW.mjs";
import "./agent-scope-_30Scclc.mjs";
import { g as redactMigrationPlan, l as markMigrationItemConflict, n as MIGRATION_REASON_TARGET_EXISTS, t as MIGRATION_REASON_MISSING_SOURCE_OR_TARGET, u as markMigrationItemError } from "./migration-DepQtnwV.mjs";
import { n as assertMemoryMigrationSourceRevision, t as MAX_MEMORY_MIGRATION_FILE_BYTES } from "./memory-migration-source-D0vEfigh.mjs";
import path from "node:path";
import fs from "node:fs/promises";
import crypto from "node:crypto";
import { writeTextAtomic } from "@testclaw/fs-safe/atomic";
...
async function backupMigrationItemTarget(target, reportDir, opts = {}) {
	...
	await fs.cp(target, backupPath, {
		recursive: true,
		force: true,
		...opts.dereference === void 0 ? {} : { dereference: opts.dereference }
	});
	return backupPath;
}
...
async function copyMigrationFileItem(item, reportDir, opts = {}) {
	...
	try {
		...
		const backupPath = opts.overwrite ? await backupMigrationItemTarget(item.target, reportDir) : void 0;
		result = {
			...item,
			details: {
				...item.details,
		...
		if (isFileAlreadyExistsError(err)) return markMigrationItemConflict(result, MIGRATION_REASON_TARGET_EXISTS);
...
async function copyMemoryMigrationFileItem(item, reportDir, opts) {
	...
	try {
		...
		if (opts.overwrite === true && await safeRoot.exists(relativeTarget)) {
			...
			stagingDir = path.join(".testclaw-memory-import-staging", crypto.randomUUID());
		...
		if (stagingDir) {
			...
			await safeRoot.remove(".testclaw-memory-import-staging").catch(() => void 0);
		}
...
//#endregion
export { resolvePlannedMigrationTargets as a, copyMigrationFileItem as i, backupMigrationItemTarget as n, withCachedMigrationConfigRuntime as o, copyMemoryMigrationFileItem as r, writeMigrationReport as s, archiveMigrationItem as t };
