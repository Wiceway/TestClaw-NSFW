...
import { at as projectDeliveryQueueTerminalEntry, ct as deliveryQueueMetadata, rt as inferDeliveryQueueFailureRetention } from "./testclaw-state-db-read-connection-BvrNnfuu.mjs";
import "./installed-plugin-index-Cd3PE_mG.mjs";
import { o as parseRegistryNpmSpec } from "./npm-registry-spec-B-fX1tYr.mjs";
import { c as createPluginInstallRecordMap, f as parsePluginInstallRecordMap, l as getPluginInstallRecordMapEntry, m as setPluginInstallRecordMapEntry, p as serializePluginInstallRecordMap, s as copyPluginInstallRecordMap } from "./installed-plugin-record-match-BHy1WTe7.mjs";
import { t as parseInstalledPluginIndex } from "./installed-plugin-index-store-8IghgXkj.mjs";
import { A as repairLegacyTaskIdentifiers, c as runAssistantStateWriteTransaction } from "./testclaw-state-db-BXFT1fUC.mjs";
...
import { c as readLegacyMigrationSourceSnapshotSync, n as assertLegacyMigrationSourceUnchanged } from "./state-migrations.source-snapshot-DSj7jCLF.mjs";
	...
	for (const queue of LEGACY_DELIVERY_QUEUE_DIRS) {
		...
		for (const file of [...files, ...markerPaths.map((sourcePath) => ({
			...
			if (file.status === "delivered") {
				...
				if (reason && reason !== "delivered" || file.status === "failed") mediaPaths = (await import("./state-migrations.delivery-queue-media-BM2sYynL.mjs")).resolveLegacyDeliveryQueueMediaPaths(entry, params.stateDir);
		...
		for (const { snapshot, sourceKey, reason, mediaPaths, mediaBackups } of committed) {
			...
			try {
				...
				const mediaArchive = mediaPaths.length > 0 ? await (await import("./state-migrations.delivery-queue-media-BM2sYynL.mjs")).preserveLegacyDeliveryQueueMedia({
					mediaPaths,
