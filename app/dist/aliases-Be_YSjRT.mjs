...
import { n as normalizeAgentModelMapForConfig } from "./model-input-DKxKaZGG.mjs";
import { m as DEFAULT_MODEL_ALIASES } from "./validation-core-tZ7JDiKd.mjs";
import { r as logConfigUpdated } from "./logging-BDPh2tEj.mjs";
import { c as resolveModelTarget, f as upsertCanonicalModelConfigEntry, n as ensureFlagCompatibility, u as updateConfig } from "./shared-DVPg8fou.mjs";
import { t as normalizeAlias } from "./alias-name-Bwsh7Ooz.mjs";
...
async function modelsAliasesAddCommand(aliasRaw, modelRaw, runtime) {
	...
	}, (_cfg, context) => [resolveModelTarget({
		raw: modelRaw,
		cfg: context.runtimeConfig
	})]);
...
async function modelsAliasesRemoveCommand(aliasRaw, runtime) {
	...
	const updated = await updateConfig((cfg) => {
		...
		if (!found) {
			...
			if (builtinTarget && normalizedModels[builtinTarget] && normalizedModels[builtinTarget]?.alias === void 0) throw new Error(`Cannot remove "${alias}": it is a built-in alias for "${builtinTarget}" provided automatically by Assistant and is not stored in your config file. To shadow it with a different target, run ${formatCliCommand(`testclaw models aliases add ${alias} <model>`)}.`);
			throw new Error(`Alias not found: ${alias}. Run ${formatCliCommand("testclaw models aliases list")} to see configured aliases.`);
		}
		return {
			...cfg,
			agents: {
