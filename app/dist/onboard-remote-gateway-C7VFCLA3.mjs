...
import { f as isGatewayClientRequestError, v as resolveDeviceIdentityForGatewayCall } from "./call-Cm2P5Cd6.mjs";
...
async function runRemoteGatewayInferenceOnboarding(target, runtime = defaultRuntime, deps = {}) {
	const callGateway = deps.callGateway ?? (await import("./call-BFsjnbnk.mjs")).callGatewayCli;
	const runGuidedOnboarding = deps.runGuidedOnboarding ?? (await import("./commands/onboard-guided.js")).runGuidedOnboarding;
	...
	const activate = async (params) => {
		...
		let started = false;
		let terminal = false;
		const prompter = params.prompter ?? await (deps.createPrompter?.() ?? import("./clack-prompter-D4W0DvqS.mjs").then(({ createClackPrompter }) => createClackPrompter()));
		let result;
		try {
	...
	await runGuidedOnboarding({}, runtime, {
		...
		runSystemAgentChat: async () => {
			...
			await (deps.runTui ?? (await import("./tui-wi2LuHey.mjs")).runTui)({
				config: boundConfig,
				deliver: false,
