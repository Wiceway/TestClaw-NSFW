...
import { A as resolvePluginInstallSources } from "./official-external-plugin-catalog-BxZhHmRY.mjs";
...
import { a as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-DO9H3SxA.mjs";
...
const loadPluginsStatus = createModuleLoader(() => import("./status-4apRMkAV.mjs"));
...
async function applyPluginEnabledThroughGateway(pluginId, enabled, opts = {}) {
	const { resolvePluginLifecycleGateway } = await import("./plugins-lifecycle-client-DXkEGFXe.mjs");
...
async function runPluginPolicyCommand(id, enabled, acceptCapabilities) {
	...
	const { mutateManagedPluginEnabled } = await import("./management-mutations-DUZu2zAF.mjs");
	const { ManagedPluginLifecycleError } = await import("./management-lifecycle-error-DS4OoRDN.mjs");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry } = await import("./plugin-registry-CY4plK1e.mjs");
	...
	if (opts.refresh) {
		const { refreshPluginRegistry } = await import("./plugin-registry-refresh-BA_CMQmz.mjs");
...
async function runPluginMarketplaceEntriesCommand(opts) {
	const catalog = await import("./official-external-plugin-catalog-CcpX6VQ1.mjs");
	const cfg = getRuntimeConfig();
...
async function runPluginMarketplaceRefreshCommand(opts) {
	const { resolvePluginLifecycleGateway } = await import("./plugins-lifecycle-client-DXkEGFXe.mjs");
	const gateway = await resolvePluginLifecycleGateway();
	const { loadConfiguredHostedOfficialExternalPluginCatalogEntries } = await import("./official-external-plugin-catalog-CcpX6VQ1.mjs");
	const cfg = getRuntimeConfig();
	...
	const { clearManagedPluginCatalogCache } = await import("./management-catalog-P6qXcYAk.mjs");
