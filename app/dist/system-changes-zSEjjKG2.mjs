...
import { za as validateSystemChangesListParams } from "./src-BNV0SJoP.mjs";
...
function classifyConfigWriteSource(record) {
	...
	const launcherIndex = record.argv.findIndex((arg) => /(?:^|[/\\])testclaw(?:\.m?js)?$/i.test(arg));
	let command;
...
function configWriteSummary(source, changedPaths) {
	return summarizePaths(source === "doctor" ? "Doctor updated configuration" : source === "config-rpc" ? "Settings updated configuration" : source === "plugin-install" ? "Plugin installation updated configuration" : source === "system-agent" ? "Assistant updated configuration" : source === "cli" ? "CLI updated configuration" : "Configuration updated", changedPaths);
}
	...
	if (value.event === "config.external") {
		...
		return {
			entry: {
				...
				kind: "external-edit",
				source: "external",
				summary: summarizePaths("Configuration edited outside Assistant", changedPaths),
...
const systemChangesHandlers = { "testclaw.changes.list": ({ params, respond }) => {
	if (!assertValidParams(params, validateSystemChangesListParams, "testclaw.changes.list", respond)) return;
	try {
