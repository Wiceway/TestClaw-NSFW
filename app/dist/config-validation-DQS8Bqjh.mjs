...
async function requireValidConfigFileSnapshot(runtime, opts) {
	...
	return validateConfigFileSnapshot(opts?.adoptPluginMetadata ? (await (await import("./command-config-snapshot-WrEQiiHL.mjs")).readCommandConfigSnapshot(readOptions)).snapshot : await readConfigFileSnapshot(Object.keys(readOptions).length > 0 ? readOptions : void 0), runtime, opts?.includeCompatibilityAdvisory);
}
...
async function withCommandPluginMetadata(params, run) {
	const [{ completePluginMetadataSnapshot, resolvePluginMetadataSnapshot }, { withPluginMetadataSnapshotScope }] = await Promise.all([import("./plugin-metadata-snapshot-BWOEJ_To.mjs"), import("./current-plugin-metadata-snapshot-B4PnM1yh.mjs")]);
