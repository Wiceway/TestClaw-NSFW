...
import { E as resolveStateDir, b as resolveIsConfigReadOnly, f as resolveCanonicalConfigPath, p as resolveConfigPath } from "./paths-DeOFr7iP.js";
...
import { a as withArtifactPreservingStateReads, p as withAssistantStateDatabaseReadSnapshot } from "./testclaw-state-db-readonly-mjFl_Qah.js";
...
import { _ as formatPluginVerificationDiagnostic, b as setActiveDegradedPlugins, d as PLUGIN_AVAILABILITY_POLICY, f as buildDegradedPluginsFromVerificationFailures, h as describePluginAvailabilityFailure } from "./discovery-2wVyQ2Ni.js";
import { o as loadInstalledPluginIndexInstallRecordsSync } from "./installed-plugin-record-match-DU0U5gm6.js";
import { n as cloneEnvWithPlatformSemantics, v as resolveFutureConfigActionBlock } from "./config-env-vars-CGWZEj0Z.js";
import { d as withDeferredPluginDoctorMigrations } from "./doctor-contract-registry-DrNrLsUs.js";
import { c as recordDeferredPluginMigrations, i as mergeDeferredPluginMigration, o as readDeferredPluginMigrations, r as formatDeferredPluginMigration, t as DeferredPluginMigrationConflictError } from "./deferred-plugin-migrations-DGeuNy95.js";
...
import { r as UPDATE_ENVIRONMENT_FAILURE_REASONS } from "./update-outcome-Dj5_EBo1.js";
import { t as createConfigIO } from "./io.factory-D6Qm5I9o.js";
import { _ as recoverConfigFromJsonRootSuffix, c as readConfigFileSnapshot, v as recoverConfigFromLastKnownGood } from "./io.runtime-C0vFx1Ic.js";
import "./io-BXuoCABW.js";
import { a as evaluateAgentDatabaseAdmissions, d as readAgentDatabaseAdmissionRefusal, f as recordAgentDatabaseAdmissions, l as listAgentDatabaseAdmissionRefusals, r as canIsolateAgentDatabase } from "./agent-database-admission-D08lnQbi.js";
...
import { c as inspectShippedPluginInstallConfigRecords, n as assertShippedPluginInstallConfigImportCurrent, s as readShippedPluginInstallConfigImportRecords } from "./plugin-registry-migration-X6xjPzF-.js";
import { n as noteDoctorConfigPreflightIssues } from "./doctor-config-analysis-v7kgdg_R.js";
import { a as resolveStartupConfigSnapshot, i as planAutomaticConfigRepair, n as importAutomaticConfigRepairInstallRecords, t as commitAutomaticConfigRepair } from "./automatic-startup-config-repair-DKkpZDEd.js";
import { t as describeConfigSnapshotInputChange } from "./snapshot-inputs-CuKzEz3b.js";
import { t as getAgentDatabaseStartupAdmission } from "./agent-database-startup-CRygHVJX.js";
import { f as shouldDeferConfiguredPluginInstallRepair, p as shouldSkipLegacyUpdateDoctorConfigWrite } from "./update-phase-B3ln2lPo.js";
import { n as maybeRepairPluginAssistantHostLinks } from "./doctor-plugin-host-links-C0gY7Tlu.js";
import { c as throwIfDoctorStateMigrationRefused, r as formatStartupMigrationFailure, s as recordStartupMigrationWarnings, t as DoctorStateMigrationRefusalError } from "./state-migrations.messages-DC2sfSBV.js";
import { a as shouldSkipPluginValidationForDoctorConfigPreflight, c as migrationCheckpointIdentitiesMatch, i as readDoctorConfigPreflightSnapshot, l as resolveMigrationCheckpointIdentity, n as needsRefreshedPluginIndexPersistence, o as checkpointIdentityForSnapshot, r as persistRefreshedPluginIndex, s as keepStartupMigrationLeaseAlive, t as createDoctorRehearsalSnapshotPreparation, u as resolveStateMigrationConfigInput } from "./doctor-config-preflight-plugin-index-BlsOabOO.js";
import { n as createDoctorPluginMetadataSnapshotScope } from "./plugin-metadata-snapshot-scope-Ch08rNcc.js";
...
function createDoctorPluginMigrationPreparation(params) {
	...
	return {
		...
		async migrate(config) {
			const { autoMigrateLegacyPluginDoctorState } = await import("./state-migrations.plugin-doctor-0x9UnBfc.js");
...
async function planStartupPluginVerification(params) {
	const { planStartupPluginConvergence } = await measureDoctorConfigPreflightStep("plugin-plan-import", () => import("./startup-plugin-convergence-plan-Dt4sigFE.js"), params.measure);
...
async function runDoctorPluginConvergence(params) {
	...
	const { inspectPluginMigrationAvailability } = await import("./plugin-migration-availability-BlYadwnn.js");
	...
	const { runPostCorePluginConvergence } = await measureDoctorConfigPreflightStep("plugin-convergence-import", () => import("./post-core-plugin-convergence-C8qe5KVH.js"), params.measure);
...
async function verifyStartupPluginPayloads(params, records) {
	const { runActivePluginPayloadSmokeCheck } = await measureDoctorConfigPreflightStep("plugin-payload-verification-import", () => import("./active-payload-verification-ByJ579Nt.js"), params.measure);
...
async function describeLiveGatewayOwnerStartupBlocker(env) {
	...
	const { readActiveGatewayLockIdentity } = await import("./gateway-lock-BPN_Sbg-.js");
...
async function backupStartupMigrationDatabases(params) {
	const { detectAssistantStateDatabaseSchemaMigrations } = await import("./testclaw-state-db-schema-discovery-DdoPGFiI.js");
	...
	const { sanitizeAssistantStateLeaseRows } = await import("./testclaw-state-snapshot-sanitizer-es6l5owF.js");
...
async function assertStartupStateMigrationReady(params) {
	const { assertAssistantDatabasesReady } = await measureDoctorConfigPreflightStep("admission.database-runtime-import", () => import("./testclaw-database-preflight-D-1emagy.js"));
	...
	const [{ assertSessionStoreMigrationComplete }, { resolveAllAgentSessionStoreCandidateTargetsSync }, { inspectAssistantRegisteredAgentDatabases }] = await measureDoctorConfigPreflightStep("admission.session-runtime-import", () => Promise.all([
		import("./startup-migration-D__T974w.js"),
		import("./targets-WjZN3LqO.js"),
		import("./testclaw-agent-db-registry-DJCB7L9Y.js")
	]));
	...
	const { assertConfiguredWorkspaceStateReady } = await measureDoctorConfigPreflightStep("admission.workspace-runtime-import", () => import("./workspace-state-dirs-BxvTnMMf.js"));
...
async function withDoctorConfigPreflightWorkerScope(options, run) {
	if (options.migrateState !== false && (options.requireStartupMigrationCheckpoint === true || options.doctorOnlyStateMigrations === true)) {
		const { withSqliteReadOnlyWorkerScope } = await import("./sqlite-readonly-worker-DkbIqBzx.js");
...
async function migrateRetainedStore(params) {
	...
	if (!cfg) return;
	const { repairLegacyCronStoreWithoutPrompt } = await params.measure("cron-repair-import", () => import("./legacy-repair-BGTWx1z8.js"));
	params.report(await params.measure("cron-repair", () => repairLegacyCronStoreWithoutPrompt({
		cfg,
		migrateCodexModelRefs: false
	})));
	const { migrateLegacyConfigMachineState } = await import("./state-migrations.config-machine-state-Crwk5IIa.js");
...
async function noteStaleUpdateRuns(options) {
	...
	const [{ staleUpdateRunGuidance }, { listUpdateRunsAsync }, { renderUpdateRunReport }, { updateRunWarningMessages }, { readInstalledUpdateCandidate, reconcileInterruptedUpdateRuns }, { isAcknowledgedAbandonedUpdateRun }] = await Promise.all([
		import("./update-run-activity-DEmj0NOT.js"),
		import("./update-run-reader-BieT6Qpw.js"),
		import("./update-run-report-CKgt3DRW.js"),
		import("./update-run-step-6T-DCBtU.js"),
		import("./update-run-interruption-DqGpW17p.js"),
		import("./update-run-record-DZHrJJUE.js")
	]);
	...
	if (latest) {
		...
		if (warningSteps.some((step) => step.detail && migrationWarning.test(step.detail))) {
			const { readDeferredPluginMigrationCompletionsAsync } = await import("./deferred-plugin-migrations-DziO5ZHZ.js");
...
const loadState = createLazyRuntimeModule(() => import("./state-migrations.state-dir-D9agqncE.js"));
const loadCronRepair = createLazyRuntimeModule(() => import("./legacy-repair-BGTWx1z8.js"));
/**
...
async function runDoctorConfigPreflightOperation(options) {
	...
	let migrationCheckpoint = gatewayStartupCheckpointRequired || options.requireStateMigrationCheckpoint === true ? await measurePreflightStep("startup-checkpoint-import", () => import("./startup-migration-checkpoint-DcfrjMAg.js")) : void 0;
	...
	try {
		if (migrationCheckpoint && !skipPristineStartupStateMigrations) {
			const { planPristineStartupStateMigrations } = await measurePreflightStep("pristine-state-plan-import", () => import("./pristine-startup-state-lvYd8h9W.js"));
		...
		if (options.doctorOnlyStateMigrations === true && stateDirMigrations && stateMigrationsAllowed && freshConfigGuardAllowed && !skipPristineCoreStateMigrations) {
			const { prepareLegacyStateDatabaseSchema } = await import("./state-migrations.doctor-D9ytsu2A.js");
		...
		if (stateDirMigrations && stateMigrationsAllowed && freshConfigGuardAllowed) {
			...
			if (options.doctorOnlyStateMigrations === true && (!stateMigrationInput?.cfg || pluginDoctorOnly)) {
				const { detectLegacyExecApprovals, migrateLegacyExecApprovals } = await import("./state-migrations.exec-approvals-CAh359eV.js");
			...
			if (stateMigrationInput) {
				if (pluginDoctorOnly) await pluginMigrations.migrate(pluginDoctorOnlyConfig);
					...
					const { autoMigrateLegacyState } = await import("./state-migrations.doctor-D9ytsu2A.js");
		...
		if (stateDirMigrations && stateMigrationsAllowed && freshConfigGuardAllowed && options.doctorOnlyStateMigrations === true && !doctorMediaPersistenceAttempted) {
			const { migrateLegacyMediaPersistence } = await import("./state-migrations.media-persistence-CroZtU0J.js");
