import { O as walkDirectorySync, t as FsSafeError } from "./fs-safe-CZ3jhUUr.js";
...
import { f as resolveOwningPluginIdsForProviderRef, u as resolveOwningPluginIdsForModelRefs } from "./providers-BJqVIQwV.js";
...
import "./runtime-plugin-R_Tm-ora.js";
import { a as resolveCliRuntimeOwnerFingerprint, i as resolveCliRuntimeArtifactFingerprint, n as resolveCliAuthBindingFingerprint } from "./cli-auth-epoch-DY78O0DT.js";
import { a as fingerprintResolvedAuthProfileCredential, i as fingerprintOpaqueRuntimeOwner, n as fingerprintAuthProfileOwnerShape, o as fingerprintResolvedProviderAuth, r as fingerprintAwsSdkRuntimeOwner, t as fingerprintAuthProfileCredential } from "./execution-auth-binding-CnsJRKRM.js";
import { i as resolveSystemAgentConfiguredRouteFromConfig, r as projectInferenceRoute } from "./inference-route-D2dbg9hz.js";
...
const MAX_RUNTIME_ARTIFACT_FILE_BYTES = 268435456;
const MAX_RUNTIME_ARTIFACT_TOTAL_BYTES = 536870912;
...
function hashRuntimeArtifactFile(params) {
	...
	try {
		...
		if (hashed.sizeBytes !== opened.stat.size || !sameOpenedFile(opened.stat, after)) throw new Error(changedMessage);
		return {
			hash: hashed.sha256,
		...
		if (error instanceof FsSafeError && error.code === "too-large") throw new Error(changedMessage, { cause: error });
		throw error;
	} finally {
...
function fingerprintPluginRuntimeArtifact(record) {
	...
	hash.update("testclaw-plugin-runtime-artifact-v1\0");
...
/** Exact child harness artifact every verified embedded Assistant call must carry. */
function resolveSystemAgentExpectedAgentHarnessRuntimeArtifact(binding) {
	if (binding.execution.runner !== "embedded" || binding.execution.agentHarnessRuntimeOverride === "testclaw") return;
...
function systemAgentRouteIdentity(route) {
	const { runConfig: _runConfig, sourceConfig: _sourceConfig, authProfileId: _authProfileId, ...identity } = route;
	return identity;
}
async function resolveCurrentRuntimeOwnerFingerprint(params) {
	...
	if (params.authProfileId) {
		const store = (params.deps.loadAuthProfileStoreForRuntime ?? loadAuthProfileStoreForRuntime)(params.route.agentDir, {
			readOnly: true,
	...
	if (params.kind === "plugin-harness") {
		if (params.route.agentHarnessRuntimeOverride === "testclaw") return;
...
function resolveRouteHarnessOwnerPluginIds(config, route) {
	if (route.runner !== "embedded" || !route.agentHarnessRuntimeOverride || route.agentHarnessRuntimeOverride === "testclaw") return [];
...
async function resolveCurrentAuthFingerprint(params) {
	...
	let store;
	if (params.authProfileId) {
		store = (params.deps.loadAuthProfileStoreForRuntime ?? loadAuthProfileStoreForRuntime)(params.route.agentDir, {
			readOnly: true,
			migrationProvider: params.route.provider,
			allowKeychainPrompt: false,
			config: params.route.runConfig,
		...
		if (credential.type === "oauth" || params.route.runner === "embedded" && params.route.agentHarnessRuntimeOverride !== "testclaw") {
...
async function createSystemAgentVerifiedInferenceBinding(params) {
	...
	const pluginHarnessId = execution.runner === "embedded" && successfulHarnessId !== "testclaw" ? successfulHarnessId : void 0;
	if (pluginHarnessId) {
	...
	if (pluginHarnessId && resolveRouteHarnessOwnerPluginIds(params.configuredRoute.sourceConfig, execution).length === 0) throw new Error("The successful inference harness has no trusted manifest owner.");
	...
	return {
		...
		execution,
		executionFingerprint: await projectVerifiedExecutionFingerprint(params.configuredRoute.sourceConfig, execution, ownerPluginIds, deps),
		ownerPluginIds,
...
async function resolveSystemAgentVerifiedInferenceStateInternal(binding, deps = {}) {
	...
	const config = snapshot.runtimeConfig ?? snapshot.config;
	const currentRoute = await resolveSystemAgentConfiguredRouteFromConfig(config, binding.execution.agentId, deps, snapshot);
	...
	} else if (binding.execution.runner === "embedded" && binding.execution.agentHarnessRuntimeOverride !== "testclaw") {
		const harnessId = binding.execution.agentHarnessRuntimeOverride;
		const artifactId = binding.auth.runtimeArtifactId?.trim();
