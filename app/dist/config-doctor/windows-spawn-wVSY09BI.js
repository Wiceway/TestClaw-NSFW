...
function detectWindowsSpawnCommandInlineArgs(command) {
	...
	if (!INLINE_ARGUMENT_EXECUTABLES.has(executable) || isFilePath(command)) return null;
	return {
...
function resolveWindowsExecutablePath(command, env, cwd) {
	if (command.includes("/") || command.includes("\\") || path.isAbsolute(command)) return cwd && !path.isAbsolute(command) ? path.resolve(cwd, command) : command;
	const pathValue = resolveEnvironmentValue(env, "PATH", "win32") ?? resolveEnvironmentValue(process.env, "PATH", "win32") ?? "";
...
//#endregion
export { resolveWindowsSpawnProgramCandidate as a, resolveWindowsSpawnProgram as i, materializeWindowsSpawnProgram as n, resolveWindowsExecutablePath as r, detectWindowsSpawnCommandInlineArgs as t };
