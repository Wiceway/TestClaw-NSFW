...
async function loadRootHelpRenderOptionsForConfigSensitivePlugins(env = process.env) {
	const snapshot = await (await import("./config-fCohulPn.js")).readConfigFileSnapshot({
		observe: false,
	...
	if (!Boolean(env.TESTCLAW_BUNDLED_PLUGINS_DIR?.trim() || env.TESTCLAW_DISABLE_BUNDLED_PLUGINS?.trim()) && !configAffectsPluginHelp) return null;
	return {
