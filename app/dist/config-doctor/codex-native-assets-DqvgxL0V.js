...
async function collectCodexNativeAssetInfoNotes(params) {
	...
	].join(", ")}) in ${resolveCodexHome(env)} and ${resolvePersonalAgentSkillsDir(env)}; native Codex-mode agents use isolated per-agent homes and will not load them.`, "- To review or promote them: install the Codex plugin (testclaw plugins install npm:@testclaw/codex), then run testclaw migrate plan codex."].join("\n")];
}
if (process.env.VITEST || false) globalThis[Symbol.for("testclaw.codexNativeAssetsTestApi")] = { scanCodexNativeAssets };
//#endregion
