...
import { D as isGatewayMachineOutput, V as MACHINE_OUTPUT_JSON_OPTION_DESCRIPTION } from "./argv-zmtAhw2-.js";
...
import { o as resolveGatewayRpcOptions, r as callGatewayFromCliWithTransport, s as resolveGatewayRpcOptionsWithLocalPort, t as addGatewayClientOptions } from "./gateway-rpc-D4qc8nHD.js";
...
import { t as addGatewayRunCommand } from "./run-command-Bl47S36e.js";
...
function addGatewayRestartHandoffCommands(gateway) {
	...
	restartHandoff.command("consume").description("Atomically consume a gateway restart handoff").helpOption(false).allowUnknownOption().allowExcessArguments().option("--expected-pid [pid]", "PID of the exited gateway process", collectExpectedPid, []).option("--json", MACHINE_OUTPUT_JSON_OPTION_DESCRIPTION).action(async (opts, command) => {
		...
		try {
			const { consumeGatewayRestartHandoffSync } = await import("./restart-handoff-D9hUHSLb.js");
...
const loadConfigModule = createLazyPromise(() => import("./read-best-effort-config.runtime-DlWgHxYc.js"));
const loadGatewayStatusModule = createLazyPromise(() => import("./gateway-status-YVUTLqse.js"));
const loadGatewayHealthModule = createLazyPromise(() => import("./health-DUYyJJSA.js"));
const loadBonjourDiscoveryModule = createLazyPromise(() => import("./bonjour-discovery-DzOfLvGo.js"));
const loadWideAreaDnsModule = createLazyPromise(() => import("./widearea-dns-DQAiQTrS.js"));
const loadHealthStyleModule = createLazyPromise(() => import("./health-style-C62tqMJP.js"));
const loadUsageFormatModule = createLazyPromise(() => import("./usage-format-B4dlftXg.js"));
const loadStabilityBundleModule = createLazyPromise(() => import("./diagnostic-stability-bundle-D0b5I0It.js"));
const loadSupportExportModule = createLazyPromise(() => import("./diagnostic-support-export-BEyWB3OD.js"));
const loadDaemonStatusGatherModule = createLazyPromise(() => import("./status.gather-DHUPKp4x.js"));
...
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayAuthErrorJson, formatGatewayClientRequestErrorJson, formatGatewayTransportErrorJson } = await import("./call-BUB0AMHV.js");
...
function registerGatewayCli(program, deps = {}) {
	...
	gateway.command("auth-token").description("Reveal the configured shared Gateway token").option("--show", "Print the token to an interactive terminal", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			...
			const { gatewayAuthTokenCommand } = await import("./gateway-auth-token-uLl2e3dD.js");
	...
	gatewayCallOpts(gateway.command("stability").description("Fetch payload-free Gateway stability diagnostics").option("--limit <limit>", "Maximum number of recent events", "25").option("--type <type>", "Filter by diagnostic event type").option("--since-seq <seq>", "Only include events after this sequence").option("--bundle [path]", "Read a persisted stability bundle instead of calling Gateway; pass \"latest\" for newest").option("--export", "Write a shareable support diagnostics export", false).option("--output <path>", "Diagnostics export output .zip path").action(async (opts, command) => {
		await runGatewayCommand(async () => {
			const { normalizeDiagnosticStabilityQuery, selectDiagnosticStabilitySnapshot } = await import("./diagnostic-stability-BKnSLeNG.js");
	...
	gateway.command("discover").description("Discover gateways via Bonjour (local + wide-area if configured)").option("--timeout <ms>", "Per-command timeout in ms", "2000").option("--json", "Output JSON", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			const [{ readSourceConfigBestEffort }, { discoverGatewayBeacons, resolveGatewayDiscoveryEndpoint }, { resolveWideAreaDiscoveryDomain }, { dedupeBeacons, parseDiscoverTimeoutMs, renderBeaconLines }, { withProgress }] = await Promise.all([
				...
				import("./discover-Cz9c7OUP.js"),
				import("./progress-_TAMuiGZ.js")
			]);
