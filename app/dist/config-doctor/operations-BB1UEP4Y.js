...
import { r as SYSTEM_AGENT_AUDIT_STORE_LABEL } from "./audit-DQGfckJV.js";
import { f as resolveSystemAgentConfigSchema, s as validateSystemAgentPluginInstallSpec, u as redactSystemAgentConfig } from "./operations-parse-CJ16l1J7.js";
import { _ as runConfigSetOperation, a as executeSetDefaultModel, c as formatConfigValidationLine, d as isPluginBackingDefaultInferenceRoute, f as loadOverviewForOperation, g as resolveTuiAgentId, h as resolveChannelSetupState, i as createNoExitRuntime, l as formatGatewayStatusLine, m as readConfigValueAtPath, o as executeSetup, p as readConfigFileSnapshotLazy, r as applyPersistentOperation, s as formatChannelDocsUrl, t as CONFIG_GET_OUTPUT_MAX_CHARS, u as getRegularAgentSetupNotice, v as runGatewayLifecycle } from "./operations-execution-helpers-pCkBYyZ2.js";
...
async function executeSystemAgentOperation(operation, runtime, opts = {}) {
	switch (operation.kind) {
		...
		case "config-schema": {
			const { lookupConfigSchema } = await import("./schema-C3D4KZx0.js");
		...
		case "plugin-activate-artifact": {
			const { executePluginArtifactActivation } = await import("./plugin-artifact-BrZL-NPB.js");
		...
		case "create-agent":
			...
			return await applyPersistentOperation({
				...
				opts,
				run: async (ctx) => {
					const createAgentForOperation = ctx.deps?.createAgent ?? (await import("./agent-create-QI6Q7C3c.js")).createAgent;
					const { createAgentIdentityConfig } = await import("./identity-file-DBkkXCT0.js");
		...
		case "create-team": return await applyPersistentOperation({
			...
			opts,
			run: async (ctx) => {
				const { createAgentTeam } = await import("./agent-team-Cx9hoF9K.js");
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-KfEHwz_Q.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-B_lWWoXM.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-bWd0Ez5E.js")).runTui)({
