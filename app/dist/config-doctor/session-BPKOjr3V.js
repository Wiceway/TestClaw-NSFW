...
import { a as classifySessionKeyShape, c as resolveAgentIdFromSessionKey, k as parseAgentSessionKey, o as isUnscopedSessionKeySentinel } from "./session-key-AvQIavYt.js";
import { l as resolveSessionStorePathCore } from "./paths-ViQaz2td.js";
import { n as resolvePersistedSessionStoreOwnerForKey, t as resolvePersistedSessionStoreOwner } from "./session-store-owner-cXJW6Bip.js";
...
import { i as resolveExplicitAgentSessionKey, n as canonicalizeMainSessionAlias } from "./main-session-DzZK9knv.js";
import { a as loadExactSessionEntryReadOnly, c as isInternalSessionEffectsKey } from "./session-accessor.sqlite-exact-read-CFY3B1wI.js";
...
import { s as hasTerminalMainSessionTranscriptNewerThanRegistrySync, u as resolveSessionLifecycleTimestamps } from "./lifecycle-DpcRUIUy.js";
...
function collectSessionIdMatchesForRequest(opts) {
	...
	const addMatches = (candidateEntries, candidateStorePath, candidateAgentId, options) => {
		const candidateStoreTarget = resolveUnsuffixedSqliteTargetFromSessionStorePath(candidateStorePath);
		for (const { sessionKey: candidateKey, entry: candidateEntry } of candidateEntries) {
			...
			const legacyUnscopedOwner = isLegacyUnscopedKey ? persistedStoreOwner.kind === "configured" ? persistedStoreOwner.agentId : persistedStoreOwner.kind === "retired" ? void 0 : (!candidateStoreTarget.shared ? scopedCandidateAgentId : void 0) ?? pathOwnedAgentId ?? (opts.searchOtherAgentStores ? void 0 : scopedCandidateAgentId) ?? compatibilityAgentId : void 0;
	...
	for (const agentId of configuredAgentIds) {
		...
		addMatches(loadCommandSessionEntries({
			agentId,
			storePath: candidateStorePath
...
function resolveStoredSessionKeyForSessionId(opts) {
	...
	const selection = resolveSessionIdMatchSelection(selectionMatches.map(({ sessionKey, entry }) => [sessionKey, entry]), sessionId);
	if (selection.kind !== "selected") return {
	...
	return {
		...
		sessionKey,
		sessionEntry: structuredClone(selectionMatches.find((match) => match.sessionKey === sessionKey)?.entry),
		storePath
	};
}
function resolveSessionKeyForRequestInternal(opts) {
	...
	const storeSessionKey = sessionKey ? normalizeStoreSessionKey(sessionKey) : void 0;
	const sessionEntry = storeSessionKey && !isInternalSessionEffectsKey(storeSessionKey) ? loadExactSessionEntryReadOnly({
		agentId: storeAgentId,
		storePath,
		sessionKey: storeSessionKey
	})?.entry : void 0;
	if (requestedSessionId && (!explicitSessionKey || unownedBareSessionKey) && (!sessionKey || sessionEntry?.sessionId !== requestedSessionId)) {
		const { candidates, ownerConflict } = collectSessionIdMatchesForRequest({
			cfg: opts.cfg,
			sessionEntries: loadCommandSessionEntries({
				storePath,
				agentId: storeAgentId
			}),
			storePath,
...
function resolveSession(opts) {
	const sessionCfg = opts.cfg.session;
	const { agentId: resolvedAgentId, sessionKey, sessionEntry, storePath } = resolveSessionKeyForRequestCore({
		cfg: opts.cfg,
		to: opts.to,
