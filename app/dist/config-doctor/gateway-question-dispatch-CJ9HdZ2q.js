...
function resolveAgentQuestionGatewayCall(dispatcher) {
	...
	return async (...args) => {
		...
		if (!embeddedBroker && isEmbeddedMode()) {
			const { getEmbeddedQuestionBroker } = await import("./embedded-question-broker-CPmfs4__.js");
		...
		const { callGatewayTool } = await import("./gateway-question-dispatch.runtime-BbmK3HpF.js");
