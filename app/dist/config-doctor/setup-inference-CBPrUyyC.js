...
import { a as createPluginCache } from "./plugin-cache-CsUjLuei.js";
import { o as resolveUserPath } from "./home-dir-DjuHbd5R.js";
import "./utils-BfoJTy8l.js";
import { c as normalizePluginTargetConfig } from "./config-state-D4j-tzq3.js";
import { a as resolveAgentDir, p as resolveAmbientOwnerAgentId } from "./agent-scope-config-BEuqweC1.js";
...
import { a as readNativeSessionCatalogPreference, n as hasLegacyNativeSessionCatalogDefault, o as shippedNativeSessionCatalogs, t as applyNativeSessionCatalogPreference } from "./native-session-catalog-config-CsOxp30J.js";
...
import { t as readCodexCliActiveApiKey } from "./cli-credentials-DiyVxmsJ.js";
...
import { n as resolveConfiguredPrimaryModelForAgent, r as resolveConfiguredSetupModelForAgent } from "./utility-model-CCzqUQSQ.js";
import { i as resolveManifestProviderAuthChoices, o as isProviderAuthChoicePlatformSupported } from "./provider-auth-choices-jz5pJx2z.js";
...
import { o as stripPendingPluginInstallRecords } from "./install-record-commit-bx-_yGLM.js";
import { n as resolveProviderInstallCatalogEntries } from "./provider-install-catalog-D4uH-cn1.js";
import { C as validateSetupModelTarget, S as validateSetupInferenceOwnerEvidence, a as SetupInferenceCancelledError, b as toProviderAutoSetupKind, d as parseSavedAuthSetupProfileId, f as redactSetupInferenceError, g as resolveSetupModel, h as resolveSetupInferenceWorkspace, i as SetupInferenceActivationUnavailableError, l as parseInferenceRef, o as SetupInferenceOwnerDriftError, p as resolveCandidatePresentation, r as SetupInferenceActivationIndeterminateError, s as invalidSetupConfigError, u as parseProviderAutoSetupChoiceId, v as setupInferenceLog, x as toSavedAuthSetupKind, y as throwIfSetupInferenceCancelled } from "./setup-inference-core-BwVzPhCm.js";
import { i as setupConfigPatchConflicts, n as commitSetupInferenceActivation, t as captureSetupInferenceFileUndo } from "./setup-inference-transition-Coaw8XfF.js";
import { a as listSetupInferenceManualProviders, c as supportsSetupTextInference, i as listSetupInferenceInstallOptions, n as listSetupInferenceAuthOptions, o as listSetupInferencePrepareOptions, r as listSetupInferenceEnableOptions, t as choiceMatchesCredential } from "./setup-inference-auth-options-7rcn86-s.js";
import { a as OPENAI_API_DEFAULT_MODEL_REF, i as GEMINI_CLI_DEFAULT_MODEL_REF, n as CLAUDE_CLI_DEFAULT_MODEL_REF, r as CODEX_APP_SERVER_DEFAULT_MODEL_REF, t as ANTHROPIC_API_DEFAULT_MODEL_REF } from "./onboard-inference-ambient-DV_evpy7.js";
import "./onboard-inference-HTev7S0D.js";
import { n as createQuickstartNotePrompter } from "./setup-apply-CApOVL5Y.js";
import { n as createSystemAgentModelSelectionUpdater } from "./setup-model-selection-BQrQo8Ip.js";
import { a as stageProviderAutoCandidate, i as stageProviderAuthCandidate, n as saveSetupCredential, o as stageSavedAuthCandidate } from "./setup-inference-credentials-UvqdF3uI.js";
import { a as revalidateStableSetupInferenceOwner, d as withPreparedSetupCredentialAccess, l as activatePreparedSetupCredential, o as runSetupInferenceTurn, r as loadSetupInferencePluginGeneration } from "./setup-inference-turn-BS8dJ0UK.js";
import { t as captureSystemAgentOwnerPluginArtifacts } from "./verified-inference-CVC2Uoay.js";
...
async function listSavedSetupInferenceCandidates(params) {
	const { withSetupProviderAuthMethod } = await import("./setup-provider-method-CkBi8JDP.js");
...
async function prepareSetupInferenceOptions(deps, agentId) {
	...
	if (utilitySetting.kind === "explicit") {
		const { resolveSimpleCompletionSelectionForAgent } = await import("./simple-completion-runtime-DLoaKZHH.js");
...
async function discoverSetupInference({ cfg, targetAgentId, authChoices, detectionRequiredProviders, manual }, deps, signal, onPartial) {
	...
	onPartial(partial);
	const detected = await (deps.detectInferenceBackends ?? (await import("./onboard-inference-CMC__j45.js")).detectInferenceBackends)({
		config: cfg,
		agentId: targetAgentId
	});
	signal.throwIfAborted();
	...
	if (discoveryChoices.length > 0) {
		const { probeSetupProviderChoices } = await import("./provider-setup-availability-CjANv0wo.js");
...
async function stageCodexCandidate(ctx, modelRef) {
	return await withPluginLifecycleLease({ signal: ctx.params.signal }, async () => {
		...
		const ensured = await (ctx.deps.ensureCodexRuntimePlugin ?? (await import("./codex-runtime-plugin-install-nFcqz5FI.js")).ensureCodexRuntimePluginForModelSelection)({
			cfg: enabled.config,
			model: modelRef,
		...
		if (install?.source === "npm" && install.installPath) {
			if (!await (ctx.deps.markRetainedManagedNpmInstall ?? (await import("./managed-npm-retention-CydNMRqY.js")).markRetainedManagedNpmInstall)({
...
async function activateCandidate(params) {
	...
	const snapshot = await (deps.readConfigFileSnapshot ?? (await import("./config-fCohulPn.js")).readConfigFileSnapshot)();
	if (snapshot.exists && !snapshot.valid) throw new Error(invalidSetupConfigError(snapshot));
	const cfg = snapshot.runtimeConfig ?? snapshot.config;
...
async function verifyAndActivateCandidate(ctx, staged, failure, runtimeCredential) {
	try {
		...
		const readSnapshot = deps.readConfigFileSnapshot ?? (await import("./config-fCohulPn.js")).readConfigFileSnapshot;
		...
		if (!isDeepStrictEqual(sourceCandidate, source) || savedCredential?.setup) {
			const configTarget = {
				...
				write: async (_candidate, { writeOptions, captureUndo }) => {
					return (await (deps.transformConfigWithPendingPluginInstalls ?? (await import("./install-record-commit-Cirt6yPQ.js")).transformConfigWithPendingPluginInstalls)({
						base: "source",
						writeOptions,
