...
async function measureGatewayBootstrapStep(name, run, metrics) {
	...
	try {
		...
		const { formatConsoleDiagnosticLine } = await import("./json-console-line-By3D7X1N.js");
...
function createGatewayDispatchStartupTrace(argv, source) {
	...
	let pendingMessages = [];
	const timelineModule = hasDiagnosticsTimelinePath(process.env) ? import("./diagnostics-timeline-C2iWaeMY.js").catch(() => null) : null;
