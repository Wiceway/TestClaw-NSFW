...
async function resolveExecBaseConfig(opts) {
	...
	if (opts.isolated || opts.authEnvOnly === true) {
		const { migratePersistedImplicitMainRoster } = await import("./legacy.roster-D0l9cB92.js");
		const { coerceConfig } = await import("./io.read-helpers-BnmjcW-D.js");
		return coerceConfig(migratePersistedImplicitMainRoster({}).config);
	}
	const { createConfigIO, getRuntimeConfig } = await import("./io-CYktO81x.js");
