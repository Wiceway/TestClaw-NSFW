...
import { c as serializeWorkerWorkspaceManifest, o as parseWorkerWorkspaceManifest } from "./workspace-manifest-DTqIPCiX.js";
import { c as pruneWorkspaceHashMemo, g as withoutWorkspaceHashContext, h as withWorkspaceHashMemo, p as withWorkerWorkspaceHashMemo } from "./workspace-hash-memo-Bto8rdGB.js";
import { i as parseChangedWorkspaceResult, r as manifestNodes, t as changedPaths } from "./workspace-manifest-comparison-BrsPluAH.js";
...
async function executeWorkspaceManifestComputation(command, assertBeforeMutation) {
	switch (command.type) {
		case "workspace.manifest.nodes": {
			const { localWorkspaceNode } = await import("./workspace-reconcile-fs-PW3EZBv4.js");
		...
		case "workspace.manifest.staged": {
			const { loadStagedWorkerWorkspace } = await import("./workspace-result-inventory.runtime-C4BTeZfu.js");
			return await loadStagedWorkerWorkspace(command.input.root, command.input.ref);
		}
		case "workspace.manifest.stage-input": {
			const { buildWorkspaceStageInput } = await import("./workspace-result-preparation.runtime-DpMubVuq.js");
			return await buildWorkspaceStageInput(command.input, assertBeforeMutation);
		}
		case "workspace.manifest.tree-input": {
			const { buildWorkspaceTreeInput } = await import("./workspace-result-preparation.runtime-DpMubVuq.js");
			return await buildWorkspaceTreeInput(decodeManifestValue(command), assertBeforeMutation);
		}
		case "workspace.manifest.entries": {
			const { readStagedWorkerWorkspaceEntries } = await import("./workspace-result-inventory.runtime-C4BTeZfu.js");
			return ownedWorkerBytes(await readStagedWorkerWorkspaceEntries(command.input));
		}
		case "workspace.manifest.capture": {
			const { readActualWorkspaceManifestImpl } = await import("./workspace-actual-manifest-DypBEspA.js");
			const input = decodeManifestValue(command);
			return await withHashes(input.hashes, async () => {
		...
		case "workspace.manifest.file": {
			const { readWorkspaceFileSnapshotWithLimit } = await import("./workspace-actual-manifest-DypBEspA.js");
			const input = decodeManifestValue(command);
		...
		case "workspace.reconcile.preflight": {
			const { preflightWorkspaceApplyImpl } = await import("./workspace-reconcile-preflight-CDdFZAfu.js");
			const input = decodeManifestValue(command);
