...
import { n as APPROVALS_SCOPE, t as ADMIN_SCOPE } from "./operator-scopes-D-CL26h0.js";
import { o as readBestEffortConfig } from "./io.runtime-C0vFx1Ic.js";
import "./config-CiBXBfE2.js";
import { n as isWellFormedApprovalId } from "./approval-id-BTRnO3t1.js";
import "./method-scopes-D0hbLMo0.js";
import { a as mergeExecApprovalsSocketDefaults } from "./exec-approvals-config-BN4b4XLr.js";
import { n as redactExecApprovals, t as normalizeExecApprovals } from "./exec-approvals-9hAP-z4b.js";
import { s as readExecApprovalsSnapshot, u as updateExecApprovals } from "./exec-approvals-store-BSrG9R8i.js";
import { n as formatTimeAgo } from "./format-relative-DYcw7YLb.js";
import { t as formatDocsLink } from "./links-B_2WKb2T.js";
import { l as rethrowExpectedCliError } from "./failure-output-B62fEQHE.js";
...
import { a as nodesCallOpts, u as resolveCliNodeId } from "./rpc-4Eol7QH0.js";
import { n as collectExecPolicyScopeSnapshots, t as SESSION_EXEC_OVERRIDES_NOTE } from "./exec-approvals-effective-5rHTvaqa.js";
import fs from "node:fs/promises";
import JSON5 from "json5";
//#region src/cli/exec-approvals-cli.ts
const APPROVALS_GET_DEFAULT_TIMEOUT_MS = 6e4;
const EXEC_APPROVALS_STDIN_MAX_BYTES = 1048576;
...
async function saveSnapshotTargeted(params) {
	...
	if ("native" in params) {
		...
		if (!params.opts.json) defaultRuntime.log(theme.muted("Writing local approvals."));
...
function shortenPendingApprovalSummary(value) {
	...
	return `${truncateUtf16Safe(value, 93)}...`;
}
...
async function loadPendingApprovals(opts) {
	...
	const [exec, plugin, systemAgent] = await Promise.all([
		...
		listCall("testclaw.approval.list")
	]);
	return [
...
function describeGrantState(grant, nowMs) {
	if (grant.revokedAtMs !== null) return `revoked${grant.revokedBy ? ` by ${escapeApprovalTextForTerminal(grant.revokedBy)}` : ""}`;
	if (grant.expiresAtMs !== null && grant.expiresAtMs <= nowMs) return "expired";
	if (grant.expiresAtMs !== null) return `expires in ${Math.max(1, Math.ceil((grant.expiresAtMs - nowMs) / 864e5))}d`;
...
function renderStandingGrants(grants) {
	...
	defaultRuntime.log(theme.muted("Revoke with: testclaw approvals grants revoke <grant-id>"));
}
...
async function resolvePendingApproval(idInput, decisionInput, opts) {
	...
	if (opts.expiresInDays !== void 0 && (expiresInDays === void 0 || expiresInDays > 3650)) exitWithError("--expires-in-days must be a whole number of days between 1 and 3650.");
	if (expiresInDays !== void 0 && decision !== "allow-always") exitWithError("--expires-in-days only applies to allow-always.");
	const result = await callGatewayFromCli("approval.resolve", opts, {
		id,
		kind: current.presentation.kind,
		decision,
		...expiresInDays !== void 0 ? { grantExpiresInDays: expiresInDays } : {}
	}, approvalCallOptions);
...
function buildEffectivePolicyReport(params) {
	...
	if (!params.approvals) return {
		scopes: [],
		note: params.nativePolicy ? "This node enforces a host-native exec policy; Assistant approvals-file policy math does not apply." : "Host approvals policy unavailable."
	};
...
function renderApprovalsSnapshot(snapshot, targetLabel) {
	...
	const mcpToolRows = Object.entries(agents).flatMap(([agentId, agent]) => (agent.mcpTools ?? []).map((grant) => ({
		Agent: agentId,
		Server: grant.server,
		Tool: grant.tool,
	...
	const summaryRows = [
		...
		{
			Field: "MCP tool grants",
			Value: String(mcpToolRows.length)
		}
	];
	...
	} else defaultRuntime.log(muted("No allowlist entries."));
	if (mcpToolRows.length === 0) return;
	defaultRuntime.log("");
...
function isEmptyAgent(agent) {
	const allowlist = Array.isArray(agent.allowlist) ? agent.allowlist : [];
	return !agent.security && !agent.ask && !agent.askFallback && agent.autoAllowSkills === void 0 && !agent.mcpTools?.length && allowlist.length === 0;
}
...
async function runAllowlistMutation(pattern, opts, mutate) {
	try {
		...
		if (!await mutate({
			...
			if (opts.json) defaultRuntime.writeJson(redactExecApprovals(context.snapshot), 0);
			return;
		}
...
function registerExecApprovalsCli(program) {
	...
	const approvals = program.command("approvals").alias("exec-approvals").description("Manage approval policy and pending requests").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.testclaw.ai/cli/approvals")}\n`);
	const pendingCmd = approvals.command("pending").description("List pending exec, plugin, and system-agent approvals").action(async (opts) => {
		try {
	...
	const resolveCmd = approvals.command("resolve <id> <decision>").description("Resolve a pending approval").option("--reason <text>", "Add a local note to the CLI confirmation").option("--expires-in-days <days>", "Allow-always on an automation approval: freeze this grant lifetime instead of the configured default").action(async (id, decision, opts) => {
		try {
	...
	const grants = approvals.command("grants").description("Standing grants minted by allow-always on automation approvals");
	const grantsListCmd = grants.command("list").description("List standing grants, newest first").option("--limit <n>", "Maximum rows to return (default 200)").action(async (opts) => {
		try {
			const limit = parseStrictPositiveInteger(opts.limit);
			if (opts.limit !== void 0 && limit === void 0) exitWithError("--limit must be a positive integer.");
			const result = await callGatewayFromCli("exec.approval.grants.list", opts, limit !== void 0 ? { limit } : {});
			if (opts.json) {
	...
	const grantsRevokeCmd = grants.command("revoke <grantId>").description("Revoke a standing grant; the next occurrence prompts again").action(async (grantId, opts) => {
		try {
			const result = await callGatewayFromCli("exec.approval.grants.revoke", opts, { grantId });
			if (opts.json) {
				defaultRuntime.writeJson(result, 0);
				return;
			}
			if (result.outcome === "revoked") defaultRuntime.log(`Grant ${grantId} revoked. The next occurrence prompts again.`);
			else if (result.outcome === "already-revoked") defaultRuntime.log(`Grant ${grantId} was already revoked.`);
			else exitWithError(`Grant ${grantId} not found.`);
		} catch (err) {
			failApprovalsCommand(err, opts);
		}
	});
	...
	const allowlist = approvals.command("allowlist").description("Edit the per-agent allowlist").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatExample("testclaw approvals allowlist add \"~/Projects/**/bin/rg\"", "Allowlist a local binary pattern for the main agent.")}\n${formatExample("testclaw approvals allowlist add --agent main --node <id|name|ip> \"/usr/bin/uptime\"", "Allowlist on a specific node/agent.")}\n${formatExample("testclaw approvals allowlist add --agent \"*\" \"/usr/bin/uname\"", "Allowlist for all agents (wildcard).")}\n${formatExample("testclaw approvals allowlist remove \"~/Projects/**/bin/rg\"", "Remove an allowlist pattern.")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.testclaw.ai/cli/approvals")}\n`);
