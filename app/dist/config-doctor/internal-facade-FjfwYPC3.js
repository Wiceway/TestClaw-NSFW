...
import { a as waitForGatewayDispatch, i as unwrapGatewayMethodDispatchResponse, r as throwIfGatewayDispatchAborted } from "./server-in-process-dispatch-DVz377la.js";
import { t as abortChatRunById } from "./chat-abort-DEpgr_1N.js";
import { r as validateGatewayMethodParams } from "./validation-BZLpfukT.js";
import { i as runWithGatewayRequestEnvelope, n as createRequestGatewayMethodRegistry, t as authorizeGatewayRequestPreDispatch } from "./server-methods-DguCePds.js";
				...
				try {
					return await runWithGatewayRequestEnvelope(method, options.client, async () => {
						const [{ prepareAgentRequestPreflight }, { createAgentTurnService }, { captureAgentTurnPrincipal, resolveAgentTurnRunObserver }] = await Promise.all([
							import("./agent-request-preflight-C8w0HsXL.js"),
							import("./agent-turn-service-DvXubZPk.js"),
							import("./principal-Bm6vVUS2.js")
						]);
						throwIfGatewayDispatchAborted(method, dispatchOptions.signal);
						entry?.assertOpen();
						options.assertContextCurrent?.();
	...
	const wait = async (params, timeoutMs, signal, onSignalAbort) => {
		...
		try {
			...
			const result = context.trackExecution(async () => {
				...
				try {
					return await runWithGatewayRequestEnvelope(method, options.client, async () => {
						const { createAgentTurnService } = await import("./agent-turn-service-DvXubZPk.js");
						throwIfGatewayDispatchAborted(method, signal);
						entry?.assertOpen();
						options.assertContextCurrent?.();
						entry?.release();
