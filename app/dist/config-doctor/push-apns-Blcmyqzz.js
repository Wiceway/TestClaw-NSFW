...
import { c as readSecretFile } from "./fs-safe-advanced-CBSOsiER.js";
import { t as formatErrorMessage } from "./errors-cp9Var1Z.js";
import { a as getApnsResponseBodyCaptureText, i as createApnsResponseBodyCapture, n as appendApnsResponseBodyCapture, r as connectApnsHttp2Session, t as APNS_HTTP2_CANCEL_CODE } from "./push-apns-http2-Bvt1NbMd.js";
import { a as isValidApnsTopic, d as normalizeApnsTopic, h as sendApnsRelayPush, r as isLikelyApnsToken, u as normalizeApnsToken } from "./push-apns-store-DV_aAo_z.js";
import { createHash, createPrivateKey, sign } from "node:crypto";
//#region src/infra/push-apns-auth.ts
const APNS_JWT_TTL_MS = 3e6;
let cachedJwt = null;
...
async function resolveApnsAuthConfigFromEnv(env = process.env) {
	const teamId = normalizeNonEmptyString(env.TESTCLAW_APNS_TEAM_ID);
	const keyId = normalizeNonEmptyString(env.TESTCLAW_APNS_KEY_ID);
	if (!teamId || !keyId) return {
		ok: false,
		error: "APNs auth missing: set TESTCLAW_APNS_TEAM_ID and TESTCLAW_APNS_KEY_ID"
	};
	const inlineKeyRaw = normalizeNonEmptyString(env.TESTCLAW_APNS_PRIVATE_KEY_P8) ?? normalizeNonEmptyString(env.TESTCLAW_APNS_PRIVATE_KEY);
	...
	const keyPath = normalizeNonEmptyString(env.TESTCLAW_APNS_PRIVATE_KEY_PATH);
	if (!keyPath) return {
		ok: false,
		error: "APNs private key missing: set TESTCLAW_APNS_PRIVATE_KEY_P8 or TESTCLAW_APNS_PRIVATE_KEY_PATH"
	};
	try {
		return {
			ok: true,
		...
		return {
			ok: false,
			error: `failed reading TESTCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${formatErrorMessage(err)}`
		};
	}
}
//#endregion
//#region src/infra/push-apns-payloads.ts
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Assistant to review this request.";
...
function resolvePluginApprovalAlertBody(description) {
	...
	return `${truncateUtf16Safe(body, 255).trimEnd()}…`;
}
...
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "testclaw.exec-approval";
const PLUGIN_APPROVAL_NOTIFICATION_CATEGORY = "testclaw.plugin-approval";
...
async function sendApnsAlert(params) {
	return await sendApnsPush({
		transport: params,
		payload: createApnsAlertPayload({
			nodeId: params.nodeId,
			title: params.title,
			body: params.body
		}),
...
async function sendApnsBackgroundWake(params) {
	return await sendApnsPush({
		transport: params,
		payload: createApnsBackgroundPayload({
			nodeId: params.nodeId,
...
async function sendApnsPush(params, controls) {
