...
async function readExactStateRequest(filename) {
	...
	const { exactStateRetirementSchema } = await import("./snapshot-exact-state-contract-CN5o7muo.js");
...
function registerWorktreesCli(program) {
	...
	worktrees.command("list").description("List active and restorable managed worktrees").option("--json", "Output JSON", false).action(async (opts) => {
		const { managedWorktrees } = await import("./service-vKy15yCE.js");
	...
	worktrees.command("gc").description("Run managed worktree cleanup now").option("--json", "Output JSON", false).action(async (opts) => {
		const { formatWorktreeGcResult } = await import("./gc-result-DcU_k5SL.js");
		const { createManagedWorktreeOwnerPolicy } = await import("./owner-protection-DN6YXXCV.js");
		const { managedWorktrees, resolveWorktreeCleanupLimits } = await import("./service-vKy15yCE.js");
		...
		if (result.outcome === "partial") {
			const { exitCliAfterOutput } = await import("./one-shot-exit-B9FLk8nI.js");
