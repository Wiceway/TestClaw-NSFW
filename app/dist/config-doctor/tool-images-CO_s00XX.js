...
import { c as isImageProcessorUnavailableError, f as resizeToJpeg, n as buildImageResizeSideGrid, o as getImageMetadata, t as IMAGE_REDUCE_QUALITY_STEPS, u as readImageMetadataFromHeader } from "./image-ops-BTYtooXp.js";
//#region src/agents/image-sanitization.ts
const DEFAULT_IMAGE_MAX_DIMENSION_PX = 1200;
const DEFAULT_IMAGE_MAX_BYTES = 5242880;
...
const MAX_IMAGE_INPUT_BYTES = 10485760;
...
async function resizeImageBase64IfNeeded(params) {
	...
	const sourcePixels = typeof width === "number" && typeof height === "number" ? `${width}x${height}px` : "unknown";
	const sourceWithFile = params.fileName ? `${params.fileName} ${sourcePixels}` : sourcePixels;
	log.warn(`Image resize failed to fit limits: ${sourceWithFile} best=${formatBytesShort(bestSize)} limit=${formatBytesShort(params.maxBytes)}`, {
		label: params.label,
		fileName: params.fileName,
		sourceMimeType: params.mimeType,
		sourceWidth: width,
	...
	throw new Error(`Image could not be reduced below ${formatBytesShort(params.maxBytes)} (got ${formatBytesShort(bestSize)})`);
}
...
//#endregion
export { resolveImageSanitizationLimits as a, DEFAULT_IMAGE_MAX_DIMENSION_PX as i, sanitizeImageBlocks as n, sanitizeToolResultImages as r, sanitizeContentBlocksImages as t };
