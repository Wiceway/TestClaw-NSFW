...
import "./input-provenance-DGaz_sh7.js";
import { n as splitMediaOutput } from "./parse-output-DwFdnEGG.js";
import { r as stripSuppressedControlReplyToken } from "./control-reply-text-DBCNOdDS.js";
import { t as escapeHtml } from "./html-escape-BMD_QFeA.js";
import { t as MarkdownItCallable } from "./markdown-it-Dciw_JAI.js";
...
//#region src/gateway/control-ui-public-session-render.ts
const MAX_MESSAGES = 100;
const MAX_MESSAGE_CHARS = 32768;
const MAX_DOCUMENT_CHARS = 262144;
...
function publicMessageText(message) {
	...
	if (!entry || entry.role !== "user" && entry.role !== "assistant" || entry.display === false || entry.customType !== void 0 || entry.senderSession !== void 0 || entry.toolCallId !== void 0 || entry.tool_call_id !== void 0) return;
	const provenance = asOptionalRecord(entry.provenance);
	if (entry.provenance !== void 0 && provenance?.kind !== "external_user") return;
	let text;
	if (entry.role === "assistant") {
		if (entry.phase !== void 0 && entry.phase !== "final_answer") return;
		text = extractAssistantPhaseText(entry);
	} else if (typeof entry.content === "string") text = entry.content;
	else if (Array.isArray(entry.content)) text = entry.content.flatMap((value) => {
		const block = asOptionalRecord(value);
		return (block?.type === "text" || block?.type === "input_text") && typeof block.text === "string" ? [block.text] : [];
	}).join("\n\n");
	else if (typeof entry.text === "string") text = entry.text;
	if (!text || text.includes("[Inter-session message]")) return;
	text = entry.role === "user" ? stripUserEnvelopeForDisplay(text) : stripInternalMetadataForDisplay(text);
	if (entry.role === "assistant") text = sanitizeAssistantVisibleTextWithProfile(text, "history", true);
	const roleContent = {
		role: entry.role,
		content: text
	};
	if (isHeartbeatUserMessage(roleContent, HEARTBEAT_PROMPT) || isHeartbeatOkResponse(roleContent)) return;
	if (entry.role === "assistant") text = stripSuppressedControlReplyToken(text);
	text = splitMediaOutput(text, { extractAudioDirectives: false }).text;
...
function renderPublicSessionDocument(params) {
	const isLatest = params.isLatest !== false;
	const title = escapeHtml(redactToolPayloadText(stripInternalMetadataForDisplay(params.title)).slice(0, 200).trim() || "Shared conversation");
	let truncated = params.truncated || params.messages.length > MAX_MESSAGES;
	let remaining = MAX_DOCUMENT_CHARS;
	const rows = [];
	for (const value of params.messages.slice(-100).toReversed()) {
		...
		rows.push(`<article class="message ${message.role}" aria-label="${message.role === "user" ? "User" : "Assistant"} message"><h2>${message.role === "user" ? "User" : "Assistant"}</h2><div class="content">${markdown.render(text)}${clipped ? "<p class=\"omitted\">Message shortened for this public view.</p>" : ""}</div></article>`);
	}
	const description = "A public, read-only Assistant conversation. No login required.";
	const canonicalMetadata = params.canonicalUrl ? `<link rel="canonical" href="${escapeHtml(params.canonicalUrl)}">
<meta property="og:url" content="${escapeHtml(params.canonicalUrl)}">` : "";
	const navigation = params.olderUrl ? `<nav class="pagination" aria-label="Conversation pages"><a href="${escapeHtml(params.olderUrl)}" rel="prev">← Older messages</a></nav>` : "";
	return `<!doctype html>
<html lang="en"><head>
...
<title>${title} · Assistant</title>${canonicalMetadata}
...
<header><div class="masthead"><div class="brand"><span aria-hidden="true">✳</span>Assistant</div><span class="badge">Public · Read-only</span></div>
...
//#endregion
export { renderPublicSessionDocument };
