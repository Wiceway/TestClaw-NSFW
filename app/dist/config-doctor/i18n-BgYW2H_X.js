	...
	en: {
		...
		wizard: {
			...
			gateway: {
				...
				plaintextPasswordHint: "Stores the password directly in Assistant config",
				...
				tokenStoreProvisioned: "Generated a Gateway secret and stored it in the Assistant secret store as {name}. Config keeps only a reference; inspect it with `testclaw secrets store list`.",
			...
			migration: {
				...
				complete: "Migration complete. Run `testclaw doctor` next.",
			...
			memoryImport: {
				...
				confirm: "Import these memories into your Assistant workspace?",
			...
			remote: {
				...
				insecureRemoteUrl: "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. Break-glass: TESTCLAW_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks.",
				...
				plaintextTokenHint: "Stores the secret directly in Assistant config",
			...
			guided: {
				...
				laneSecurityLine: "Assistant runs an AI agent with real access to this machine. Security guide: https://docs.testclaw.ai/gateway/security",
				...
				quickstartBackground: "To keep the Gateway running in the background later: testclaw gateway install",
				quickstartReopen: "Run testclaw to open the TUI, or testclaw dashboard to reopen the web dashboard.",
				...
				quickstartGatewayPending: "The Gateway is not reachable yet. Keep this terminal open and run testclaw gateway status to check startup.",
				accessFullHint: "I find AI apps and keys on this machine; you choose which connection to test.",
				...
				custodianIntro: "Hi — I'm Assistant. I keep this system running. Let's get you set up.",
				findMeLater: "You can always find me later — run `testclaw` in a terminal, or open Settings in the dashboard.",
				...
				complete: "Assistant is ready.",
				completeWithoutAi: "Assistant setup is saved. Connect AI before opening chat.",
				...
				escapeHatches: "For the full step-by-step wizard, run `testclaw onboard --classic`.",
				...
				invalidConfigDetails: "Assistant config {path} is invalid:\n{issues}",
				...
				nextSteps: "Workspace: {workspace}\nAdd a channel: `testclaw channels add`\nPrefer chatting? Run `testclaw setup` and say `connect telegram` (or `connect slack`).\nOpen the dashboard: `testclaw dashboard`\nChat later: `testclaw`",
				nextStepsWithoutAi: "Workspace: {workspace}\nAdd AI later: re-run `testclaw onboard`\nAfter AI connects, add a channel: `testclaw channels add`\nOpen the dashboard: `testclaw dashboard`",
				...
				ttyRequired: "Onboarding needs an interactive TTY. Use `testclaw onboard --non-interactive --accept-risk ...` for automation.",
			...
			security: {
				...
				attribution: "Assistant is an open-source assistant that learns and grows with you, by the Assistant Foundation (a non-profit).",
				...
				hardeningRequired: "If you're not comfortable managing access controls and security hardening, don't run Assistant without help.",
				learnMore: "Learn more at",
				notMultitenant: "Assistant is not designed to safely separate multiple users by default.",
				personalAgent: "By default, Assistant is a personal agent for one operator.",
			...
			telemetry: {
				...
				decline: "No thanks",
				description: "Share which features you use (channels, providers, plugin count) as part of the daily update check. Never messages, never identifiers. See exactly what is sent: `testclaw telemetry show`. Change anytime: `testclaw telemetry on|off`.",
				title: "Help make Assistant better?"
			},
			skills: {
				...
				docsLine: "Docs: https://docs.testclaw.ai/skills",
				enterEnv: "Enter {env}",
				...
				manualPrereqsDoctorHint: "Install those tools first, then run `testclaw doctor` to review skills + requirements.",
			...
			channels: {
				...
				commandOwnerHelp: "Chat access lets someone talk to your agent. A command owner can also update Assistant, restart the Gateway, change configuration, and approve commands. Choose only your own trusted operator account. This works in servers and groups without DM pairing; it does not grant chat access or change channel access rules.",
			...
			clickclack: {
				...
				helpCreateToken: "In ClickClack: Workspace settings → Integrations → Assistant → create bot → copy token",
			...
			nextcloudTalk: {
				...
				helpInstallCommand: "2) Run: ./occ talk:bot:install \"Assistant\" \"<shared-secret>\" \"<webhook-url>\" --feature webhook --feature response --feature reaction",
			...
			whatsapp: {
				...
				ownerNumberNote: "We need the sender/owner number so Assistant can allowlist you.",
				...
				separatePhoneLabel: "Separate phone just for Assistant",
			...
			irc: {
				...
				nextStatusCommand: "Command: testclaw channels status --probe",
			...
			synologyChat: {
				...
				incomingWebhookHelpReplies: "This is the URL Assistant uses to send replies back to Chat.",
				...
				publicWebhookUrlScope: "Expose only this webhook route; Assistant uses it for short-lived attachment downloads.",
			...
			signal: {
				...
				multipleEntries: "Multiple entries: comma-separated.",
				nextLinkDevice: "Link device with: signal-cli link -n \"Assistant\"",
			...
			twitch: {
				...
				envPrompt: "Twitch env var TESTCLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
				...
				helpEnvVars: "Env vars supported: TESTCLAW_TWITCH_ACCESS_TOKEN",
			...
			channelsPrimer: {
				...
				blurbs: {
					...
					synologyChat: "Connect your Synology NAS Chat to Assistant with full agent capabilities.",
			...
			search: {
				...
				configured: "configured",
				configureLaterHint: "Configure later with testclaw configure --section web",
				docsLine: "Docs: {url}",
			...
			finalize: {
				...
				controlUiDocs: "Docs: https://docs.testclaw.ai/web/control-ui",
				dashboardCopyPaste: "Copy/paste this URL in a browser on this machine to control Assistant.",
				dashboardOpened: "Opened in your browser. Keep that tab to control Assistant.",
				...
				gatewayTokenStored: "Stored in: $TESTCLAW_CONFIG_PATH (default: ~/.testclaw/testclaw.json) under gateway.auth.token, or in TESTCLAW_GATEWAY_TOKEN.",
				...
				outroDashboardLink: "Onboarding complete. Use the dashboard link above to control Assistant.",
				outroDashboardOpened: "Onboarding complete. Dashboard opened; keep that tab to control Assistant.",
				...
				containerSystemdUnavailable: "Systemd user services are not available inside this container. Assistant is skipping only the background service install; run the Gateway in the foreground or use your container supervisor.",
				securityReminder: "Running agents on your computer is risky — harden your setup: https://docs.testclaw.ai/security",
				...
				webDocs: "Docs: https://docs.testclaw.ai/tools/web",
				...
				webUiUrl: "Web UI: {url}",
				whatNow: "What now: https://testclaw.ai/showcase (\"What People Are Building\").",
				whatNowTitle: "What now",
				workspaceBackupTitle: "Workspace backup",
				workspaceDocs: "Docs: https://docs.testclaw.ai/concepts/agent-workspace",
	...
	"zh-CN": {
		...
		wizard: {
			...
			gateway: {
				...
				plaintextPasswordHint: "直接把密码存入 Assistant 配置",
				...
				tokenStoreProvisioned: "已生成 Gateway 令牌并以 {name} 存入 Assistant 密钥存储。配置中只保留引用；可用 `testclaw secrets store list` 查看。",
			...
			migration: {
				...
				appliedTitle: "迁移已应用",
				cancelled: "迁移已取消",
				complete: "迁移完成。下一步运行 `testclaw doctor`。",
			...
			memoryImport: {
				...
				confirm: "将这些 memory 导入你的 Assistant 工作区？",
			...
			remote: {
				...
				insecureRemoteUrl: "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 TESTCLAW_ALLOW_INSECURE_PRIVATE_WS=1。",
				...
				plaintextTokenHint: "将密钥直接存储在 Assistant 配置中",
			...
			guided: {
				laneQuestion: "你想如何开始？",
				laneSecurityLine: "Assistant 运行的 AI 智能体拥有这台机器的实际访问权限。安全指南：https://docs.testclaw.ai/gateway/security",
				...
				quickstartBackground: "如需以后让网关在后台持续运行，请运行：testclaw gateway install",
				quickstartReopen: "运行 testclaw 打开 TUI，或运行 testclaw dashboard 重新打开网页仪表板。",
				quickstartBrowserUnavailable: "浏览器交接未完成。网关仍在运行；准备好后请打开仪表板。",
				quickstartGatewayPending: "网关尚不可达。请保持此终端打开，并运行 testclaw gateway status 检查启动状态。",
				...
				custodianIntro: "你好 — 我是 Assistant，负责维护这套系统。我们开始设置吧。",
				findMeLater: "以后随时可以找到我 — 在终端运行 `testclaw`，或在仪表盘中打开设置。",
				...
				setupDone: "一切就绪。",
				appliedTitle: "推理已就绪",
				complete: "Assistant 已准备就绪。",
				completeWithoutAi: "Assistant 设置已保存。连接 AI 后再打开聊天。",
				detected: "AI 检测完成。",
				detectedCandidate: "{label} — {detail}{recommended}",
				...
				escapeHatches: "如需完整的分步向导，请运行 `testclaw onboard --classic`。",
				...
				intro: "连接你的 AI",
				invalidConfigDetails: "Assistant 配置 {path} 无效：\n{issues}",
				...
				nextSteps: "工作区：{workspace}\n添加频道：`testclaw channels add`\n更喜欢聊天？运行 `testclaw setup`，然后说 `connect telegram`（或 `connect slack`）。\n打开 dashboard：`testclaw dashboard`\n稍后聊天：`testclaw`",
				nextStepsWithoutAi: "工作区：{workspace}\n稍后添加 AI：重新运行 `testclaw onboard`\n连接 AI 后添加频道：`testclaw channels add`\n打开 dashboard：`testclaw dashboard`",
				nextStepsTitle: "下一步",
				...
				ttyRequired: "Onboarding 需要交互式 TTY。自动化请使用 `testclaw onboard --non-interactive --accept-risk ...`。",
				welcomeTitle: "设置选项",
			...
			security: {
				...
				attribution: "Assistant 是由 Assistant 基金会（非营利组织）开发的开源助手，会与你一同学习成长。",
				...
				confirm: "我理解 Assistant 默认面向个人使用；共享/多用户使用需要加固。继续？",
				hardeningRequired: "如果你不熟悉访问控制和安全加固，请在获得帮助前不要运行 Assistant。",
				learnMore: "了解更多请访问",
				notMultitenant: "默认情况下，Assistant 并非旨在安全地隔离多个用户。",
				personalAgent: "默认情况下，Assistant 是供一位操作员使用的个人 agent。",
			...
			telemetry: {
				accept: "是，分享功能使用统计",
				decline: "不用，谢谢",
				description: "在每日更新检查中分享你使用的功能（频道、提供商、插件数量）。绝不包含消息或标识符。查看实际发送的内容：`testclaw telemetry show`。随时更改：`testclaw telemetry on|off`。",
				title: "帮助 Assistant 变得更好？"
			},
			skills: {
				configure: "现在配置技能？（推荐）",
				...
				docsLine: "文档：https://docs.testclaw.ai/skills",
				enterEnv: "输入 {env}",
				...
				manualPrereqsDoctorHint: "请先安装这些工具，然后运行 `testclaw doctor` 检查技能和要求。",
			...
			channels: {
				...
				commandOwnerHelp: "聊天访问权限允许用户与你的代理交谈。命令所有者还可以更新 Assistant、重启 Gateway、更改配置并批准命令。请仅选择你自己信任的管理员账号。服务器和群组无需 DM 配对也可使用此功能；它不会授予聊天访问权限或更改频道访问规则。",
			...
			clickclack: {
				...
				helpCreateToken: "在 ClickClack 中：工作区设置 → Integrations → Assistant → 创建 bot → 复制 token",
			...
			nextcloudTalk: {
				...
				helpInstallCommand: "2) 运行：./occ talk:bot:install \"Assistant\" \"<shared-secret>\" \"<webhook-url>\" --feature webhook --feature response --feature reaction",
			...
			whatsapp: {
				...
				ownerNumberNote: "需要发送者/所有者号码，以便 Assistant 将你加入允许列表。",
				...
				separatePhoneLabel: "Assistant 专用独立手机",
			...
			irc: {
				...
				nextStatusCommand: "命令：testclaw channels status --probe",
			...
			synologyChat: {
				...
				incomingWebhookHelpReplies: "这是 Assistant 用来向 Chat 发送回复的 URL。",
				...
				publicWebhookUrlScope: "仅公开此 webhook 路由；Assistant 使用它提供短期附件下载。",
			...
			signal: {
				...
				multipleEntries: "多个条目请用逗号分隔。",
				nextLinkDevice: "绑定设备：signal-cli link -n \"Assistant\"",
			...
			twitch: {
				...
				envPrompt: "检测到 Twitch 环境变量 TESTCLAW_TWITCH_ACCESS_TOKEN。使用环境 token？",
				helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
				helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
				helpEnvVars: "支持环境变量：TESTCLAW_TWITCH_ACCESS_TOKEN",
			...
			channelsPrimer: {
				...
				blurbs: {
					...
					synologyChat: "将 Synology NAS Chat 连接到 Assistant，并启用完整 agent 能力。",
			...
			search: {
				...
				configured: "已配置",
				configureLaterHint: "稍后可用 testclaw configure --section web 配置",
				docsLine: "文档：{url}",
			...
			finalize: {
				...
				controlUiTitle: "Control UI",
				controlUiDocs: "文档：https://docs.testclaw.ai/web/control-ui",
				dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 Assistant。",
				dashboardOpened: "已在浏览器中打开。保留该标签页以控制 Assistant。",
				...
				gatewayTokenStored: "存储位置：$TESTCLAW_CONFIG_PATH（默认 ~/.testclaw/testclaw.json）中的 gateway.auth.token，或 TESTCLAW_GATEWAY_TOKEN。",
				...
				nodeMac: "macOS app（system + notifications）",
				optionalApps: "可选应用",
				outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 Assistant。",
				outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 Assistant。",
				...
				containerSystemdUnavailable: "此容器内没有 systemd 用户服务。Assistant 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
				securityReminder: "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.testclaw.ai/security",
				...
				webDocs: "文档：https://docs.testclaw.ai/tools/web",
				...
				webUiUrl: "Web UI：{url}",
				whatNow: "下一步：https://testclaw.ai/showcase（\"What People Are Building\"）。",
				whatNowTitle: "下一步",
				workspaceBackupTitle: "工作区备份",
				workspaceDocs: "文档：https://docs.testclaw.ai/concepts/agent-workspace",
	...
	"zh-TW": {
		...
		wizard: {
			...
			gateway: {
				...
				plaintextPasswordHint: "直接把密碼存入 Assistant 設定",
				...
				tokenStoreProvisioned: "已產生 Gateway 權杖並以 {name} 存入 Assistant 祕密儲存。設定中只保留參照；可用 `testclaw secrets store list` 檢視。",
			...
			migration: {
				...
				appliedTitle: "遷移已套用",
				cancelled: "遷移已取消",
				complete: "遷移完成。下一步執行 `testclaw doctor`。",
			...
			memoryImport: {
				...
				confirm: "將這些 memory 匯入你的 Assistant 工作區？",
			...
			remote: {
				...
				insecureRemoteUrl: "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 TESTCLAW_ALLOW_INSECURE_PRIVATE_WS=1。",
				...
				plaintextTokenHint: "將密鑰直接儲存在 Assistant 設定中",
			...
			guided: {
				laneQuestion: "你想如何開始？",
				laneSecurityLine: "Assistant 執行的 AI 代理程式擁有這台機器的實際存取權限。安全指南：https://docs.testclaw.ai/gateway/security",
				...
				quickstartBackground: "如需日後讓閘道在背景持續執行，請執行：testclaw gateway install",
				quickstartReopen: "執行 testclaw 開啟 TUI，或執行 testclaw dashboard 重新開啟網頁儀表板。",
				quickstartBrowserUnavailable: "瀏覽器交接未完成。閘道仍在執行；準備好後請開啟儀表板。",
				quickstartGatewayPending: "閘道尚無法連線。請保持此終端機開啟，並執行 testclaw gateway status 檢查啟動狀態。",
				...
				custodianIntro: "你好 — 我是 Assistant，負責維護這套系統。我們開始設定吧。",
				findMeLater: "之後隨時找得到我 — 在終端機執行 `testclaw`，或在儀表板中開啟設定。",
				...
				setupDone: "一切就緒。",
				appliedTitle: "推理已就緒",
				complete: "Assistant 已準備就緒。",
				completeWithoutAi: "Assistant 設定已儲存。連接 AI 後再開啟聊天。",
				detected: "AI 偵測完成。",
				detectedCandidate: "{label} — {detail}{recommended}",
				...
				escapeHatches: "如需完整的分步精靈，請執行 `testclaw onboard --classic`。",
				...
				intro: "連接你的 AI",
				invalidConfigDetails: "Assistant 設定 {path} 無效：\n{issues}",
				...
				nextSteps: "工作區：{workspace}\n新增頻道：`testclaw channels add`\n偏好聊天？執行 `testclaw setup`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`testclaw dashboard`\n稍後聊天：`testclaw`",
				nextStepsWithoutAi: "工作區：{workspace}\n稍後新增 AI：重新執行 `testclaw onboard`\n連接 AI 後新增頻道：`testclaw channels add`\n開啟 dashboard：`testclaw dashboard`",
				nextStepsTitle: "下一步",
				...
				ttyRequired: "Onboarding 需要互動式 TTY。自動化請使用 `testclaw onboard --non-interactive --accept-risk ...`。",
				welcomeTitle: "設定選項",
			...
			security: {
				...
				attribution: "Assistant 是由 Assistant 基金會（非營利組織）開發的開源助手，會與你一同學習成長。",
				...
				confirm: "我理解 Assistant 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
				hardeningRequired: "如果你不熟悉存取控制和安全加固，請在獲得協助前不要執行 Assistant。",
				learnMore: "了解更多請參閱",
				notMultitenant: "預設情況下，Assistant 並非旨在安全地隔離多個使用者。",
				personalAgent: "預設情況下，Assistant 是供一位操作者使用的個人 agent。",
			...
			telemetry: {
				accept: "是，分享功能使用統計",
				decline: "不用，謝謝",
				description: "在每日更新檢查中分享你使用的功能（頻道、提供者、插件數量）。絕不包含訊息或識別碼。查看實際傳送的內容：`testclaw telemetry show`。隨時變更：`testclaw telemetry on|off`。",
				title: "幫助 Assistant 變得更好？"
			},
			skills: {
				configure: "現在設定技能？（建議）",
				...
				docsLine: "文件：https://docs.testclaw.ai/skills",
				enterEnv: "輸入 {env}",
				...
				manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `testclaw doctor` 檢查技能和需求。",
			...
			channels: {
				...
				commandOwnerHelp: "聊天存取權限允許使用者與你的代理交談。命令擁有者還可以更新 Assistant、重新啟動 Gateway、變更設定並核准命令。請僅選擇你自己信任的管理員帳號。伺服器和群組無需 DM 配對也可使用此功能；它不會授予聊天存取權限或變更頻道存取規則。",
			...
			clickclack: {
				...
				helpCreateToken: "在 ClickClack 中：工作區設定 → Integrations → Assistant → 建立 bot → 複製 token",
			...
			nextcloudTalk: {
				...
				helpInstallCommand: "2) 執行：./occ talk:bot:install \"Assistant\" \"<shared-secret>\" \"<webhook-url>\" --feature webhook --feature response --feature reaction",
			...
			whatsapp: {
				...
				ownerNumberNote: "需要傳送者/擁有者號碼，以便 Assistant 將你加入允許清單。",
				...
				separatePhoneLabel: "Assistant 專用獨立手機",
			...
			synologyChat: {
				...
				incomingWebhookHelpReplies: "這是 Assistant 用來向 Chat 傳送回覆的 URL。",
				...
				publicWebhookUrlScope: "僅公開此 webhook 路由；Assistant 使用它提供短期附件下載。",
			...
			signal: {
				...
				multipleEntries: "多個項目請用逗號分隔。",
				nextLinkDevice: "綁定裝置：signal-cli link -n \"Assistant\"",
			...
			twitch: {
				...
				envPrompt: "偵測到 Twitch 環境變數 TESTCLAW_TWITCH_ACCESS_TOKEN。使用環境 token？",
				helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
				helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
				helpEnvVars: "支援環境變數：TESTCLAW_TWITCH_ACCESS_TOKEN",
			...
			channelsPrimer: {
				...
				blurbs: {
					...
					synologyChat: "將 Synology NAS Chat 連接到 Assistant，並啟用完整 agent 能力。",
			...
			search: {
				...
				configured: "已設定",
				configureLaterHint: "稍後可用 testclaw configure --section web 設定",
				docsLine: "文件：{url}",
			...
			finalize: {
				...
				controlUiTitle: "Control UI",
				controlUiDocs: "文件：https://docs.testclaw.ai/web/control-ui",
				dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 Assistant。",
				dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 Assistant。",
				...
				gatewayTokenStored: "儲存位置：$TESTCLAW_CONFIG_PATH（預設 ~/.testclaw/testclaw.json）中的 gateway.auth.token，或 TESTCLAW_GATEWAY_TOKEN。",
				...
				nodeMac: "macOS app（system + notifications）",
				optionalApps: "可選應用",
				outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 Assistant。",
				outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 Assistant。",
				...
				containerSystemdUnavailable: "此容器內沒有 systemd 使用者服務。Assistant 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
				securityReminder: "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.testclaw.ai/security",
				...
				webDocs: "文件：https://docs.testclaw.ai/tools/web",
				...
				workspaceDocs: "文件：https://docs.testclaw.ai/concepts/agent-workspace",
