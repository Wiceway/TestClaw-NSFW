...
async function cleanupRetainedPluginInstallGenerations(params) {
	try {
		const { clearLoadInstalledPluginIndexInstallRecordsCache, loadInstalledPluginIndexInstallRecordsSync } = await import("./installed-plugin-index-records-DB71deh6.js");
		clearLoadInstalledPluginIndexInstallRecordsCache();
		const records = loadInstalledPluginIndexInstallRecordsSync();
		const { cleanupRetainedManagedNpmInstallGenerations } = await import("./managed-npm-retention-CydNMRqY.js");
