...
//#region src/infra/update-network-budget.ts
const UPDATE_NETWORK_TIMEOUT_MS = 3e5;
//#endregion
...
function resolveEnvPath(env, primaryKey, fallbackKey) {
...
function resolveNpmConfigFiles(env, scope = {}, userNpmrc = resolveEnvPath(env, "NPM_CONFIG_USERCONFIG", "npm_config_userconfig") ?? resolveHomeNpmrc(env)) {
	const files = [
		resolveScopedProjectNpmrc(scope),
		userNpmrc,
		resolveEnvPath(env, "NPM_CONFIG_GLOBALCONFIG", "npm_config_globalconfig"),
	...
	return uniqueStrings(files.filter((file) => Boolean(file)));
}
...
/**
* Builds npm args that bypass host freshness policies for Assistant-managed installs.
...
function createNpmProjectInstallEnv(env, options = {}, now = /* @__PURE__ */ new Date()) {
	...
	const installEnv = {
		...
		npm_config_fetch_timeout: nextEnv.npm_config_fetch_timeout ?? String(3e5),
...
//#endregion
export { UPDATE_NETWORK_TIMEOUT_MS as a, createNpmProjectInstallEnv as i, applyPosixNpmScriptShellEnv as n, createNpmFreshnessBypassArgs as r, applyNpmFreshnessBypassEnv as t };
