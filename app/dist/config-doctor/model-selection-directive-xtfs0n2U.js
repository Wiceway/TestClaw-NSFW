...
function formatNotAllowedError(params) {
	...
	if (rawRuntime && normalizeProviderId(rawRuntime) === "codex") lines.push("If the Codex runtime is missing, run: testclaw plugins enable codex");
	return lines.join("\n");
}
...
function resolveModelDirectiveSelection(params) {
	...
	const resolveFuzzy = (paramsLocal) => {
		...
		for (const key of allowedModelKeys) {
			...
			if (model.endsWith("*") || !policy.allows({
				provider,
				model
			})) continue;
		...
		if (!paramsLocal.provider) {
			...
			for (const match of aliasMatches) {
				if (!policy.allows(match)) continue;
	...
	if (permitted && (rawLower.includes("/") || resolved.alias || allowedModelKeys.has(resolvedKey))) return explicitSelection;
	...
	const fuzzy = resolveFuzzy({ fragment: rawTrimmed });
	if (fuzzy.selection || fuzzy.error) return fuzzy;
	if (permitted) return explicitSelection;
