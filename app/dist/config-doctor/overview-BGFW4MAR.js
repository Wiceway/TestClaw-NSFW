...
import { k as listAgentEntries, p as resolveAmbientOwnerAgentId } from "./agent-scope-config-BEuqweC1.js";
import { p as resolveConfigPath, v as resolveGatewayPort } from "./paths-DeOFr7iP.js";
...
import { n as TESTCLAW_SOURCE_URL, r as resolveAssistantReferencePaths, t as TESTCLAW_DOCS_URL } from "./docs-path-BUdO-egB.js";
import { n as probeLocalCommand, t as probeGatewayUrl } from "./probes-DJJ8L6Eo.js";
...
function buildAgentSummaries(cfg, defaultAgentId) {
	...
	if (entries.length === 0) {
		...
		return [{
			id: defaultAgentId,
			isDefault: true,
			model: resolveConfiguredPrimaryModelForAgent({
				cfg,
	...
	for (const entry of entries) {
		...
		const model = resolveConfiguredPrimaryModelForAgent({
			cfg,
			agentId: id
		});
		if (model) summary.model = model;
		const utility = readUtilityModelSetting(cfg, id);
		if (utility.kind === "explicit") summary.utilityModel = utility.modelRef;
...
async function loadSystemAgentOverview(opts = {}) {
	...
	const defaultModel = resolveConfiguredPrimaryModelForAgent({
		cfg,
		agentId: defaultAgentId
	...
	let gatewayError;
	try {
		const details = (deps.buildGatewayConnectionDetails ?? (await import("./call-BUB0AMHV.js")).buildGatewayConnectionDetails)({
			config: cfg,
			configPath
		});
	...
	const resolveReferences = deps.resolveAssistantReferencePaths ?? resolveAssistantReferencePaths;
	...
	return {
		...
		references: {
			...
			docsUrl: TESTCLAW_DOCS_URL,
			sourcePath: references.sourcePath ?? void 0,
			sourceUrl: TESTCLAW_SOURCE_URL
		}
	};
}
...
function formatSystemAgentOverview(overview) {
	...
	return [
		"Assistant online. Little claws, typed tools.",
		"",
		`Config: ${configStatus}`,
		...
		`AI: ${overview.defaultModel || overview.setupModel ? `conversation runs on ${overview.defaultModel ?? overview.setupModel}` : "inference unavailable; run testclaw onboard before starting Assistant"}`,
...
function recommendSystemAgentNextStep(overview) {
	if (!overview.config.exists) return "run \"testclaw onboard\" to establish inference";
	if (!overview.config.valid) return "run \"validate config\" or \"doctor\" to inspect the config";
	if (!overview.defaultModel) return overview.setupModel ? "continue setup here; run \"testclaw onboard\" to choose your regular agent model" : "run \"testclaw onboard\" to establish inference";
...
function formatStartupAction(overview) {
	...
	if (!overview.defaultModel && !overview.setupModel) return "Inference is unavailable. Run `testclaw onboard` and complete a live model check.";
	if (!overview.defaultModel) return "Setup and utility inference are ready. Choose a regular agent model in Model Setup or run `testclaw onboard`.";
}
/**
* Welcome shown right after inference activation. Assistant owns the
...
function formatSystemAgentOnboardingWelcome(overview) {
	return [
		...
		"",
		overview.defaultModel ? "Say `talk to agent` to meet your agent right here, or `help` for everything I can do." : "Your setup model stays available here. Choose a primary model in Model Setup or run `testclaw onboard` before opening regular agent chat."
	].join("\n");
}
function formatSystemAgentStartupMessage(overview) {
	...
	return [
		"Hi, I'm Assistant — caretaker of this gateway, config, channels, and agents.",
		overview.defaultModel ? `Model: ${overview.defaultModel}.` : overview.setupModel ? `Setup model: ${overview.setupModel}.` : "Inference is unavailable.",
...
//#endregion
export { loadSystemAgentOverview as i, formatSystemAgentOverview as n, formatSystemAgentStartupMessage as r, formatSystemAgentOnboardingWelcome as t };
