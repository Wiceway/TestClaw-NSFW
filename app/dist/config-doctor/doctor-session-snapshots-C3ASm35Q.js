...
import { p as shortenHomePath, w as resolveConfigDir } from "./utils-BfoJTy8l.js";
import { E as resolveStateDir } from "./paths-DeOFr7iP.js";
import { n as hydrateSessionStoreSkillPromptRefs } from "./skill-prompt-blobs-DSVi3nkm.js";
...
function isTempBackedAssistantRoot(segments) {
	const lower = segments.map((segment) => segment.toLowerCase());
	const testclawIndex = lower.lastIndexOf("testclaw");
	if (testclawIndex < 1) return false;
	return lower[testclawIndex - 1] === "tmp" || lower[testclawIndex - 1] === "temp";
}
function isBundledRuntimeSkillsPath(cachedPath, skillRootIndex) {
	...
	return beforeSkillRoot.map((segment) => segment.toLowerCase()).some((segment) => segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("testclaw@")) || isTempBackedAssistantRoot(beforeSkillRoot);
}
...
function sessionSnapshotIssueToHealthFinding(issue) {
	return {
		...
		fixHint: "No repair is needed for this historical metadata. Doctor preserves migration originals; active sessions use canonical SQLite state and the current runtime skill catalog."
	};
}
...
if (process.env.VITEST || false) globalThis[Symbol.for("testclaw.doctorSessionSnapshotsTestApi")] = {
	resolveSessionSnapshotBundledSkillsDir,
	scanSessionStoreForStaleRuntimeSnapshotPaths
};
//#endregion
export { detectSessionSnapshotHealthIssues, noteSessionSnapshotHealth, sessionSnapshotIssueToHealthFinding };
