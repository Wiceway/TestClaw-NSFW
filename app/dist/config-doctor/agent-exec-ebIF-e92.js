...
import { n as resolveAgentExecPrompt, r as resolveExecBaseConfig, t as buildExecRunConfig } from "./agent-exec-input-4ZirtF2c.js";
...
async function agentExecCommand(positionalMessage, opts, runtime, deps = {}) {
	...
	try {
		...
		configIo = await import("./io-CYktO81x.js");
		...
		const pluginInstallContext = opts.isolated !== true && opts.authEnvOnly !== true ? await import("./install-root-context-DOPJErSl.js") : void 0;
		...
		const { resolveAgentDir, resolveAmbientOwnerAgentId } = await import("./agent-scope-config-DjAEl3pT.js");
		...
		if (getInstallationTarget()) {
			const [{ resolveSandboxConfigForAgent }, { resolveExecToolConfig }] = await Promise.all([import("./config-BYFcIrXD.js"), import("./lazy-exec-tool-DX5ZL5tu.js")]);
		...
		runtimePaths = await import("./paths-BkEiZ7Rh.js");
		...
		if (processScopeKey) {
			const { getProcessSupervisor } = await import("./supervisor-6zcLnPH6.js");
		...
		if (temporaryStateDir) {
			const { createAssistantDatabaseMaintenanceScope } = await import("./testclaw-state-db-async-lifecycle-DRyXPImT.js");
		...
		const [{ withAuthProfileStoreAgentDir, withEnvOnlyAuthProfileStore }, { withHostExecInheritedEnvOmitted }, { listKnownProviderAuthEnvVarNamesCore }, runAgent] = await Promise.all([
			import("./auth-profiles-TP6SVtjs.js"),
			import("./host-env-security-MNevTlBE.js"),
			import("./provider-env-vars-68pmu7dI.js"),
			deps.runAgent ? Promise.resolve(deps.runAgent) : import("./agent-C7g2N2PY.js").then((module) => module.agentCommand)
		]);
		...
		const run = async () => {
			if (isExecutionIdentityCollectionEnabled(runConfig)) try {
				stopLocalAuditWriter = (await import("./agent-local-audit-CnyOEamE.js")).startAgentLocalAuditWriter(runConfig, { stateDir });
			} catch {}
