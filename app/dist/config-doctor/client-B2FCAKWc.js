...
import { a as omitEnvKeysCaseInsensitive, r as listKnownProviderAuthEnvVarNamesCore } from "./provider-env-vars-B8YMBgKQ.js";
import { r as isKnownCoreToolId } from "./tool-catalog-BmsCxUYu.js";
import { n as isMutatingToolCall } from "./tool-mutation-CMmOHpf5.js";
...
function buildAcpClientStripKeys(params) {
	...
	if (params.stripProviderAuthEnvVars) for (const key of listKnownProviderAuthEnvVarNamesCore()) stripKeys.add(key);
	return stripKeys;
}
...
/** Interactive stdio ACP client used to connect a terminal session to an Assistant ACP server. */
...
async function waitForChildExit(child, timeoutMs) {
	if (!hasChildExited(child)) await once(child, "exit", { signal: AbortSignal.timeout(timeoutMs) }).catch(() => {});
...
async function createAcpClient(opts = {}) {
	...
	const defaultServerCommand = entryPath ? process.execPath : "testclaw";
	...
	const { getActiveSkillEnvKeys } = await import("./env-overrides.runtime-2lULXe5s.js");
...
async function runAcpClientInteractive(opts = {}) {
	...
	console.log("Assistant ACP client");
	console.log(`Session: ${sessionId}`);
	console.log("Type a prompt, or \"exit\" to quit.\n");
	let quitting = false;
	const quit = async () => {
		if (quitting || hasChildExited(agent)) return;
		quitting = true;
		await terminateAcpServer(agent);
		rl.close();
		process.exit(0);
