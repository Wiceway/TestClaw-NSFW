...
import { N as tryResolveDefaultAgentId, k as listAgentEntries } from "./agent-scope-config-BEuqweC1.js";
...
import { n as formatIcaclsResetCommand, t as createIcaclsResetCommand } from "./permissions-Dw0_GTuv.js";
import { p as resolveAuthProfileDatabaseFilePaths } from "./sqlite-C9aIS6tB.js";
import { p as getSecurityAuditCommandSecretTargetIds } from "./command-secret-targets-CdV9gKy0.js";
...
async function collectChannelSecurityConfigFixMutation(params) {
	...
	const collectPlugins = async () => {
		...
		try {
			...
			const { listBundledChannelPlugins } = await import("./bundled-BtzMAfHg.js");
...
function registerSecurityCli(program) {
	program.command("security").description("Audit local config and state for common security foot-guns").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["testclaw security audit", "Run a local security audit."],
		["testclaw security audit --deep", "Include best-effort live Gateway probes and plugin-owned security audit collectors."],
		["testclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
		["testclaw security audit --deep --password <password>", "Use explicit password for deep probe."],
		["testclaw security audit --auth password --password <password>", "Audit a runtime-only password-mode Gateway secret."],
		["testclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
		["testclaw security audit --json", "Output machine-readable JSON."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.testclaw.ai/cli/security")}\n`).command("audit").description("Audit config + local state for common security foot-guns").option("--deep", "Attempt live Gateway probes and plugin-owned collector checks", false).option("--auth <mode>", "Runtime gateway auth mode (\"none\"|\"token\"|\"password\"|\"trusted-proxy\")").option("--token <token>", "Use explicit gateway token for deep probe auth").option("--password <password>", "Use explicit gateway password for deep probe auth").option("--fix", "Apply safe fixes (tighten defaults + chmod state/config)", false).option("--json", "Print JSON", false).action(async (opts) => {
		...
		const lines = [];
		lines.push(heading("Assistant security audit"));
		...
		lines.push(muted(`Run deeper: ${formatCliCommand("testclaw security audit --deep")}`));
		for (const diagnostic of secretDiagnostics) lines.push(muted(`[secrets] ${diagnostic}`));
		if (opts.fix) {
			lines.push(muted(`Fix: ${formatCliCommand("testclaw security audit --fix")}`));
