...
import { S as parseCronRunScopeSuffix, k as parseAgentSessionKey } from "./session-key-AvQIavYt.js";
import { Yt as cronTaskRecordToRunLogEntry } from "./testclaw-state-db-readonly-mjFl_Qah.js";
...
import { Ja as validateTasksGetParams, Ns as TasksHistoryResultSchema, Xa as validateTasksListParams, Ya as validateTasksHistoryParams, Za as validateTasksRecoveryParams, qa as validateTasksCancelParams } from "./validator-registry-Dpl5QmuY.js";
import { n as resolveRequestedSessionAgentId } from "./session-request-agent-cU98pfB6.js";
import { n as prepareTaskRegistryRead, t as createTaskRegistryReadPreparation } from "./task-registry-read-BjHX4Kh8.js";
import { o as listTaskRecordPage, r as getTaskById } from "./task-registry-query-R9q--_2Z.js";
...
import { i as retrySubagentCompletionDelivery, n as dismissSubagentCompletionDelivery } from "./subagent-completion-delivery-B6uhWOJ5.js";
import { t as assertValidParams } from "./validation-BZLpfukT.js";
import { a as prepareTaskSessionReadFilter, i as canAccessTaskRequesterSession, n as resolveTaskHistoryHarness, r as taskTranscriptSessionKey, t as mapTaskSummary } from "./task-summary-DQ0qa28M.js";
...
const taskHistoryHandler = async (opts) => {
	...
	try {
		...
		if (sessionKey) {
			const { chatHistoryHandlers, handleChatHistoryRequest } = await import("./chat-history-handler-CP5VBh9d.js");
			assertCurrent();
...
const tasksHandlers = {
	...
	"tasks.cancel": async ({ params, respond, context, client }) => {
		...
		const { cancelDetachedTaskRunByIdCore } = await import("./task-executor-cancel.runtime-CgBDyR-F.js");
		const cfg = context.getRuntimeConfig();
	...
	"tasks.dismiss": async ({ params, respond, context, client }) => {
		...
		const { discardSubagentTerminalDelivery } = await import("./subagent-registry-B89YIjYr.js");
		const results = [];
		const cfg = context.getRuntimeConfig();
		for (const taskId of params.taskIds) {
			const task = getTaskById(taskId);
