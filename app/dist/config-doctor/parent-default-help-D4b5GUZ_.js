...
function outputParentHelpWithoutStartupBanner(parent) {
	const previous = process.env.TESTCLAW_SUPPRESS_HELP_BANNER;
	process.env.TESTCLAW_SUPPRESS_HELP_BANNER = "1";
	try {
		parent.outputHelp();
	} finally {
		if (previous === void 0) delete process.env.TESTCLAW_SUPPRESS_HELP_BANNER;
		else process.env.TESTCLAW_SUPPRESS_HELP_BANNER = previous;
	}
}
/**
