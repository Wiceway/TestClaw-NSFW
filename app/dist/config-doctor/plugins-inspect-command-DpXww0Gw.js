...
import { n as formatPluginTrustDiagnostic } from "./plugin-trust-DvRfJOp-.js";
...
import { r as formatPluginStatus, t as formatPluginBundleFormat } from "./plugins-list-format-Oaov1Gl6.js";
...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, withPluginDiagnosticsReportForInspection, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-CwBAwNMY.js");
	const { loadPluginMetadataSnapshot } = await import("./plugin-metadata-snapshot-BsDsrUZI.js");
	...
	if (!targetPlugin) {
		...
		if (id === "skill-workshop") {
			const { detectSkillWorkshopToolPolicyDiagnostic } = await import("./tool-policy-diagnostic-BE94eoaq.js");
