...
function loadSetupShared() {
	return setupSharedPromise ??= import("./setup.shared-C8Nq4VZk.js");
}
...
function requireLocalGateway(config) {
	...
	throw new Error("Hosted Gateway setup manages only a local Gateway. Use `testclaw onboard` for fresh setup or `testclaw configure` for the mode question, then retry after selecting local mode.");
}
...
async function runHostedSetup(params) {
	try {
		...
		return await runOutsidePluginRuntimeGenerationScope(() => withPluginCache(cache, async () => {
			const { readSetupConfigFileSnapshot, writeWizardConfigFile } = await loadSetupShared();
			const snapshot = await readSetupConfigFileSnapshot();
			if (!snapshot.exists || !snapshot.valid || !snapshot.hash) throw new Error(`${params.label} requires a valid saved config snapshot. On the machine running Assistant, run \`testclaw doctor --fix\` and resolve any remaining validation errors; then retry.`);
...
async function runHostedSkillsSetup(prompter, beforePersistentApply, runtime) {
	const [{ setupSkills }, { resolveSystemAgentOnboardingTarget }] = await Promise.all([import("./onboard-skills-Z986F5_h.js"), import("./onboard-agent-target-FQoB8WU6.js")]);
	return await runHostedSetup({
		label: "Skills setup",
		runtime,
		beforePersistentApply,
...
async function runHostedGatewaySetup(prompter, beforePersistentApply, runtime) {
	const [{ resolveGatewayPort }, { configureGatewayForSetup }, { resolveQuickstartGatewayDefaults }] = await Promise.all([
		...
		import("./setup.gateway-config-026ATagq.js"),
		loadSetupShared()
	]);
	return await runHostedSetup({
		label: "Gateway setup",
...
async function runHostedMemoryImport(prompter, beforePersistentApply, onProviderOutcome) {
	const [{ readSetupConfigFileSnapshot }, { resolveSystemAgentOnboardingTarget }] = await Promise.all([loadSetupShared(), import("./onboard-agent-target-FQoB8WU6.js")]);
	const snapshot = await readSetupConfigFileSnapshot();
	if (!snapshot.exists || !snapshot.valid || !snapshot.hash) throw new Error("Memory import requires a valid saved config. Run `testclaw doctor --fix`, then retry.");
...
async function renderMemoryImport(outcome, appendAuditEntry = appendSystemAgentAuditEntry) {
	...
	if (outcome.status === "workspace-missing") return [`Memory import is unavailable because the default agent workspace does not exist at ${outcome.workspace}.`, "Finish onboarding first with `testclaw onboard`, then retry."].join("\n");
