...
import { o as requestTelemetryConsent, s as requireRiskAcknowledgement, u as getSecurityNoteTitle } from "./setup.shared-Du-pVvdm.js";
import { n as showSessionMigrationWarnings, t as promptFirstOnboardingAgent } from "./onboard-first-agent-BPUE5gkC.js";
import { t as enableDefaultOnboardingInternalHooks } from "./onboard-hooks-Ca9woCpp.js";
import { n as runGuidedOnboardingHandoff, r as runInteractiveOnboarding, t as hasInteractiveOnboardingTty } from "./onboard-interactive-runner-5T0Quyoj.js";
import path from "node:path";
import { randomUUID } from "node:crypto";
//#region src/commands/onboard-guided-consent.ts
async function persistGuidedAccessMode(mode) {
	const { mutateConfigFileWithRetry } = await import("./config-fCohulPn.js");
...
async function runManualStage(params) {
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BfW3jC_l.js");
	while (true) {
		...
		const modelTarget = candidate ? candidate.modelTarget : presentedOption ? presentedOption.modelTarget : (await import("./provider-flow-CrGgjnl9.js")).resolveProviderSetupFlowContributions({
			config: params.config,
			workspaceDir: params.workspace
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	const onboardHelpers = await import("./onboard-helpers-Bfj0vknI.js");
	...
	const localOnboarding = custodianMode ? await import("./local-onboarding-state-CwbouCmN.js") : void 0;
	...
	if (resumingSetup && hasAuthoredRoster) {
		const { matchesLocalSetupWorkspace } = await import("./setup-recovery-CURko1X2.js");
	...
	const activateInference = deps.activate ?? (await import("./setup-inference-DqvZe-fa.js")).activateSetupInference;
	const detect = deps.detect ?? (await import("./setup-inference-DqvZe-fa.js")).detectSetupInference;
	let detection;
	...
	if (wantsDiscovery) {
		...
		recommendedInstalls: [],
		...await (deps.listManualOptions ?? (await import("./setup-inference-DqvZe-fa.js")).listManualSetupInferenceOptions)()
	};
	...
	if (localOnboarding && (resumingSetup || !existingConfig.gateway && !detection.setupComplete)) {
		const { withConfigMutationExclusive } = await import("./config-fCohulPn.js");
	...
	if (!custodianMode) {
		...
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-B5HmTF7p.js")).runSetupMemoryImportStep)({
	...
	const { resolveSetupWorkspaceSelection } = await import("./setup.workspace-gBNwZfj_.js");
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-FCovioRZ.js")).applySystemAgentSetup;
			...
			const appliedSnapshot = localSetup?.status === "pending" ? await (await import("./setup-recovery-CURko1X2.js")).completeLocalSetupRecovery({
				owner: localSetup,
	...
	const { resolveConfiguredSetupModelForAgent } = await import("./utility-model-CkYA544q.js");
	...
	if (wantsDiscovery && !quickstart && !setupOnly) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-B5HmTF7p.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime,
			...handoffAgentId ? { agentId: handoffAgentId } : {}
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-DyKsvmdm.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
	...
	if (opts.tui !== true) {
		if ((await (deps.runBrowserHandoff ?? (await import("./onboard-browser-handoff-BrrQp9Dt.js")).runBrowserHatchHandoff)({
			config: persistedConfig,
			prompter,
			...handoffAgentId ? { agentId: handoffAgentId } : {},
