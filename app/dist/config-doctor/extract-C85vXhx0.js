...
import { A as SOURCE_EXECUTABLES, L as splitShellArgs, M as parseEnvInvocationPrelude, N as resolveCarrierCommandArgv, O as unwrapKnownDispatchWrapperInvocation, P as normalizeExecutableToken, S as resolveInlineCommandMatch, a as extractShellWrapperCommand, f as resolveShellWrapperTransportArgv, h as POSIX_INLINE_COMMAND_FLAGS, j as isEnvAssignmentToken, k as COMMAND_CARRIER_EXECUTABLES, o as extractShellWrapperInlineCommand, t as POSIX_PARSEABLE_SHELL_WRAPPERS, u as isShellWrapperExecutable } from "./shell-wrapper-resolution-BVUBYqqS.js";
...
async function loadParser() {
	...
	const language = await TreeSitter.Language.load(new URL(new URL("tree-sitter-bash.wasm", import.meta.url).href));
...
async function visitNode(node, output, context, state) {
	...
	if (node.type === "command" || node.type === "declaration_command" || node.type === "test_command") {
		...
		if (node.type === "command" && nameNode && !parsed) output.risks.push({
			...
			if (nameNode) step.argvSpans = [step.executableSpan, ...parsed.arguments.map((argument) => argument.span)];
...
//#endregion
export { detectCommandCarrierArgv as a, describeInterpreterInlineEval as c, buildCommandPayloadCandidates as i, isInterpreterLikeAllowlistPattern as l, explainShellCommand as n, detectInlineEvalArgv as o, buildCommandPayloadArgvCandidates as r, detectInlineEvalInSegments as s, CommandExplanationWorkLimitError as t };
