...
import { n as GUARDED_CONFIG_INCLUDE_WRITE_ERROR } from "./mutation-conflict-Be0wSyDG.js";
...
import { d as isPluginBackingDefaultInferenceRoute, r as applyPersistentOperation } from "./operations-execution-helpers-pCkBYyZ2.js";
...
async function inspectArtifact(rootDir) {
	...
	const { resolvePluginArtifactDeclaredSurface } = await import("./capability-artifact-t16iGjti.js");
	const { computeDeclaredSurfaceHash } = await import("./capability-summary-qN-PX38e.js");
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => await withArtifactImports(async (files, assertOwned) => {
			...
			return await withVerifiedArtifact(bytes, async ({ archivePath, review }) => {
				const { assertConfigWriteAllowedInCurrentMode } = await import("./config-fCohulPn.js");
				const { loadConfigForInstall } = await import("./install-config-DYuRJuJZ.js");
