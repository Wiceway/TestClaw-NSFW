...
import { d as normalizeStringEntries, p as normalizeTrimmedStringList, y as uniqueStrings } from "./string-normalization-DsCfAx8q.js";
...
import { T as statRegularFile, g as readRegularFile } from "./fs-safe-CZ3jhUUr.js";
import { n as isPathInside } from "./path-safety-DGxmmiKh.js";
import { n as MANIFEST_KEY } from "./legacy-names-CiZDHwOT.js";
import { t as formatCliCommand } from "./command-format-CH2GYoFl.js";
import { C as resolveOAuthDir } from "./paths-DeOFr7iP.js";
import { n as LEGACY_IMPLICIT_AGENT_ID } from "./session-key-AvQIavYt.js";
import { d as extensionUsesSkippedScannerPath } from "./includes-Be6ircIw.js";
import { p as resolveAuthProfileDatabaseFilePaths } from "./sqlite-C9aIS6tB.js";
import { t as resolveWorkshopSkillsDir } from "./skills-root-KReIJMyw.js";
import { r as loadWorkspaceSkills } from "./workspace-skill-loader-CyHERWkN.js";
import { c as loadSkillRootRecords } from "./plugin-skills-C7XFWmHw.js";
import { t as collectIncludePathsRecursive } from "./includes-scan-BuqrRP_m.js";
import path from "node:path";
import fs from "node:fs/promises";
import { clearTimeout, setTimeout } from "node:timers";
//#region src/security/installed-plugin-dirs.ts
const IGNORED_INSTALLED_PLUGIN_DIR_NAMES = /* @__PURE__ */ new Set(["node_modules", ".testclaw-install-backups"]);
/**
...
const loadAuditFsModule = createLazyRuntimeModule(() => import("./audit-fs-nty9xD_J.js"));
const loadAgentScopeModule = createLazyRuntimeModule(() => import("./agent-scope-BM4mAou3.js"));
const loadAgentWorkspaceDirsModule = createLazyRuntimeModule(() => import("./workspace-dirs-DKdoIa71.js"));
const loadSkillSourceModule = createLazyRuntimeModule(() => import("./source-BH0mEPb4.js"));
const loadSkillScannerModule = createLazyRuntimeModule(() => import("./scanner-GJtL1CJu.js"));
const loadExecDockerRaw = createLazyRuntimeNamedExport(() => import("./docker-29_8A4C6.js"), "execDockerRaw");
const loadSandboxBrowserSecurityHashEpoch = createLazyRuntimeNamedExport(() => import("./constants-W2tkEafR.js"), "SANDBOX_BROWSER_SECURITY_HASH_EPOCH");
...
const MAX_PLUGIN_MANIFEST_BYTES = 1048576;
...
async function listSandboxBrowserContainers(params) {
	try {
		const result = await withDockerProbeTimeout(params.timeoutMs, (signal) => params.execDockerRawFn([
			...
			"-a",
			"--filter",
			"label=testclaw.sandboxBrowser=1",
			"--format",
			"{{.Names}}"
		], {
			allowFailure: true,
...
async function readSandboxBrowserHashLabels(params) {
	try {
		const result = await withDockerProbeTimeout(params.timeoutMs, (signal) => params.execDockerRawFn([
			"inspect",
			"-f",
			"{{ index .Config.Labels \"testclaw.configHash\" }}	{{ index .Config.Labels \"testclaw.browserConfigEpoch\" }}",
...
async function collectSandboxBrowserHashLabelFindings(params) {
	...
	if (missingHash.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("testclaw sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (staleEpoch.length > 0) findings.push({
		...
		detail: `Containers: ${staleEpoch.join(", ")}. Expected testclaw.browserConfigEpoch=${browserHashEpoch}.`,
		remediation: `${formatCliCommand("testclaw sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (nonLoopbackPublished.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("testclaw sandbox recreate --browser --all")} (add --force to skip prompt), then verify published ports are bound to 127.0.0.1.`
	});
	if (timedOut) findings.push(buildSandboxBrowserDockerProbeTimeoutFinding(timeoutMs));
	return findings;
}
function buildSandboxBrowserDockerProbeTimeoutFinding(timeoutMs) {
	return {
		...
		detail: `Docker did not answer within ${timeoutMs}ms while checking sandbox browser containers. Assistant skipped any remaining sandbox browser container drift checks for this status run.`,
	...
	const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
		...
		onReadError: (err) => {
			findings.push({
				...
				remediation: "Check file permissions and plugin layout, then rerun `testclaw security audit --deep`."
			});
		}
	});
	for (const pluginName of pluginDirs) {
		...
		try {
			...
			findings.push({
				...
				detail: `Could not parse plugin manifest: ${String(manifestErr)}.\nThe extension entrypoint list is unavailable. Deep scan will cover the plugin directory but may miss entries declared via \`testclaw.extensions\`.`,
				remediation: "Inspect the plugin package.json for syntax errors. If the plugin is untrusted, remove it from your Assistant extensions state directory."
			});
		}
		...
		if (escapedEntries.length > 0) findings.push({
			...
			remediation: "Update the plugin manifest so all testclaw.extensions entries stay inside the plugin directory."
		});
		...
		if (summary.critical > 0) {
			...
			findings.push({
				...
				remediation: "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Assistant extensions state directory."
			});
...
async function collectInstalledSkillsCodeSafetyFindings(params) {
	...
	const entries = [...workspaceDirs].flatMap((workspaceDir) => loadWorkspaceSkills(workspaceDir, { config: params.cfg }));
	const { listAgentIds } = await loadAgentScopeModule();
	const env = {
		...process.env,
		TESTCLAW_STATE_DIR: params.stateDir
	};
	const reportWorkshopScanFailure = (filePath, error) => {
		findings.push({
			checkId: "skills.code_safety.scan_failed",
			severity: "warn",
			title: "Workshop skill inventory scan failed",
			detail: `Static code scan could not inspect ${filePath}: ${String(error)}`,
			remediation: "Check file permissions and skill layout, then rerun `testclaw security audit --deep`."
		});
	};
	for (const agentId of listAgentIds(params.cfg)) {
		const workshopDir = resolveWorkshopSkillsDir(params.cfg, agentId, env);
		entries.push(...loadSkillRootRecords({
			dir: workshopDir,
	...
	for (const entry of entries) {
		if (resolveSkillSource(entry.skill) === "testclaw-bundled") continue;
