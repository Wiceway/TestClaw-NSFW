import { i as listLoadedRuntimePluginIds, n as getActiveRuntimePluginRegistry } from "./active-runtime-registry-CRb0op67.js";
...
import { n as listPersistedRuntimeToolSchemaQuarantines } from "./tool-schema-quarantine-health-BhzmdZT-.js";
import { a as isChannelPluginFailureDiagnostic, n as dedupePluginDiagnostics, o as mergeStatusPluginHealthSnapshots, t as dedupeChannelPluginFailures } from "./status-plugin-health-mzV6EHKR.js";
...
async function collectInstalledPluginHealthSnapshot(params) {
	const { buildPluginCompatibilityNotices, buildPluginSnapshotReport } = await import("./status-CwBAwNMY.js");
...
async function resolveUnregisteredMemoryEmbeddingProviders(params) {
	...
	try {
		const { collectRegisteredEmbeddingProviderIds, collectUnregisteredConfiguredMemoryEmbeddingProviders } = await import("./gateway-startup-plugin-ids-EEF69M1S.js");
...
async function resolveShouldRunPluginIds(params) {
	...
	try {
		const { loadGatewayStartupPluginPlan } = await import("./gateway-startup-plugin-ids-EEF69M1S.js");
		const { resolvePluginActivationSourceConfig } = await import("./activation-source-config-1q9lYHST.js");
		const { resolveGatewayStartupPluginActivationConfig } = await import("./plugin-activation-runtime-config-ubCDQ1XP.js");
