...
import { A as listAgentEntriesWithSource, P as tryResolveSoleAgentId, j as listAgentIds, k as listAgentEntries } from "./agent-scope-config-BEuqweC1.js";
import { E as resolveStateDir, o as isDefaultInstallIdentity } from "./paths-DeOFr7iP.js";
...
import { r as hasConfiguredCommandOwners } from "./doctor-command-owner-YHrnffI4.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-DTCVEE9D.js";
...
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-DQByVFJT.js";
import { i as noteChromeMcpBrowserReadiness, n as maybeArchiveLegacyClawdBrowserProfileResidue, r as maybeRepairOwnedChromeExtensionNativeHosts, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-CEGHPr4z.js";
import { t as hasActiveGatewayExecCredential } from "./doctor-gateway-exec-credential-DBYF8ogh.js";
import { n as configValidationIssuesToHealthFindings, r as configValidationWarningsToHealthFindings, t as FINAL_CONFIG_VALIDATION_CHECK_ID } from "./doctor-config-validation-findings-D7tAFYfr.js";
import { n as disableUnavailableSkillsInConfig, r as formatMissingSkillSummary } from "./doctor-skills-core-DZQVCoNO.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-Cosp2mfN.js";
import { i as resolveKnownModelRefMigrationTarget, n as collectCodexRuntimeCompatibilityWarnings, o as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-CfV_PhiC.js";
...
import { r as securityAuditFindingToHealthFinding, t as copyHealthCheck } from "./health-check-adapter-BrC920MN.js";
...
function createModelReferenceCheck() {
	return {
		...
		source: "doctor",
		async detect(ctx) {
			const { inspectConfiguredModelReferences } = await import("./model-reference-validation-Uk5uqNpj.js");
...
async function collectRuntimeToolSchemaFindingsWithRuntime(ctx) {
	const runtime = await import("./doctor-tool-schema-runtime-CGBu6Zkb.js");
...
const loadDoctorCoreChecksRuntimeModule = async () => await import("./doctor-core-checks.runtime-CCpMrWXX.js");
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace--39cBGc-.js");
...
async function collectSecurityWarningsWithRuntime(cfg, env) {
	const { collectSecurityWarnings } = await import("./doctor-security-DuhArdBR.js");
	return collectSecurityWarnings(cfg, env);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-CRW5RUj6.js");
...
async function collectGatewayHealthFindingsWithRuntime(ctx) {
	return (await import("./doctor-gateway-health-DPjTCDkf.js")).collectGatewayHealthFindings(ctx);
}
...
const skillWorkshopRelocationCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { inspectLegacySkillWorkshopMigration } = await import("./doctor-skill-workshop-sqlite-DjdOvefP.js");
...
const legacyStateCheck = {
	...
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./state-migrations.doctor-D9ytsu2A.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		if (!ctx.cwd) return [];
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget, isFixedUserCapFile } = await import("./bootstrap-budget-C0xshtaw.js");
		const { resolveBootstrapContextForDiagnostics } = await import("./bootstrap-files-diagnostics-TFWPkO36.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-C-s_-8Zz.js");
		const { USER_BOOTSTRAP_MAX_CHARS } = await import("./bootstrap-DaY6ya6I.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./provider-openai-chatgpt-oauth-tls-w0tiOTNL.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	defaultEnabled: false,
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-C9Ck9yVm.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-C9Ck9yVm.js");
...
const staleRuntimeBuildCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectStaleRuntimeBuildFindings } = await import("./doctor-stale-runtime-build-CiPV1-X-.js");
...
const telegramGeneralTopicConversationsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectTelegramGeneralTopicConversationRepairs } = await import("./doctor-telegram-general-topic-conversations-BFFIKqGj.js");
	...
	async repair(ctx) {
		const { repairTelegramGeneralTopicConversations } = await import("./doctor-telegram-general-topic-conversations-BFFIKqGj.js");
		const effect = {
			kind: "state",
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-CxgnC34e.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-CxgnC34e.js");
...
const gatewayPlatformNotesCheck = {
	...
	async detect(ctx) {
		...
		const { collectGatewayPlatformWarnings } = await import("./doctor-platform-notes-Bg-3PM8r.js");
...
function createConvertedWorkflowChecks(deps) {
	return [
		...
		...isExperimentalClawsEnabled() ? [{
			...
			source: "doctor",
			async detect(ctx) {
				const [{ collectClawStateHealthFindings }, { listConfiguredMcpServers }] = await Promise.all([import("./doctor-D631RWts.js"), import("./mcp-config-DSw_ns1_.js")]);
