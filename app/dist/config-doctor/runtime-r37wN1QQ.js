...
import { a as containsSecretSentinel, c as resolveSecretSentinel, i as SECRET_SENTINEL_SUFFIX, n as SECRET_SENTINEL_PATTERN, o as looksLikeSecretSentinel, r as SECRET_SENTINEL_PREFIX } from "./sentinel-De2SwPfV.js";
import { i as publishSecretEgressProxy, t as clearSecretEgressProxy } from "./registry-BwQOxD7I.js";
import { n as ensureSecretEgressProxyCa, r as generateLocalProxyLeaf } from "./ca-D9jRfzVQ.js";
...
import { createServer as createServer$1, rootCertificates } from "node:tls";
import { ServerResponse, createServer as createServer$2 } from "node:http";
...
async function startSecretEgressProxyServer(params) {
		...
		if (!context) {
			context = certificates.createContext({
				...
				createServer: (leaf) => {
					...
					const httpServer = createServer$2(handleRequest).on("upgrade", (request, _socket, head) => handleUpgradeRequest(handleRequest, request, head));
					const tlsServer = createServer$1(leaf).on("secureConnection", (socket) => {
	...
	const proxy = createServer$2(handleProxyRequest).on("upgrade", (request, _socket, head) => handleUpgradeRequest(handleProxyRequest, request, head));
