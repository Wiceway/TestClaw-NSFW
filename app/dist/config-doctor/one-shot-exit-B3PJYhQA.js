...
import { a as waitForCliSignalExit } from "./signal-exit-barrier-D8TPSn7h.js";
import { t as drainProcessOutput } from "./output-drain-DMosb1D5.js";
...
async function closeCliResources(cleanup) {
	...
	const finalizers = {
		"agent-harnesses": async () => {
			const { listRegisteredAgentHarnesses, disposeRegisteredAgentHarnesses } = await import("./registry-BIPAI_Em.js");
			const registered = listRegisteredAgentHarnesses();
			if (!cleanup) {
				if (registered.length > 0) await disposeRegisteredAgentHarnesses();
				return;
			}
			const { markPluginRegistryRetired } = await import("./registry-lifecycle-DQIyAddz.js");
			try {
		...
		"provider-transport-dispatchers": async () => {
			...
			if (hasProviderTransportDispatcherPool()) {
				const { closeProviderTransportDispatcherPool } = await import("./provider-transport-dispatcher-pool-Bup0y7TP.js");
				await closeProviderTransportDispatcherPool();
			}
		},
		"mcp-loopback": async () => {
			const { getActiveMcpLoopbackRuntime } = await import("./mcp-http.loopback-runtime-sBuLo3h-.js");
			if (getActiveMcpLoopbackRuntime()) {
				const { closeMcpLoopbackServer } = await import("./mcp-http-Dy_vwazs.js");
				await closeMcpLoopbackServer();
			}
		},
		memory: async () => {
			const { hasMemoryRuntime } = await import("./memory-state-Cnnjw0fl.js");
			if (hasMemoryRuntime()) {
				const { closeActiveMemorySearchManagersCore } = await import("./memory-runtime-3E22elCV.js");
				await closeActiveMemorySearchManagersCore();
			}
		},
		"agent-databases": async () => {
			const { hasAssistantAgentDatabaseAsyncResources } = await import("./testclaw-agent-db-resources-D45KdoSo.js");
			if (!hasAssistantAgentDatabaseAsyncResources()) return;
			const { closeAssistantAgentDatabasesAsync } = await import("./testclaw-agent-db-lifecycle-D-hgK61h.js");
