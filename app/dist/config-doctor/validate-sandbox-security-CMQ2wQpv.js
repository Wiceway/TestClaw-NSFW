...
import { a as resolveRequiredOsHomeDir, i as resolveRequiredHomeDir } from "./home-dir-DjuHbd5R.js";
...
function getBlockedHostPaths() {
	const cacheKey = JSON.stringify({
		...
		testclawHome: process.env.TESTCLAW_HOME,
		osHome: os.homedir(),
...
function formatBindBlockedError(params) {
	...
	if (params.reason.kind === "reserved_target") return /* @__PURE__ */ new Error(`Sandbox security: bind mount "${params.bind}" targets reserved container path "${params.reason.reservedPath}" (resolved target: "${params.reason.targetPath}"). This can shadow Assistant sandbox mounts. Use a dangerous override only when you fully trust this runtime.`);
