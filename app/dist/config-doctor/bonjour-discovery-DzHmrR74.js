...
import { t as isTailnetIPv4 } from "./tailnet-C1rWXZuf.js";
...
function resolveGatewayDiscoveryEndpoint(beacon) {
	...
	if (!host || typeof port !== "number" || !Number.isSafeInteger(port) || port <= 0 || port > 65535) return null;
...
const GATEWAY_SERVICE_TYPE = "_testclaw-gw._tcp";
...
function applyBeaconTxt(beacon, txt) {
	for (const field of [
		"tailnetDns",
		"cliPath",
		"role",
		"transport"
	]) if (txt[field]) beacon[field] = txt[field];
	beacon.gatewayPort = parseTcpPort(txt.gatewayPort) ?? void 0;
	beacon.sshPort = parseTcpPort(txt.sshPort) ?? void 0;
	if (txt.gatewayTls) {
...
function parseDnsSdBrowse(stdout) {
	...
	for (const raw of stdout.split("\n")) {
		...
		const match = line.match(/_testclaw-gw\._tcp\.?\s+(.+)$/);
...
function parseDnsSdResolve(stdout, instanceName) {
	...
	for (const raw of stdout.split("\n")) {
		...
		if (line.includes("can be reached at")) {
			...
			if (match?.[2]) beacon.port = parseTcpPort(match[2]) ?? void 0;
			continue;
		}
...
async function discoverWideAreaViaTailnetDns(domain, timeoutMs, run) {
	...
	for (const ptr of ptrs) {
		...
		if (!ptrName) continue;
		const instanceName = ptrName.replace(/\.?_testclaw-gw\._tcp\..*$/, "");
		const srv = await run([
			"dig",
			"+short",
			"+time=1",
...
function parseAvahiBrowse(stdout) {
	...
	for (const raw of stdout.split("\n")) {
		...
		if (trimmed.startsWith("port =")) {
			...
			if (match?.[1]) current.port = parseTcpPort(match[1]) ?? void 0;
			continue;
		}
