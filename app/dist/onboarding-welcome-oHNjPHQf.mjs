...
import { t as isSystemAgentNavigationOperation } from "./operation-types-LNFFS9G0.mjs";
import { a as isPersistentSystemAgentOperation, c as isInvalidConfigSetOperation, d as redactSystemAgentConfigPath, l as isSystemAgentSensitiveConfigValue, n as SYSTEM_AGENT_OPERATOR_NAVIGATION_HANDOFF, o as parseSystemAgentOperation, r as describeSystemAgentPersistentOperation } from "./operations-parse-D2_r8HP6.mjs";
import { g as resolveTuiAgentId, n as SystemAgentOperationExitError, u as getRegularAgentSetupNotice } from "./operations-execution-helpers-CfahD7Fz.mjs";
import { t as executeSystemAgentOperation } from "./operations-mTEix0yB.mjs";
import { i as resolvePendingOperatorProposal, n as hashSystemAgentOperation, r as resolveOperatorApprovalDecision, t as classifySystemAgentApprovalText } from "./operator-approval-Ch9Cihoi.mjs";
import { n as isSystemAgentInferenceUnavailableError, t as SystemAgentInferenceUnavailableError } from "./inference-error-BHZ5ovfe.mjs";
import { i as loadSystemAgentOverview, t as formatSystemAgentOnboardingWelcome } from "./overview-DU-Fj9NZ.mjs";
...
import { t as approvalQuestion } from "./dialogue-BeWx-5HS.mjs";
...
import { n as sanitizeWizardStepForClient, r as wizardStepAwaitsInput, t as WizardSession } from "./session-DiJJJpjA.mjs";
import { s as buildSystemAgentSystemPrompt } from "./assistant-prompts-D9kt-9DL.mjs";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEventForRun }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		import("./agent-events-ClE7kQP8.mjs"),
		import("./embedded-agent-tool-results-BUhVCeLn.mjs"),
		import("./system-agent-tool-B6ZEWGrV.mjs")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-CNsYa_zr.mjs")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-Dj7EUuO_.mjs")).runEmbeddedAgent)({
				...shared,
...
var ChatTurnRouter = class {
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.options.classifyApproval ?? (await import("./approval-intent-d_ICL6DG.mjs")).classifySystemAgentApprovalIntent)({
			message: text,
...
var SystemAgentChatEngine = class {
	...
	*/
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-Cmh9sArw.mjs").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
...
async function buildOnboardingWelcome(params) {
	...
	const localSetup = params.localRecovery === true && overview.config.exists && overview.config.valid && authoredConfig !== void 0 && authoredConfig?.gateway?.mode !== "remote" ? (await import("./state/local-onboarding-state.js")).readLocalOnboardingStateForConfig(overview.config.path, authoredConfig) : void 0;
	...
	const { DEFAULT_WORKSPACE } = await import("./onboard-helpers-ByjrA4hV.mjs");
