...
async function resolveExplicitSessionSqliteMaintenancePaths(options) {
	...
	const [{ resolveSessionStoreTargets }, { resolveSqliteTargetFromSessionStorePath }, { resolveSqliteDatabaseFilePaths }] = await Promise.all([
		...
		import("./session-sqlite-target-C-f5VyLr.mjs"),
		import("./sqlite-files-CnbCCWbF.mjs")
	]);
...
async function doctorCommand(runtime, options, databasePreflight) {
	...
	if (options?.stateSqlite) {
		const { runDoctorStateSqliteCompact } = await import("./doctor-state-sqlite-compact-tbe5HXDG.mjs");
	...
	if (options?.sessionSqlite) {
		...
		const { countBlockingSessionSqliteIssues } = await import("./doctor-session-sqlite-types-BHPOBCbh.mjs");
		const { isDestructiveDoctorSessionSqliteMode, withDoctorSqliteMaintenanceLock } = await import("./doctor-sqlite-maintenance-lock-CHsxcydV.mjs");
		const { runDoctorSessionSqlite, reconcileDoctorSessionSqlitePublication } = await import("./doctor-session-sqlite-JH7NSJF8.mjs");
		const { withArtifactPreservingStateReads } = await import("./testclaw-state-db-readonly-C7srNzuk.mjs");
	...
	if (options?.postUpgrade) {
		const { runPostUpgradeProbes } = await import("./doctor-post-upgrade-S4KNKiXw.mjs");
		const { readSourceConfigBestEffort } = await import("./io.runtime.js");
	...
	await (await import("./doctor-health-BxwkRGTR.mjs")).runDoctorHealthFlow(runtime, options, void 0, databasePreflight);
}
async function maybeCreateSessionSqliteGithubIssue(runtime, report, options) {
	...
	const { resolveDoctorRepairMode } = await import("./doctor-repair-mode-DI64QSEu.mjs");
	...
	const { prepareGithubIssue, reconcileGithubIssue, submitGithubIssue } = await import("./github-issue-DZycSzr4.mjs");
	const { claimSessionSqliteMigrationGithubIssue, clearSessionSqliteMigrationGithubIssueClaim } = await import("./doctor-session-sqlite-failure-D3EY-IAj.mjs");
	...
	const { detectBrowserOpenSupport, openUrl } = await import("./browser-open-BtJcieLo.mjs");
...
async function withSessionSqliteGithubIssueReceipt(manifestPath, run) {
	const { withDoctorSqliteMaintenanceLock } = await import("./doctor-sqlite-maintenance-lock-CHsxcydV.mjs");
