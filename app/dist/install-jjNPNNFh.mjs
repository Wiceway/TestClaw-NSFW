...
import { a as detectBundleManifestFormat } from "./bundle-manifest-uCAdG9II.mjs";
import { o as unscopedPackageName, r as resolveSafeInstallDir } from "./install-safe-path-CCmFmXUk.mjs";
import { t as parseHookFrontmatter } from "./frontmatter-B6HWYTlp.mjs";
import { n as hasPackageRuntimeDependencies, t as copyPackageDirInstallTransactionRequest } from "./install-package-dir-Cy6JnOCT.mjs";
import { o as scanInstalledPackageDependencyTree, s as scanPackageInstallSource } from "./install-security-scan-Cf3A6oRi.mjs";
import path from "node:path";
//#region src/hooks/install.ts
const HOOK_MD_MAX_BYTES = 1048576;
const loadHookInstallRuntime = createLazyRuntimeModule(() => import("./install.runtime-9r6n4ig3.mjs"));
const HOOK_INSTALL_ERROR_CODE = {
	MISSING_TESTCLAW_HOOKS: "missing_testclaw_hooks",
	EMPTY_TESTCLAW_HOOKS: "empty_testclaw_hooks"
};
const defaultLogger = {};
...
function resolveAssistantHooks(manifest) {
	const hooks = manifest[MANIFEST_KEY]?.hooks;
	if (!Array.isArray(hooks)) return {
		ok: false,
		error: "package.json missing testclaw.hooks",
		code: HOOK_INSTALL_ERROR_CODE.MISSING_TESTCLAW_HOOKS
	};
	const list = normalizeTrimmedStringList(hooks);
	if (list.length === 0) return {
		ok: false,
		error: "package.json testclaw.hooks is empty",
		code: HOOK_INSTALL_ERROR_CODE.EMPTY_TESTCLAW_HOOKS
	};
	return {
		ok: true,
		entries: list
...
async function installFromResolvedHookDir(resolvedDir, params) {
	...
	const packageKind = await runtime.fileExists(path.join(resolvedDir, "testclaw.plugin.json")) || detectBundleManifestFormat(resolvedDir) !== null ? "plugin-capable" : void 0;
...
async function installHookPackageFromDir(params) {
	...
	const hookManifest = resolveAssistantHooks(manifest);
	...
	for (const entry of hookEntries) {
		...
		if (!runtime.isPathInside(params.packageDir, hookDir)) return {
			ok: false,
			error: `testclaw.hooks entry escapes package directory: ${entry}`
		};
		await validateHookDir(hookDir);
		if (!runtime.isPathInsideWithRealpath(params.packageDir, hookDir, { requireRealpath: true })) return {
			ok: false,
			error: `testclaw.hooks entry resolves outside package directory: ${entry}`
		};
...
async function installHooksFromArchive(params) {
	const runtime = await loadHookInstallRuntime();
	const { logger, timeoutMs, workTimeoutMs } = runtime.resolveTimedInstallModeOptions(params, defaultLogger);
...
async function installHooksFromNpmSpec(params) {
	...
	return await runtime.installFromValidatedNpmSpecArchive({
		tempDirPrefix: "testclaw-hook-pack-",
		spec,
		timeoutMs,
		workTimeoutMs,
