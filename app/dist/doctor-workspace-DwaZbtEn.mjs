...
import { a as resolveCanonicalRootMemoryPath, n as LEGACY_ROOT_MEMORY_FILENAME, o as resolveLegacyRootMemoryPath, s as resolveRootMemoryRepairDir, t as CANONICAL_ROOT_MEMORY_FILENAME } from "./root-memory-files-CXdmSxyG.mjs";
...
const MEMORY_SYSTEM_PROMPT = [
	...
	"https://github.com/testclaw/testclaw/commit/9ffea23f31ca1df5183b25668f8f814bee0fb34e",
	"https://github.com/testclaw/testclaw/commit/7d1fee70e76f2f634f1b41fca927ee663914183a"
].join("\n");
...
function formatRootMemoryFilesWarning(detection) {
	if (detection.canonicalExists && detection.legacyExists) return [
		...
		`Assistant uses ${CANONICAL_ROOT_MEMORY_FILENAME} as the canonical durable memory file.`,
		...
		`Run "testclaw doctor --fix" to merge the legacy file into ${CANONICAL_ROOT_MEMORY_FILENAME} with a backup.`
	].join("\n");
	return null;
}
...
function buildMergedLegacyRootMemorySection(params) {
	return [
		...
		"",
		`<!-- testclaw-root-memory-merge source=${LEGACY_ROOT_MEMORY_FILENAME} archived=${params.archivedLegacyPath} -->`,
