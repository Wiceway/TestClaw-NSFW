...
import { n as resolveGatewayProbeSnapshot } from "./status.scan.shared-Ccxefyw2.mjs";
...
async function scanStatusJsonGateway(opts) {
	...
	const bootstrap = await createStatusScanCoreBootstrap({
		...
		getTailnetHostname: async (runner) => (await import("./tailscale-DDpkPeRT.mjs")).getTailnetHostname(runner),
		getUpdateCheckResult: async (params) => (await import("./status.update-DW2arJpt.mjs")).getUpdateCheckResult(params)
	});
