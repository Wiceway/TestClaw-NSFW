...
const FILE_WRITE_HARD_MAX_BYTES = 16777216;
...
const FILE_FETCH_TOOL_DESCRIPTOR = {
	label: "File Fetch",
	name: "file_fetch",
	description: "Retrieve a file from a paired node by absolute path. Saves all fetched bytes in the gateway's file-transfer media store and returns localPath and mediaId. Returns supported images as image content blocks and inlines small text files (≤8 KB). Use this for screenshots, photos, receipts, logs, source files. The mediaId can be reused as sourceMediaId for binary copies when node-write capability is available. Requires operator opt-in: gateway.nodes.commands.allow must include 'file.fetch', and file-transfer policy must authorize the path through allowReadPaths or a remembered exact approval. Without policy configured, every call is denied.",
...
const FILE_WRITE_TOOL_DESCRIPTOR = {
	...
	parameters: Type.Object({
		...
		sourceMediaId: Type.Optional(Type.String({ description: "mediaId of a previously fetched file in the file-transfer media store. Reuses saved bytes for binary copies. Not a local path or an ID from another media store." })),
...
//#endregion
export { FILE_FETCH_HARD_MAX_BYTES as a, FILE_WRITE_HARD_MAX_BYTES as c, FILE_FETCH_DEFAULT_MAX_BYTES as i, FILE_WRITE_TOOL_DESCRIPTOR as l, DIR_LIST_HARD_MAX_ENTRIES as n, FILE_FETCH_TOOL_DESCRIPTOR as o, DIR_LIST_TOOL_DESCRIPTOR as r, FILE_TRANSFER_SUBDIR as s, DIR_FETCH_TOOL_DESCRIPTOR as t };
