...
import { i as collectExplicitAllowlist } from "./tool-policy-6aEa6C7R.mjs";
...
async function collectBundleMcpRuntimeToolSchemaFindings(params) {
	const { collectNormalizedToolSchemaFindings } = await import("./doctor-tool-schema-projection-DpqYVPDh.mjs");
...
async function collectRuntimeToolSchemaFindings(sourceConfig, options = {}) {
	const [{ captureRuntimeConfig }, { prepareDoctorToolSchemaFrames }, { collectAgentRuntimeToolSchemaFindings }] = await Promise.all([
		import("./runtime-source-projection-dKLgYTCs.mjs"),
		import("./doctor-tool-schema-frames-DItKBpt7.mjs"),
		import("./doctor-tool-schema-projection-DpqYVPDh.mjs")
	]);
	const cfg = captureRuntimeConfig(sourceConfig);
	const env = options.env ?? process.env;
	const runWithPluginMetadataSnapshot = options.runWithPluginMetadataSnapshot ?? (await import("./plugin-metadata-snapshot-scope-Ulz8F9Ph.mjs")).createDoctorPluginMetadataSnapshotScope({ env }).run;
	...
	try {
		if (frames.length > 0) {
			try {
				const [{ acquirePluginToolInspectionRegistry }, { resolvePluginRuntimeLoadContext }] = await Promise.all([import("./tools-DluSfr8C.mjs"), import("./load-context.resolve-CZZxgTHg.mjs")]);
		...
		for (const frame of frames) {
			...
			const collectForAgent = async () => {
				...
				if (!bundleRuntimeByContext.has(runtimeContext) && !bundleRuntimeLoadErrorsByContext.has(runtimeContext)) try {
					const { createBundleMcpToolRuntime } = await import("./agent-bundle-mcp-tools-shhXGOep.mjs");
