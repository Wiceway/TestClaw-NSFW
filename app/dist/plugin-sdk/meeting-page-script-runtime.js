//#region src/meeting-bot/browser-audio-capture-source.ts
/** Captures browser playback before the native virtual microphone bus. */
function createMeetingBrowserAudioCaptureSource(params) {
	return `async () => {
  ...
  const current = window.__testclawMeetingRemoteAudio;
  if (action === "stop") {
  ...
  const capture = {
    ...
    isCurrent: () => !stopped && window.__testclawMeetingRemoteAudio === capture && ownsSession(),
    stop: async () => {
      if (stopped) return;
      ...
      if (window.__testclawMeetingRemoteAudio === capture) delete window.__testclawMeetingRemoteAudio;
      await context.close();
    },
    scan: () => scan(),
  };
  ...
  window.__testclawMeetingRemoteAudio = capture;
  try {
...
//#endregion
export { createMeetingBrowserAudioCaptureSource, createMeetingLeaveSource, createMeetingTranscriptSource };
