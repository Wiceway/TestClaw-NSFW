...
import { n as pickPrimaryTailnetIPv4, r as pickPrimaryTailnetIPv6 } from "./tailnet-BnoWEUJN.mjs";
import { n as renderTable, t as getTerminalTableWidth } from "./table-B8YZjM-g.mjs";
import { a as resolveWideAreaDiscoveryDomain, i as replaceWideAreaZoneFile, n as normalizeWideAreaDomain, t as getWideAreaZonePath } from "./widearea-dns-1dYTFvyH.mjs";
...
function registerDnsCli(program) {
	program.command("dns").description("DNS helpers for wide-area discovery (Tailscale + CoreDNS)").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.testclaw.ai/cli/dns")}\n`).command("setup").description("Set up CoreDNS to serve your discovery domain for unicast DNS-SD (Wide-Area Bonjour)").option("--domain <domain>", "Wide-area discovery domain (e.g. testclaw.internal)").option("--apply", "Install/update CoreDNS config and (re)start the service (requires sudo)", false).action(async (opts) => {
		...
		defaultRuntime.log("");
		defaultRuntime.log(theme.heading("Recommended config ($TESTCLAW_CONFIG_PATH, default ~/.testclaw/testclaw.json):"));
		...
		if (zoneFileNeedsBootstrap(zonePath)) {
			...
			const zoneLines = [
				`; created by testclaw dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
		...
		if (!cfg.discovery?.wideArea?.domain?.trim()) {
			defaultRuntime.log("");
			defaultRuntime.log(theme.muted("Note: set discovery.wideArea.domain in the active Assistant config ($TESTCLAW_CONFIG_PATH, default ~/.testclaw/testclaw.json) on the gateway and restart the gateway so it writes the DNS-SD zone."));
		}
	});
}
//#endregion
