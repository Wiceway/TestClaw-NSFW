...
import { n as THINKING_LEVELS_HELP, s as normalizeThinkLevel } from "./thinking.shared-DS6qUUiH.mjs";
...
import { g as randomIdempotencyKey, o as callGateway } from "./call-Cm2P5Cd6.mjs";
import { p as listProfilesForProvider } from "./order-BnTFWeei.mjs";
import { o as loadAuthProfileStoreForRuntime, p as updateAuthProfileStoreWithLock } from "./store-runtime-CZ_l0ERR.mjs";
import "./auth-profiles-3zYAJqiZ.mjs";
import { t as canonicalizeCaseOnlyCatalogModelRef } from "./model-selection-7SY54ACM.mjs";
...
import { t as buildExplicitSessionIdSessionKey } from "./session-CQurO7xa.mjs";
...
import { a as providerHasGenericConfig, c as resolveCapabilityAgentOption, f as resolveSelectedProviderFromModelRef, l as resolveCapabilityProviderAgentId, p as resolveTransport, s as requireProviderModelOverride, u as resolveLocalCapabilityRuntimeConfig } from "./shared-BTJ-7cM0.mjs";
...
async function runModelAuthStatus(agent) {
	const captured = [];
	const { modelsStatusCommand } = await import("./list.status-command-C4OLCHUm.mjs");
...
function registerModelCapabilityCommands(capability) {
	...
	modelAuth.command("login").description("Run provider auth login").requiredOption("--provider <id>", "Provider id").option("--method <id>", "Provider auth method id").option("--agent <id>", "Agent id (default: configured default agent)").action(async (opts, command) => {
		await runCommandWithRuntime(defaultRuntime, async () => {
			...
			const { modelsAuthLoginCommand } = await import("./auth-CbiwGSzE.mjs");
