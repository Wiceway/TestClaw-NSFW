...
import { L as readStateSchemaPublicationBlocker } from "./testclaw-state-db-read-connection-BvrNnfuu.mjs";
...
import { t as prepareDoctorDatabasePreflight } from "./doctor-database-preflight-Df_4gqMB.mjs";
import { n as recordUpdateDoctorRefusal, r as resolveUpdateDoctorGitRecovery } from "./doctor-update-refusal-CibZ7MUg.mjs";
	...
	if (updater.canDeferStateSchema && blockedMigrations.every((entry) => entry.kind === "agent")) {
		...
		if (authority?.runId === updater.runId && updater.postCoreStarted && maintenance?.ownsSchemaMaintenance) {
			...
			const [{ createBackupArchive }, { verifyBackupArchive }, { resolveUpdateCaptureRoot }] = await Promise.all([
				import("./backup-create-1kjAO25b.mjs"),
				import("./backup-verify-aMqBHvdn.mjs"),
				import("./update-capture-paths-DMrneA32.mjs")
			]);
			assertCurrent();
			let snapshotFacts = [];
...
async function rehearseDeferredUpdateDoctorSchema(schemas, runtime = defaultRuntime) {
	...
	const [{ createConfigIO }, { resolveAssistantPackageRoot }, { resolveUpdateInstallKind }, { prepareUpdateCandidateRehearsal }, { resolveGatewayInstallEntrypoint }, { runUtf8CommandWithTimeout }, { resolveSqliteInspectionBudget }] = await Promise.all([
		...
		import("./testclaw-root-D9pI-rV5.mjs"),
		import("./update-check-pI_cEzua.mjs"),
		import("./update-candidate-rehearsal-atqC3wQQ.mjs"),
		import("./gateway-entrypoint-C_UIseX5.mjs"),
		import("./exec-DTHFtwAf.mjs"),
		import("./sqlite-readonly-worker-D8-3X6e-.mjs")
	]);
	...
	try {
		...
		if (result.code !== 0 || result.termination !== "exit" || result.outputLimitExceeded) {
			const { redactSupportString } = await import("./diagnostic-support-redaction-D7z-iR-Z.mjs");
	...
	const { UPDATE_POST_INSTALL_DOCTOR_RESULT_PATH_ENV, writeUpdatePostInstallDoctorResult } = await import("./update-doctor-result-BmvocCog.mjs");
