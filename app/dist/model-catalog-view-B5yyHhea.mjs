...
import { a as resolveAgentDir, d as resolveAgentWorkspaceDir, y as resolveNativeModelPrimary } from "./agent-scope-config-Dm8T0OhW.mjs";
...
import { a as resolveMergedModelProviderConfig, n as createModelProviderRouteOverrideResolver, t as createConfiguredProviderModelResolver } from "./model-provider-config-DE6LDhzP.mjs";
import { i as resolveProviderModelPolicySurface, r as resolveProviderModelCatalogId } from "./provider-model-routes-_ELjd8aW.mjs";
import { n as dedupeByKey, t as PREPARED_THINKING_POLICY } from "./provider-thinking-catalog-B0d_iUnw.mjs";
...
async function loadPreparedModelCatalogView(params) {
	if (params.kind === "status") {
		const { getPublishedPreparedModelCatalogOwnerSnapshot, loadPreparedModelCatalogOwnerSnapshot, materializePreparedModelCatalogOwner } = await import("./prepared-model-catalog-C1J9Stvc.mjs");
		const { getPreparedModelRuntimeAuthLabels, getPreparedModelRuntimeAuthStore } = await import("./prepared-model-runtime-auth-CvCN5U3s.mjs");
		const { formatModelCatalogAuthLabel } = await import("./model-catalog-auth-labels-DcmDQ-Up.mjs");
		const { resolveSessionRuntimeOverrideForProvider } = await import("./session-runtime-compat-BJL4o0TF.mjs");
		const { buildAgentRuntimeAuthPlan } = await import("./auth-BTlJ9X3P.mjs");
	...
	const { buildConfiguredModelCatalog } = await import("./model-selection-shared-DhGUnBI0.mjs");
	...
	if (params.preferredProvider && params.providerScoped && catalog.length > 0) {
		const { loadStaticManifestCatalogRowsForList } = await import("./list.manifest-catalog-63SqcwmY.mjs");
...
async function acquirePickerModelCatalogView(params) {
	...
	const { loadPreparedModelCatalogSnapshot } = await import("./prepared-model-catalog-C1J9Stvc.mjs");
	if (params.preferredProvider) {
		if (params.preferLiveProviderCatalog) {
			const { resolveDefaultAgentDir } = await import("./agent-scope-CbPqWO_3.mjs");
			const { resolvePluginMetadataSnapshot } = await import("./plugin-metadata-snapshot-BWOEJ_To.mjs");
			const { createPreparedModelCatalogProviderNormalizer } = await import("./model-catalog-provider-normalizer-CLOHYNd0.mjs");
