...
import { u as hasUnresolvedConfigPath } from "./resolution-facts-CSuKIPux.mjs";
import { n as listExplicitAgentWorkspaceDirs, t as listAgentWorkspaceDirs } from "./workspace-dirs-B1dBNTn1.mjs";
...
import { h as createPluginRegistryIdNormalizer } from "./plugin-metadata-snapshot-BdghSFzd.mjs";
...
import { n as isDangerousNetworkMode, r as normalizeNetworkMode } from "./network-mode-Bx3uuH84.mjs";
import { r as resolveGatewayAuthForConfig } from "./auth-resolve-Dtpx822I.mjs";
import { a as readInstalledPackageVersion } from "./package-update-utils-Bh6bkgOw.mjs";
...
import { r as listEffectiveGroupRouteBindings } from "./resolve-route-C_VACgEb.mjs";
import { i as resolveSandboxConfigForAgent } from "./config-DBSLaQuW.mjs";
import { c as resolveSessionToolsVisibility, s as resolveSandboxSessionToolsVisibility, t as createAgentToAgentPolicy } from "./session-visibility-WXo_iVWW.mjs";
import { t as getBlockedBindReason } from "./validate-sandbox-security-CK7xVx6D.mjs";
import { s as listDangerousPluginNodeCommands, t as DEFAULT_DANGEROUS_NODE_COMMANDS, u as resolveNodePairingCommandAllowlist } from "./node-command-policy-DonFHl1h.mjs";
import { t as listReadOnlyChannelPluginsForConfig } from "./read-only-CekTo0oS.mjs";
import { n as GATEWAY_CONTROL_PLANE_TOOLS } from "./dangerous-tools-Bc8D70-S.mjs";
import { t as describeBinding } from "./agents.binding-format-BRYI5aWJ.mjs";
import { r as resolveNativeSkillsEnabled } from "./commands-DEdbj4we.mjs";
import { t as inspectReadOnlyChannelAccount } from "./read-only-account-inspect-Bqp6zmdL.mjs";
import { r as resolveInternalHookSelection } from "./configured-BOL7g-bU.mjs";
import { t as resolveAllowedAgentIds } from "./hooks-policy-Ci1pPdqJ.mjs";
import { t as readHookInstalls } from "./installs-D3ZBJtHO.mjs";
import { t as inferParamBFromIdOrName } from "./model-param-b-DBLunvNX.mjs";
import { a as collectStateDeepFilesystemFindings, i as collectSandboxBrowserHashLabelFindings, o as readConfigSnapshotForAudit, s as listInstalledPluginDirs, t as collectIncludeFilePermFindings } from "./audit-extra.async-BVbEs8JF.mjs";
...
function formatHooksTokenReuseRemediation(reuse) {
	...
	return `Run ${formatCliCommand("testclaw doctor --fix")} to rotate a persisted hooks.token, then update external hook senders to use the new hook token.`;
}
...
function collectSyncedFolderFindings(params) {
	...
	if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) findings.push({
		...
		remediation: `Keep TESTCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("testclaw security audit --fix")}".`
	});
	return findings;
}
function collectSecretsInConfigFindings(cfg) {
	...
	if ((normalizeOptionalString(cfg.gateway?.auth?.password) ?? "") && !hasUnresolvedConfigPath(cfg, "gateway.auth.password")) findings.push({
		...
		remediation: "Prefer TESTCLAW_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk."
	});
...
function collectGatewayHttpSessionKeyOverrideFindings(cfg) {
	...
	findings.push({
		...
		detail: `${enabledEndpoints.join(", ")} accept x-testclaw-session-key for per-request session routing. Treat API credential holders as trusted principals.`
	});
	return findings;
}
...
function collectExposureMatrixFindings(cfg) {
	...
	if (controlPlaneContexts.length > 0) findings.push({
		...
		detail: `Found inbound policy="open" at:\n${openInboundPolicies.map((p) => `- ${p}`).join("\n")}\nControl-plane tool exposure contexts:\n${controlPlaneContexts.map((line) => `- ${line}`).join("\n")}\nPrompt injection in open conversations can trigger Assistant updates, plugin changes, or scheduled automation.`,
...
function collectLikelyMultiUserSetupFindings(cfg) {
	...
	if (mainGroupScopes.length > 0) findings.push({
		...
		remediation: "Use session.groupScope=\"per-group\" globally and remove binding overrides, or reserve \"main\" for rooms whose members you trust: https://docs.testclaw.ai/channels/groups#session-keys"
	});
	...
	findings.push({
		...
		detail: "Heuristic signals indicate this gateway may be reachable by multiple users:\n" + signals.map((signal) => `- ${signal}`).join("\n") + `\n${impactLine}\n${riskyContextsDetail}\nAssistant's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.testclaw.ai/gateway/multi-tenant-hosting`,
...
function collectAttackSurfaceSummaryFindings(cfg) {
	...
	return [{
		...
		detail: `groups: open=${group.open}, allowlist=${group.allowlist}\ntools.elevated: ${elevated ? "enabled" : "disabled"}\nhooks.webhooks: ${webhooksEnabled ? "enabled" : "disabled"}\nhooks.internal: ${internalHooksEnabled ? "enabled" : "disabled"}\nbrowser control: ${browserEnabled ? "enabled" : "disabled"}\ntrust model: personal assistant (one trusted operator boundary), not hostile multi-tenant on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.testclaw.ai/gateway/multi-tenant-hosting`
	}];
}
...
const loadPluginTrustPolicyDeps = createLazyPromise(() => Promise.all([
	import("./config-CtF2qSVL.mjs"),
	import("./tool-policy-CTuZMAuk.mjs"),
	import("./tool-policy-match-1UyY3Snx.mjs"),
	import("./tool-policy-CMCkxD-_.mjs"),
	import("./sandbox-tool-policy-CPM87leB.mjs")
...
async function analyzeNpmInstalls(installs, stateDir, directory) {
	const unpinned = installs.filter(([, record]) => typeof record.spec === "string" && !isPinnedRegistrySpec(record.spec)).map(([id, record]) => `${id} (${record.spec})`);
	const missingIntegrity = installs.filter(([, record]) => typeof record.integrity !== "string" || record.integrity.trim() === "").map(([id]) => id);
	const versionDrift = [];
...
async function collectPluginsTrustFindings(params) {
	...
	if (pluginDirs.length > 0) {
		...
		if (enabledExtensionPluginIds.length > 0) {
			...
			for (const context of contexts) {
				...
				const broadPolicy = deps.isToolAllowedByPolicies("__testclaw_plugin_probe__", policies);
	...
	if (npmPluginInstalls.length > 0) {
		...
		if (metadata.unpinned.length > 0) findings.push({
		...
		if (metadata.missingIntegrity.length > 0) findings.push({
		...
		if (metadata.versionDrift.length > 0) findings.push({
			...
			remediation: "Run `testclaw plugins update --all` (or reinstall affected plugins) to refresh install metadata."
		});
	}
	...
	if (npmHookInstalls.length > 0) {
		...
		if (metadata.versionDrift.length > 0) findings.push({
			...
			remediation: "Run `testclaw hooks update --all` (or reinstall affected hooks) to refresh install metadata."
		});
	}
	return findings;
}
