...
import { S as runWithGatewayIndependentRootWorkAdmission, f as isGatewayWorkAdmissionClosed } from "./gateway-work-admission-DeFm4gyw.mjs";
import { l as getSessionEventWakeAbortSignal } from "./heartbeat-wake-1H_xbiA3.mjs";
...
import { d as recordSessionHumanDirectMessage, p as recordSessionStateEventAsync } from "./session-state-events-DiPU1ldX.mjs";
import { i as updateSessionUpstreamLinkMarker, n as listWatchedSessionUpstreamLinks, r as readSessionUpstreamLink, t as deleteSessionUpstreamLink } from "./session-upstream-links-BXJyTmbg.mjs";
...
import { t as computeBackoffMs } from "./delivery-recovery.shared-CzYIEWau.mjs";
...
import { I as startSessionDeliveryRuntime, P as schedulePendingSessionDeliveries } from "./subagent-completion-admission.store-JZXyb7da.mjs";
...
import { r as assertQueuedConversationDeliveryAttemptAuthorized } from "./conversation-route-ownership-BQx_HRkx.mjs";
...
import "./server-idle-task-BriLt28Y.mjs";
import "./server-runtime-startup-services-CmCmJ---.mjs";
...
function startPendingOutboundDeliveryRecovery(params) {
	...
	const recover = () => {
		...
		const settled = runWithGatewayIndependentRootWorkAdmission(async () => {
			if (stopped) return;
			const { drainPendingDeliveriesCore, recoverPendingDeliveries } = await import("./delivery-queue-recovery-CNLMLLdP.mjs");
			const { deliverOutboundPayloadsInternal } = await import("./deliver-DwoP7GNN.mjs");
			if (stopped) return;
			...
			if (initialPass) {
				...
				initialPass = false;
				const { countPendingDeliveryQueueEntries } = await import("./delivery-queue-sqlite-4d-3GMK5.mjs");
...
function startPendingSessionDeliveryRuntime(params) {
	...
	const timer = setTimeout(() => {
		recovery = runWithGatewayIndependentRootWorkAdmission(async () => {
			const { deliverQueuedSessionDelivery, recoverPendingRestartContinuationDeliveries, settleQueuedSessionDelivery } = await import("./server-restart-sentinel-tHBB1KQy.mjs");
