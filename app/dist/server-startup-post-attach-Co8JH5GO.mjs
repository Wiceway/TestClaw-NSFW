...
import { i as getGatewayContextLifetime } from "./gateway-context-binding-VqB7gkMe.mjs";
...
import { u as hasRestartSentinel } from "./restart-sentinel-Csb-WRIg.mjs";
import { S as runWithGatewayIndependentRootWorkAdmission, a as getActiveGatewayRootWorkCount, s as getGatewayRestartDrainSignal } from "./gateway-work-admission-DeFm4gyw.mjs";
import { n as findCapabilityProviderEntry } from "./provider-registry-shared-CGngP_UC.mjs";
import { v as sweepSessionStateWatchNotices } from "./session-state-events-DiPU1ldX.mjs";
...
import { a as canReadDetailedUpdateMetadata, o as projectUpdateAvailable, r as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-CzKbLwW2.mjs";
import { n as wakeUpdateRunWatcher, t as startUpdateRunWatcher } from "./update-run-watcher-BU2_tguF.mjs";
import { t as createGatewayUpdateLifecycle } from "./update-check-lifecycle-CXkZRgsW.mjs";
import { t as scheduleGatewayIdleTask } from "./server-idle-task-BriLt28Y.mjs";
import { n as measureStartup } from "./server-startup-trace-CD8BamQh.mjs";
import { n as logGatewayReady, r as logGatewaySidecarsReady, t as beginMacOSSystemCaWarmupOnce } from "./system-ca-warmup-Dx8aa5Rp.mjs";
...
function scheduleContextCachePrewarm(params) {
	...
	const warm = async () => {
		if (stopped) return;
		const { prewarmContextWindowCacheAfterReady } = await import("./context-rhN1BJK7.mjs");
...
function dashboardDataPrewarmItems(cfg) {
	return [{
		name: "plugins",
		load: async () => {
			const { listManagedPlugins } = await import("./management-service-DQ6rIaMR.mjs");
...
async function hydrateConfiguredExternalCliAuth(params) {
	const deps = await params.deps ?? await Promise.all([
		...
		import("./external-cli-discovery-DLrTg2oX.mjs")
...
async function publishConfiguredModelRuntimeSnapshots(params) {
	const { refreshPreparedModelRuntimeSnapshots } = await import("./prepared-model-runtime-676SbNLR.mjs");
...
async function runGatewayStartupObservers(params) {
	...
	try {
		...
		const { withPluginHttpRouteRegistry } = await import("./http-registry-Bvw-kG41.mjs");
		if (params.isClosing?.()) return;
...
function scheduleTranscriptsSidecar(params) {
	...
	const start = () => {
		...
		if (service) withPluginRuntimeRegistryScope(params.getPluginRegistry(), () => service.start(config));
			...
			run: async (isStopped) => {
				const { createTranscriptsAutoStartService } = await import("./auto-start-MM9eA8NO.mjs");
				if (isStopped()) return;
		...
		stop: async () => {
			...
			const failures = (await Promise.allSettled([sidecar?.stop(), (async () => {
				if (!shutdownCapturePolicy) {
					const { prepareTranscriptCaptureDisable } = await import("./capture-operations--i-5Gonk.mjs");
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-B3GsU71S.mjs"));
const loadMainSessionRestartRecoveryMarkingModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-marking-UaWzJ1rb.mjs"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-D-l0Kfsa.mjs"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-De2pDgyb.mjs"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-3LY2EyP-.mjs"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-tHBB1KQy.mjs"));
...
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-CIX_Erbe.mjs");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			const { prepareInternalHooks } = await import("./loader-CvUs2Aex.mjs");
	...
	if (params.pluginRuntimeClaim?.isCurrent() !== false && params.shouldStartPluginServices?.() !== false) {
		...
		await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
			try {
				const { startPluginServices } = await import("./services-B8rkIUs1.mjs");
				await params.pluginRuntimeClaim?.waitForUnblocked();
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-D0GTRXy8.mjs");
			if (isStopped()) return;
...
const defaultGatewayPostAttachRuntimeDeps = {
	createHookRunner: async (...args) => (await import("./hooks-Bres9jjZ.mjs")).createHookRunner(...args),
	logGatewayStartup: async (params) => (await import("./server-startup-log-BLpp3vBs.mjs")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	createGatewayUpdateCheck: async (...args) => (await import("./update-startup-dF7jBvCY.mjs")).createGatewayUpdateCheck(...args),
	startGatewaySidecars,
	warmSystemCa: beginMacOSSystemCaWarmupOnce,
	loadSubagentRegistryActivation: async () => (await import("./subagent-registry-BuC6YZFG.mjs")).activateSubagentRegistry
};
	...
	const loadStartupPluginsIfNeeded = async () => {
		...
		const disposeUnattached = async () => {
			...
			if (loaded.pluginRegistry !== current) {
				...
				const { disposePluginRegistryInstances } = await import("./runtime-CjlkJ-tl.mjs");
