...
function startAssistantDatabaseIntegrityVerifier(options) {
	...
	const run = async () => {
		...
		queue.paths.clear();
		try {
			const { applyAssistantDatabaseVerificationResults, collectAssistantDatabaseVerifyTargets, runDatabaseVerifyWorker } = await import("./testclaw-database-verify.impl-C9qUxCR3.mjs");
			if (stopped) return;
	...
	return { stop: () => {
		...
		stopPromise = (async () => {
			try {
				...
				if (worker) {
					const { terminateDatabaseVerifyWorker } = await import("./testclaw-database-verify.impl-C9qUxCR3.mjs");
