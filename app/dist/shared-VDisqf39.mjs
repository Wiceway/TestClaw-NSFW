...
import { n as toAcpRuntimeErrorText } from "./error-text-DmZPcu3s.mjs";
import "./errors-6egNnljU.mjs";
import { n as commandReply } from "./command-gates-DBQdi4sc.mjs";
import { u as supportsAutomaticThreadBindingSpawn } from "./thread-bindings-policy-toE2K7ZV.mjs";
import { n as resolveAcpCommandChannel, r as resolveAcpCommandThreadId } from "./context-w9sZFMIx.mjs";
...
function readOptionValue(params) {
	...
	} else if (token.startsWith(`${params.flag}=`)) value = token.slice(`${params.flag}=`.length).trim();
...
function resolveDefaultSpawnThreadMode(params) {
	...
	if (!supportsAutomaticThreadBindingSpawn(channel)) return "off";
...
function resolveAcpHelpText() {
	return [
		...
		"",
		"Notes:",
		"- /acp spawn harness-id is an ACP runtime harness alias (for example codex), not an Assistant agents.list id.",
