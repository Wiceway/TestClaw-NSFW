...
function dedupePluginDiagnostics(diagnostics) {
	return dedupeByKey(diagnostics, (entry) => JSON.stringify([
		entry.level,
		entry.pluginId ?? "",
		entry.code ?? "",
		entry.message
...
function dedupeChannelPluginFailures(failures) {
	return dedupeByKey(failures, (entry) => JSON.stringify([
		entry.channelId,
		entry.pluginId ?? "",
		entry.message
	]));
...
function dedupeCompatibilityNotices(notices) {
	return dedupeByKey(notices, (entry) => JSON.stringify([
		entry.pluginId,
		entry.severity,
		entry.code ?? "",
		entry.message
...
function formatDetailedPluginHealth(snapshot) {
	...
	function appendSection(label, entries, format) {
		if (entries.length > 0) lines.push(`${label}: ${entries.length}`, ...entries.slice(0, 8).map(format));
	}
