...
import { i as removeProjectCheckoutReferenceInDatabase, t as ensureProjectRegistrySchema } from "./project-registry.kernel-CzroIb07.mjs";
...
async function registerPreparedProjectRegistry(prepared, lease, context, onRegistered) {
	...
	const { runWithAssistantStateLeaseWorker } = await import("./testclaw-state-lease-worker-storage-RJdU-sme.mjs");
...
async function selectStoredProjectRegistry(id, options = {}) {
	...
	return {
		...
		withRollback: async (run) => await withProjectCheckoutLifecycle(repoRoot, {
			...
			const { withAssistantStateLeaseWorkerAdmission } = await import("./testclaw-state-lease-worker-owner-qR9grwHl.mjs");
