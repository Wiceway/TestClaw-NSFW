...
import { a as resolveAgentDir, g as resolveDefaultAgentId, p as resolveAmbientOwnerAgentId } from "./agent-scope-config-Dm8T0OhW.mjs";
...
import { g as writeJsonAtomic, r as readDurableJsonFile } from "./json-files-BOBkrvx7.mjs";
...
import { i as listAvailableManifestContractPlugins, o as loadManifestContractSnapshot } from "./manifest-contract-eligibility-Cir-JbRF.mjs";
...
import { l as openAssistantAgentDatabase, o as disposeAssistantAgentDatabaseByPath } from "./testclaw-agent-db-DAdiee0a.mjs";
import { a as unregisterAssistantAgentDatabase, i as registerAssistantAgentDatabase } from "./testclaw-agent-db-registry-XnPsrvLm.mjs";
...
import { l as resolveOnboardingSetupTarget, n as applyOnboardingPrimaryModel, o as prepareAgentModelDefaults, r as applyOnboardingUtilityModel, s as projectAgentModelDefaults } from "./onboard-agent-target--U5T7XvY.mjs";
import { n as commitSetupInferenceActivation, r as restoreSetupInferenceConfig } from "./setup-inference-transition-2Mqt6miK.mjs";
import { n as WizardCancelledError } from "./prompts-DLsO8MlU.mjs";
import { t as isSetupCredentialReplacement } from "./setup-inference-credentials-Gt2h_Xht.mjs";
import { a as revalidateStableSetupInferenceOwner, u as activateSavedSetupCredential } from "./setup-inference-turn-DVFg2qoq.mjs";
import { y as summarizeMigrationItems } from "./migration-DepQtnwV.mjs";
import { s as writeMigrationReport } from "./migration-runtime-DXNo72DZ.mjs";
import { n as runWizardWithPromptNavigationScope } from "./navigation-prompter-DZbGfNMu.mjs";
...
const loadAuthChoiceModule = createLazyRuntimeModule(() => import("./auth-choice-DYz7InSv.mjs"));
const loadModelPickerModule = createLazyRuntimeModule(() => import("./model-picker-BRTFdrGD.mjs"));
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProviderCore }] = await Promise.all([import("./provider-auth-choices-CmvTQUjS.mjs"), import("./setup-registry-BknbEkOx.mjs")]);
...
async function runSetupModelAuthStep(params) {
	...
	if (authChoiceFromPrompt) {
		const [{ promptAuthChoiceGrouped: promptAuthChoice, isKeepCurrentAuthChoice: isKeepCurrentChoice }, { detectAvailableSetupProviderIds }] = await Promise.all([import("./auth-choice-prompt-HVg72TtT.mjs"), import("./provider-setup-availability-BY2TCPXd.mjs")]);
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const [{ promptCustomApiConfig }, { prepareCustomSetupCredentials }, { persistProviderAuthProfileBatch }] = await Promise.all([
				import("./onboard-custom-CqD40VxY.mjs"),
				import("./setup-inference-custom-CxFAWohU.mjs"),
				import("./provider-auth-persistence-DvRBtu1R.mjs")
			]);
	...
	const verify = async (candidate) => {
		...
		try {
			...
			if (candidate.authProfiles.length > 0) {
				const { saveSetupCredential, selectSetupCredential } = await import("./setup-inference-credentials-CQ8Gwqn-.mjs");
				const { projectSetupInferenceConfig } = await import("./setup-model-selection-DthnADDc.mjs");
...
const loadMigrationProviderRuntimeModule = createLazyRuntimeModule(() => import("./migration-provider-runtime-5vVHqEc5.mjs"));
const loadMigrationContextModule = createLazyRuntimeModule(() => import("./context-CKxv6OLa.mjs"));
const loadConfigPathsModule = createLazyRuntimeModule(() => import("./paths-CoqwxRra.mjs"));
...
async function runSetupMigrationImport(params) {
	const [{ applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig }, { createMigrationLogger, buildMigrationReportDir }, { assertApplySucceeded, assertConflictFreePlan, formatMigrationPreview, formatMigrationResult }, { resolveStateDir }, onboardHelpers] = await Promise.all([
		import("./onboard-config-BSOaaaHU.mjs"),
		loadMigrationContextModule(),
		import("./output-Cr8jv4Hs.mjs"),
		loadConfigPathsModule(),
		import("./onboard-helpers-ByjrA4hV.mjs")
	]);
