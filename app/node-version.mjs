...
export function formatUnsupportedNodeVersionMessage(version) {
  return [
    `Node ${version ?? "unknown"} is unsupported; Testclaw requires ${SUPPORTED_NODE_VERSIONS}.`,
    "npm can finish installing Testclaw without running its preinstall check; a successful install does not mean Node is supported.",
...
export function classifyUnsupportedNodeCommand(argv) {
  ...
  // Testclaw database writable and never starts a Gateway or a repair agent.
...
export function canRunTestclawNodeDiagnostics(value, hasNodeSqlite) {
  return (
    hasNodeSqlite &&
...
/** Checks Testclaw's supported release lines. Older Node lines with lossy SQLite TEXT reads are unsupported. */
export function isSupportedTestclawNodeVersion(value) {
...
// Worker bootstrap runs before Testclaw is transferred. Carry the canonical
