...
export async function resolveUpdatedNodeRuntime(
  ...
  if (env.TESTCLAW_NODE_UPDATE_RESPAWNED === "1" && !installCommand) {
    return null;
  }
  ...
  if (!installCommand) {
    process.stderr.write(
      "Install a compatible Node.js for Testclaw only and retry this command.\n" +
  ...
  if (
    ...
    if (!installCommand) {
      process.stderr.write(
        "testclaw: Node.js update failed; install a compatible Node.js manually.\n",
      );
    }
    return null;
  }
  if (!installCommand) {
    process.stderr.write("testclaw: Node.js updated. Retrying your command.\n");
  }
