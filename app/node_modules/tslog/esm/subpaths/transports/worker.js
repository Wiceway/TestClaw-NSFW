...
async function runnerFileExists(url) {
    try {
        const fs = await import("node:fs");
...
export function workerTransport(options = {}) {
     ...
     */
    function handleWorkerDeath(died, error) {
        if (worker !== died) {
        ...
        try {
            nativeConsoleMethod("error")(`tslog: worker transport "${options.name ?? "worker"}" thread died unexpectedly${workerGaveUp ? "; falling back to inline writes" : "; respawning on the next write"}`, error);
        }
        catch {
    ...
    async function inlineWrite(line) {
