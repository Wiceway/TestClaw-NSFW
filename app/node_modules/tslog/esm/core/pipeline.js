 ...
 */
export const PIPELINE_MASKED_ARGS = Symbol("tslog.pipeline.maskedArgs");
/**
 ...
 */
export function attachMaskedArgs(record, maskedArgs) {
 ...
 */
export function getMaskedArgs(record, metaProperty) {
    const attached = record[PIPELINE_MASKED_ARGS];
 ...
 */
export function runMiddleware(context, middleware) {
    let current = context;
 ...
 */
export function pretty(provider) {
 ...
 */
export function timestamp(inner) {
    return (record, settings) => {
        const meta = record[settings.meta.property];
 ...
 */
export function errors(inner, provider) {
    return (record, settings) => {
        const body = inner(record, settings);
        const maskedArgs = getMaskedArgs(record, settings.meta.property);
        const rendered = maskedArgs.filter((arg) => provider.isError(arg)).map((arg) => provider.prettyFormatErrorObj(arg, settings));
 ...
 */
export function resolveFormatter(format, provider, jsonLine) {
    if (format === "json") {
 ...
 */
export function defaultFormatter(settings, provider, jsonLine) {
