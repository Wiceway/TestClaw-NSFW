import { forceColorRequested, isBrowserEnvironment, isReactNativeEnvironment, isWorkerEnvironment, noColorRequested, resolveDefaultType, safeEnvGet, stdoutIsTTY, } from "../internal/environment.js";
...
export function devWarningsEnabled() {
...
const reportedContextStorages = new WeakSet();
// Callers gate on devWarningsEnabled() before building a message, so this only emits.
export function emitConfigWarning(message) {
    try {
 ...
 */
export function validateSettingsParam(settings) {
    if (settings == null) {
        return;
    }
    ...
    try {
        const storage = settings.contextStorage;
        if (storage != null) {
            let malformed = false;
            try {
            ...
            if (malformed && (strict || !reportedContextStorages.has(storage))) {
                try {
                    reportedContextStorages.add(storage);
                }
                catch {
 ...
 */
function resolveStackCapture(settings, type) {
...
export function normalizeSettings(settings) {
    ...
    return {
        ...
        pretty: {
            template: settings?.pretty?.template ?? "{{yyyy}}.{{mm}}.{{dd}} {{hh}}:{{MM}}:{{ss}}:{{ms}}\t{{logLevelName}}\t{{filePathWithLine}}{{nameWithDelimiterPrefix}}\t",
