 ...
 */
export function styleTokenToAnsi(token) {
 ...
 */
export function styleTokenToCss(token) {
 ...
 */
export const prettyLogStyles = Object.fromEntries(Object.entries(STYLE_PALETTE).map(([token, definition]) => [token, definition.ansi]));
/** Reverse lookup for {@link ansiToCssConsoleFormat}: SGR open code -> its CSS declaration + close code. */
const ANSI_OPEN_TO_CSS = new Map(Object.values(STYLE_PALETTE)
    .filter((definition) => definition.css != null && definition.ansi[0] !== definition.ansi[1])
 ...
 */
export function ansiToCssConsoleFormat(input) {
    const parts = [];
    const styles = [];
