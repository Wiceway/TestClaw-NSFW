 ...
 */
export function toFlatJsonObject(record, settings) {
 ...
 */
function buildFlat(record, settings) {
    const metaProperty = settings.meta.property;
    const json = settings.json;
    const meta = record[metaProperty];
    ...
    if (!recordIsSpreadError) {
        ...
        errors.push(stripMeta(record, metaProperty));
    ...
    if (pinoLeading) {
        const leadingObject = userFields["0"];
        delete userFields["0"];
    ...
    if (messageFirst) {
        ...
        delete userFields["0"];
        const trailingObject = userFields["1"];
        delete userFields["1"];
 ...
 * actually emits it.
 */
function isReservedHeadKey(key, json) {
 ...
 */
function writeHead(flat, meta, json, dateIso) {
    if (meta == null) {
        return;
    }
 ...
 */
function writeMeta(flat, meta, metaProperty, stable, dateIso, awk) {
    if (meta == null) {
        return;
    }
    const metaObj = meta;
    const body = { v: JSON_SCHEMA_VERSION };
 ...
 * // '{"message":"hi","level":"INFO","levelId":3,"time":"…","_logMeta":{"v":5,…}}'
 */
export function renderJson(record, settings) {
 ...
 * suite (tests/56).
 */
export function renderJsonUnplanned(record, settings) {
...
const linePlanCache = new WeakMap();
...
function isPlanStaticValue(value) {
...
function buildLinePlan(meta, settings) {
    const json = settings.json;
...
function planMatchesSettings(plan, settings) {
    const json = settings.json;
    return (plan.messageKey === json.messageKey &&
 ...
 */
export function __linePlanActive(settings) {
 ...
 */
function renderPlannedLine(record, settings) {
    const metaProperty = settings.meta.property;
    const meta = record[metaProperty];
    if (meta == null || !(meta.date instanceof Date)) {
        return undefined;
    }
    let plan = linePlanCache.get(settings);
    if (plan === false) {
        return undefined;
    }
    if (plan === undefined || !planMatchesSettings(plan, settings)) {
        ...
        linePlanCache.set(settings, built);
        if (built === false) {
            return undefined;
        }
        plan = built;
    ...
    if (chunk === undefined) {
        const head = `${JSON.stringify(plan.levelKey)}:${JSON.stringify(levelName)}` +
        ...
        for (const segment of plan.metaSegments) {
            const piece = typeof segment === "string"
                ? segment
                : segment.dyn === "levelId"
 ...
 */
export function jsonStringifyValue(value) {
 ...
 */
function containsAwkwardValue(value, ancestors = new WeakSet()) {
    ...
    for (const key of Object.keys(value)) {
        if (containsAwkwardValue(value[key], ancestors)) {
            return true;
        }
    }
    ancestors.delete(value);
 ...
 */
function deepSortKeys(value, awk, seen = new WeakSet()) {
    ...
    for (const key of Object.keys(value).sort()) {
        sorted[key] = deepSortKeys(value[key], awk, seen);
    }
    seen.delete(value);
    return sorted;
}
...
function stripMeta(record, metaProperty) {
    const out = {};
