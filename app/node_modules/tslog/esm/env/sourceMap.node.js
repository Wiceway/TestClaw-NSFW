...
const BASE64_VLQ_INDEX = Object.fromEntries([...BASE64_VLQ_CHARS].map((char, index) => [char, index]));
...
function requireNodeModule(name) {
    ...
    if (typeof getBuiltin === "function") {
        try {
            const resolved = getBuiltin(name);
...
function getFs() {
    if (!fsProbed) {
        fsProbed = true;
        cachedFs = requireNodeModule("node:fs");
    }
    return cachedFs;
}
...
function loadRawSourceMap(filePath, fs) {
    let fileContents;
    try {
    ...
    try {
        if (mappingUrl.startsWith("data:")) {
            ...
            return { raw: JSON.parse(decoded), mapDir: dirnameOf(filePath) };
        }
        ...
        return { raw: JSON.parse(fs.readFileSync(mapPath, "utf8")), mapDir: dirnameOf(mapPath) };
    }
    catch {
        return undefined;
    }
...
function getParsedSourceMap(filePath) {
 ...
 */
function parseRawMap(raw, mapDir, fs, depth = 0) {
            ...
            if (subRaw == null && section.url != null) {
                ...
                try {
                    ...
                    if (fs.existsSync(subPath)) {
                        subRaw = JSON.parse(fs.readFileSync(subPath, "utf8"));
 ...
 */
function resolveInParsedMap(map, line0, column0) {
    if (map.kind === "flat") {
...
function findSegment(segments, genColumn) {
 ...
 */
export function resolveOriginalPosition(filePath, line, column) {
...
export function clearSourceMapCacheForTests() {
 ...
 */
export function sourceMapResolutionEnabled() {
