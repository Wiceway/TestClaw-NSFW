 ...
 *
 * Routes models that need Testclaw-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
import type { Api, Model, StreamFn } from "@testclaw/llm-core";
...
function hasTestclawTransportRequirement(model: Model): boolean {
...
export function resolveTransportAwareSimpleApi(api: Api): Api | undefined {
  if (OPENAI_RESPONSES_APIS.has(api)) {
    const alias = `testclaw-${api}-transport` as Api;
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasTestclawTransportRequirement(model)) {
    return undefined;
  }
...
export function createTestclawTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Testclaw's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same Testclaw request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
