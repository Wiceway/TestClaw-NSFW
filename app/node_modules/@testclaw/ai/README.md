...
Testclaw. The package supports isolated runtime instances; importing it does not
...
`@testclaw/ai/event-stream`, `@testclaw/ai/transports`, and
`@testclaw/ai/validation`. No second Testclaw runtime package is required.

Provider ids, credentials, model catalogs, retries, and failover remain
application concerns. Testclaw supplies those policies around this package.
...
The explicit `@testclaw/ai/internal/anthropic`, `google-model-family`, `openai`,
`openai-completions-compat`, `openai-responses-payload-policy`, `retry-after`, `runtime`, `shared`, and
`tool-schema` subpaths exist for the Testclaw application itself.
They carry no semver guarantee and can change or disappear in any release; do
not depend on them outside Testclaw.
