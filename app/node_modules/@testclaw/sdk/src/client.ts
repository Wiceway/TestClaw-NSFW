...
import { asRecord } from "@testclaw/normalization-core/record-coerce";
import { readNonEmptyStringPreservingWhitespace as readNonEmptyString } from "@testclaw/normalization-core/string-coerce";
...
// High-level Testclaw SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the Testclaw SDK client. */
export type TestclawOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: TestclawTransport;
};

function resolveGatewayUrl(options: TestclawOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Testclaw Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Testclaw Gateway yet`);
}
...
type RunTerminalSource = { kind: "canonical" } | { kind: "chat"; eventType: TestclawEvent["type"] };
...
function readChatProjection(event: TestclawEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: TestclawEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: TestclawEvent): boolean {
  return (
...
export class Testclaw {
  ...
  private readonly transport: TestclawTransport;
  private readonly normalizedEvents = new EventHub<TestclawEvent>();
  private readonly replayByRunId = new Map<string, TestclawEvent[]>();
  ...
  constructor(options: TestclawOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: TestclawEvent) => boolean): AsyncIterable<TestclawEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: TestclawEvent) => boolean,
  ): AsyncIterable<TestclawEvent> {
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("Testclaw SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: TestclawEvent): TestclawEvent | undefined => {
    ...
    const matches = (event: TestclawEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: TestclawEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): TestclawEvent[] {
...
export class Agent {
  constructor(
    private readonly client: Testclaw,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: Testclaw) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<TestclawEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Testclaw,
