...
```ts
import { BoundedReplayBuffer } from "@testclaw/libterminal/stream";
import { decodeTerminalFrame } from "@testclaw/libterminal/protocol";
```
...
- `@testclaw/libterminal`: universal terminal types and errors
- `@testclaw/libterminal/protocol`: terminal protocol v2 codecs
- `@testclaw/libterminal/stream`: bounded replay, fanout, and batching
- `@testclaw/libterminal/browser`: Ghostty WASM terminal integration and terminal hub client
- `@testclaw/libterminal/node`: local PTY, raw stdin, and asset helpers
- `@testclaw/libterminal/worker`: Worker-compatible WebSocket bridging
- `@testclaw/libterminal/worker-assets`: optional Worker-safe Ghostty asset payloads
- `@testclaw/libterminal/testing`: deterministic terminal test doubles
...
```ts
import { createGhosttyTerminal } from "@testclaw/libterminal/browser";
...
```ts
import { createTerminalDefaultColorQueryResponder } from "@testclaw/libterminal/browser";
...
```ts
import { TerminalHubClient } from "@testclaw/libterminal/browser";
...
```ts
import { attachLocalStdio, spawnLocalPty } from "@testclaw/libterminal/node";
...
```ts
import { bridgeWebSockets } from "@testclaw/libterminal/worker";
...
```ts
import { GHOSTTY_ASSET_PATHS, readGhosttyWorkerAsset } from "@testclaw/libterminal/worker-assets";
...
`@testclaw/libterminal/protocol` owns terminal protocol v2 codecs and golden
