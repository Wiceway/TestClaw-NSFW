...
import { isLoopbackIpAddress, type ParsedIpAddress } from "@testclaw/net-policy/ip";
import { isWssUrl } from "@testclaw/net-policy/url-protocol";
  ...
  if (!isSecureWebSocketUrl(params.url, { allowPrivateWs })) {
    ...
    throw new GatewayWebSocketTransportConfigurationError(
      `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
        ...
        (allowPrivateWs
          ? ""
          : "Break-glass (trusted private networks only): set TESTCLAW_ALLOW_INSECURE_PRIVATE_WS=1. ") +
        "Run `testclaw doctor --fix` for guidance.",
    );
  }
