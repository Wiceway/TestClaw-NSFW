import { asNullableRecord as readRecord } from "@testclaw/normalization-core/record-coerce";
...
export function readSessionMessageSequence(
  ...
  const metadata = readRecord(readRecord(message)?.["__testclaw"]);
...
export function readSessionMessageIdentity(
  ...
  const metadata = readRecord(record["__testclaw"]);
  ...
  const fallbackRunId = normalizeSessionProjectionRunId(
    readRecord(record.testclawStreamFallback)?.runId,
  );
...
export function readAssistantStreamSegmentIdentity(
  ...
  const fallback = readRecord(record?.testclawStreamFallback);
...
export function isLocallyOptimisticSessionMessage(message: unknown): boolean {
  ...
  if (readRecord(record?.testclawStreamFallback)) {
    return false;
  }
  const metadata = readRecord(record?.["__testclaw"]);
...
export function createSessionProjectionEntry(
  ...
  const fallback = readRecord(readRecord(message)?.testclawStreamFallback);
