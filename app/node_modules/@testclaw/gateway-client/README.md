# `@testclaw/gateway-client`

Reference WebSocket client for the Testclaw Gateway protocol. It provides the
connection state machine used by Testclaw's own Node and browser clients:
...
[Gateway protocol specification]((документация продукта)/gateway/protocol) for
...
Package versions follow the Testclaw calendar release train: `YYYY.M.PATCH`,
including the Testclaw prerelease suffix when applicable. The package version is
...
```bash
npm install --save-exact @testclaw/gateway-client@2026.8.1 @testclaw/gateway-protocol@2026.8.1
```

See the canonical [installation guide]((документация продукта)/gateway/clients#install-the-packages)
for package/wire-version rules and recovery from reserved `0.0.0` artifacts.
Test it with the Gateway version you deploy; the root `testclaw` CLI has its own
...
storage through `GatewayClientHostDeps`; the package does not load Testclaw's
...
- `@testclaw/gateway-client` exports the Node `GatewayClient`, device-auth
  helpers, readiness helpers, and timeout utilities.
- `@testclaw/gateway-client/browser` exports the browser-safe protocol client,
...
- `@testclaw/gateway-client/readiness` exports helpers that delay client startup
  until the event loop can process Gateway IO.
- `@testclaw/gateway-client/timeouts` exports timeout constants and safe timer
  resolution helpers.
- `@testclaw/gateway-client/websocket-data` converts every Node `ws` raw-data
  shape to UTF-8 text.

## Node quickstart

```ts
import { GatewayClient } from "@testclaw/gateway-client";
import { PROTOCOL_VERSION } from "@testclaw/gateway-protocol/version";

const connected = Promise.withResolvers<void>();
const client = new GatewayClient({
  url: "ws://127.0.0.1:18789",
  token: process.env.TESTCLAW_GATEWAY_TOKEN,
...
[device identity and pairing]((документация продукта)/gateway/protocol#device-identity-and-pairing).
...
Import `@testclaw/gateway-client/browser` when the host owns the WebSocket
...
[Gateway protocol specification]((документация продукта)/gateway/protocol#client-constants).
...
The retry supervisor and the small `@testclaw/net-policy/ip` implementation are
...
`ws`, `@testclaw/gateway-protocol`, and `ipaddr.js` remain external in the
published distribution. Consumers should import protocol types and constants
from `@testclaw/gateway-protocol`, not from bundled implementation paths.
