...
export type MemoryIndexIdentityState =
  ...
  | ({ status: "mismatched"; reason: string } & (
      | {
          ...
          // matches, so a pending Testclaw chunking upgrade cannot mask a narrowed
...
export function resolveMemoryIndexIdentityDiagnostic(
  ...
  if (
    ...
    identity.owner === "testclaw"
  ) {
    return { status: "missing", reason, code: "metadata_missing", owner: "testclaw" };
  }
...
export function formatMemoryIndexRebuildGuidance(
  ...
  const command = `testclaw memory status --index${agentId?.trim() ? ` --agent ${agentId.trim()}` : ""}`;
...
export function resolveMemoryIndexSearchDiagnostic(
  ...
  const cause =
    diagnostic.owner === "configuration"
      ...
      : diagnostic.code === "metadata_missing"
        ...
        : newerIndex
          ? diagnostic.reason
          : `this Testclaw version changed the memory index format (${diagnostic.reason}); no configuration change is needed`;
  ...
  return {
    ...
    action: newerIndex
      ? `Tell the user to upgrade Testclaw or reindex explicitly: ${guidance}`
    ...
    staleness: {
      ...
      action: newerIndex
        ? `Upgrade Testclaw or reindex explicitly: ${guidance}`
        : `Run: ${guidance}`,
    },
  };
}
