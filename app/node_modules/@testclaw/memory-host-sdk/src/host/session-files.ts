...
import {
  ...
  isIncognitoTestclawAgentSqlitePath,
...
} from "./testclaw-runtime-session.js";
...
export { readTranscriptStatsBatchReadOnlySync } from "./testclaw-runtime-session.js";
...
function isDreamingNarrativeGeneratedRecord(record: unknown): boolean {
  ...
  if (
    ...
    candidate.customType === "testclaw:bootstrap-context:full" &&
 ...
 * Testclaw-owned usage-counted transcript in the canonical agent sessions dir.
 */
...
export async function buildSessionEntry(
  ...
  const prepare = async () => {
    ...
    if (
      ...
      !isIncognitoTestclawAgentSqlitePath(identity.storePath, { agentId: identity.agentId })
    ) {
