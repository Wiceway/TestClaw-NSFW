...
Small, framework-agnostic router for Testclaw UI surfaces. It handles route
...
```sh
pnpm add @testclaw/uirouter
# or
npm install @testclaw/uirouter
# or
yarn add @testclaw/uirouter
```
...
```ts
import { createRouter, definePage } from "@testclaw/uirouter";
...
```ts
import type { RouterHistory, RouteLocation } from "@testclaw/uirouter";
...
```ts
import { definePage, notFound, redirect } from "@testclaw/uirouter";
