...
Deterministic local messaging-channel mocks for Testclaw QA.

`crabline` is config-driven, CI-friendly, and deliberately has no `testclaw`
...
that the Crabline channel driver starts so a real Testclaw channel plugin can
...
fixture-level local mock provider feature. They let the real Testclaw channel
...
The `script` adapter can bridge any Testclaw channel represented by the
...
`serve` starts local provider servers for real Testclaw channel plugins. This is
...
provider server, or `startTestclawCrablineAdapter`. Crabline awaits the callback
...
import { startFeishuServer } from "@testclaw/crabline";
...
server catalog and Testclaw bridge do not register Feishu.
...
Testclaw Discord plugin: bot identity, `/gateway/bot`, JSON Gateway
...
- `baseUrl`: Testclaw `channels.mattermost.baseUrl` / `MATTERMOST_URL`
- `botToken`: Testclaw `channels.mattermost.botToken` / `MATTERMOST_BOT_TOKEN`
...
- `baseUrl`: Testclaw `channels.matrix.homeserver`
- `accessToken`: Testclaw `channels.matrix.accessToken`
- `botUserId`: Testclaw `channels.matrix.userId`
...
- `endpoints.apiRoot`: set Testclaw's Slack API override / `SLACK_API_URL` to
  this value
- `botToken`: set Testclaw `channels.slack.botToken` to this value
- `signingSecret`: set Testclaw `channels.slack.signingSecret` to this value
- `adminToken`: send this as the `X-Crabline-Admin-Token` header when posting
  test user messages
...
- `endpoints.apiRoot`: set Testclaw `channels.telegram.apiRoot` to this value
- `botToken`: set Testclaw `channels.telegram.botToken` to this value
- `adminToken`: send this as the `X-Crabline-Admin-Token` header when posting
  test user messages
- `endpoints.adminInboundUrl`: authenticated POST endpoint for test user
  messages; Testclaw reads them through Telegram `getUpdates`
- `recorderPath`: JSONL file of local provider API/admin traffic
...
- `botToken`: Testclaw `channels.zalo.botToken` / `ZALO_BOT_TOKEN`
...
The server implements the Zalo Bot API subset used by Testclaw: `getMe`,
...
directly. Testclaw-specific endpoint, config, and target mapping remain in the
...
paths, and Testclaw live-vs-mock guidance.
