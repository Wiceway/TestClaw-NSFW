...
Crabline is a local mock service for Testclaw channel contracts. It has two
surfaces:

- fixture-level local mocks used directly by the Crabline CLI
- local provider servers that real Testclaw channel plugins can target

Live channel testing belongs in Testclaw's live channel adapters. Crabline
...
for `adapter: script`, where Crabline needs to know which Testclaw channel the
...
labels the Testclaw channel represented by the bridge; it does not allow the
...
Crabline does not ship a generic Testclaw gateway command bridge. Supply and
...
Import `startFeishuServer` from `@testclaw/crabline`. Supply `tls: { key, cert }`
...
The ordinary Testclaw text reply uses a native `post`; text-only qualification
...
distributed quotas or retry policy. The CLI catalog and Testclaw bridge remain
...
test-only voice CA only in the isolated client process. Testclaw-specific
endpoint injection and target translation belong to the Crabline Testclaw
bridge and the Testclaw Discord plugin; the provider server itself implements
Discord's public protocol subset and contains no Testclaw behavior.
...
Discord provider server exists so the real Testclaw Discord plugin can use
...
Use the manifest's `baseUrl` and `botToken` as Testclaw's Mattermost endpoint
...
`true`. The Testclaw bridge does this automatically.
...
targets require the later Testclaw QA wiring step.

The server itself is provider-shaped and has no Testclaw runtime dependency. It
...
subset. Testclaw configuration and QA target mapping remain in the separate
Testclaw bridge.
...
Use the manifest's `baseUrl`, `accessToken`, and `botUserId` as Testclaw's
Matrix homeserver, access token, and user ID. Set encryption to `false` and,
because the server is loopback HTTP, enable
`channels.matrix.network.dangerouslyAllowPrivateNetwork`. The Testclaw bridge
...
the Matrix API. Testclaw configuration and QA target mapping remain in the
separate Testclaw bridge.
...
Local provider servers sit below Testclaw's normal channel adapters. QA starts
the server, writes the emitted runtime manifest into Testclaw config/env, and
then Testclaw talks to the local provider instead of the public provider.
...
- `endpoints.apiRoot`: Testclaw Slack Web API root / `SLACK_API_URL`
- `botToken`: Testclaw `channels.slack.botToken`
- `signingSecret`: Testclaw `channels.slack.signingSecret`
...
Testclaw consumes that message through Slack Events API shape; outbound adapter
...
accepts typing, receipt, and reaction RPCs. Testclaw-specific config and target
mapping live in Crabline's Testclaw bridge, outside the provider server.
...
- `endpoints.apiRoot`: Testclaw `channels.telegram.apiRoot`
- `botToken`: Testclaw `channels.telegram.botToken`
- `adminToken`: value for the `X-Crabline-Admin-Token` header on admin ingress
- `endpoints.adminInboundUrl`: authenticated admin ingress for test user messages
- `recorderPath`: JSONL provider traffic recorder
...
Testclaw consumes that message through Telegram `getUpdates`; outbound adapter
...
also recorded as normalized accepted-send evidence for the Testclaw bridge.
Group outbound uses sender-key `skmsg` encryption and is outside this supported
subset, so Testclaw Crabline outbound targets are direct users only. Group
...
Testclaw bridge callers should post injected user messages with the
`providerUrl`, `providerHeaders`, and `providerBody` returned by
`createTestclawCrablineInbound()`.
...
For an Testclaw bridge voice-note fixture, pass exactly one inline audio
...
`botToken` as `channels.zalo.botToken` or `ZALO_BOT_TOKEN`. The Testclaw bridge
...
- Testclaw WhatsApp bridge users: `15551234567@s.whatsapp.net`
  (legacy `15551234567@c.us` inputs are accepted and canonicalized)
- Testclaw WhatsApp bridge groups: `120363001234567890@g.us`
...
Testclaw bridge QA targets reserve the exact forms `dm:<id>`, `group:<id>`,
...
processes. `runTestclawCrablineProviderReadiness` probes the selected local
provider API; it does not claim that Testclaw started or completed a channel
roundtrip. Real Testclaw channel proof comes from QA scenarios that launch the
...
For release or live verification, use Testclaw's live driver:
