...
[![npm](https://img.shields.io/npm/v/@testclaw/fs-safe.svg?color=10b981&label=npm)](https://www.npmjs.com/package/@testclaw/fs-safe)
[![ci](https://github.com/testclaw/fs-safe/actions/workflows/ci.yml/badge.svg)](https://github.com/testclaw/fs-safe/actions/workflows/ci.yml)
[![node](https://img.shields.io/node/v/@testclaw/fs-safe.svg?color=10b981)](https://nodejs.org)
[![license](https://img.shields.io/npm/l/@testclaw/fs-safe.svg?color=10b981)](LICENSE)
...
| **`@testclaw/fs-safe`** | **✓** | **✓** | **✓** | **Linux atomic; others best-effort** | **✓ (ZIP/TAR/gzip/zstd/bzip2)** |
...
```ts
import { configureFsSafeNative } from "@testclaw/fs-safe";
...
and Testclaw Python environment names emit one `FS_SAFE_PYTHON_DEPRECATED`
...
contract. Low-level helpers that Testclaw needs to compose higher-level APIs are grouped under
`@testclaw/fs-safe/advanced` instead of being separate public leaf contracts.

| Subpath | Contents |
|---|---|
| `@testclaw/fs-safe/root` | `root()`, `Root`, `RootDefaults`, and root-bounded walking with pruning/error markers |
| `@testclaw/fs-safe/config` | process-global native helper and lock defaults |
| `@testclaw/fs-safe/path` | canonical path checks: `isPathInside`, `safeRealpathSync`, `isNotFoundPathError`, `isSymlinkOpenError` |
| `@testclaw/fs-safe/json` | `tryReadJson`, `readJson`, `readJsonIfExists`, `writeJson`, sync variants |
| `@testclaw/fs-safe/output` | `writeExternalFileWithinRoot` for external libraries that need a temp output path |
| `@testclaw/fs-safe/store` | `fileStore`, `fileStoreSync`, and `jsonStore` |
| `@testclaw/fs-safe/secret` | sync/async strict and try-style secret reads, atomic replace, and create-only secret writes |
| `@testclaw/fs-safe/atomic` | `replaceFileAtomic`, `replaceFileAtomicSync`, `replaceDirectoryAtomic`, `movePathWithCopyFallback` |
| `@testclaw/fs-safe/durability` | pinned directory identities, strict directory sync, durable nested-directory creation, exclusive publication, streaming and synchronous SHA-256, provenance receipts, and sync-failure policy |
| `@testclaw/fs-safe/temp` | `tempWorkspace`, `tempWorkspaceSync`, `withTempWorkspace`, `resolveSecureTempRoot` |
| `@testclaw/fs-safe/secure-file` | fd-pinned absolute file reads with owner, mode, ACL, trusted-dir, size, and timeout checks |
| `@testclaw/fs-safe/file-lock` | async/sync sidecar locks, root-bounded sidecars, ownership verification, and stale policy |
| `@testclaw/fs-safe/permissions` | POSIX mode and Windows ACL inspection, raw owner/ACE facts, private-directory creation, and remediation helpers |
| `@testclaw/fs-safe/walk` | budget-bounded directory walking with symlink policy, filters, and truncation accounting; not root-bounded |
| `@testclaw/fs-safe/copy` | directory copying with `clone: "auto"`, `"always"`, or `"never"`; native APFS, Btrfs, ReFS, XFS, and ZFS cloning, portable byte copying, and clone metadata; see [directory copying](docs/copy.md) |
| `@testclaw/fs-safe/archive` | policy-driven ZIP/TAR extraction, clamp/filter policy, metadata/path-depth limits, gzip/zstd/bzip2 support, and bounded entry reads |
| `@testclaw/fs-safe/advanced` | lower-level composition helpers such as path scopes, root-file open, bounded descriptor reads, [borrowed-handle and descriptor copying](docs/copy.md#borrowed-filehandle-transfers), [complete byte-window writes](docs/advanced.md#borrowed-handle-writes), [exact directory identity](docs/directory-identity.md), [case probing](docs/path-case.md), [suffix-alias probing](docs/path-suffix-aliases.md), [in-place writes](docs/in-place-write.md), [versioned install-ID encoding](docs/install-path.md#safepathsegmenthashedv2), filename sanitizing, temp-file targets, sibling-temp writes, local-root readers, regular-file helpers, `pathExists`, and `withTimeout`; less stable than focused public subpaths |
| `@testclaw/fs-safe/errors` | `FsSafeError`, closed codes/categories, causes, and operation-specific details receipts |
| `@testclaw/fs-safe/types` | shared types: `DirEntry`, `PathStat`, … |
| `@testclaw/fs-safe/test-hooks` | hooks the test suite uses to inject races; registration requires `NODE_ENV=test` or `VITEST=true` |
| `@testclaw/fs-safe/guest` | Python source and exit constants for caller-launched filesystem operations in Linux/macOS guests without Node; see the [guest protocol and trust boundary](docs/guest.md) |
...
```ts
import { readJson, tryReadJson } from "@testclaw/fs-safe/json";
...
```ts
import { ensureDurableDirectory, pinDirectory } from "@testclaw/fs-safe/durability";
...
```ts
import { replaceFileAtomic } from "@testclaw/fs-safe/atomic";
...
```ts
import { fileStore } from "@testclaw/fs-safe/store";
...
```ts
import { tempFile } from "@testclaw/fs-safe/advanced";
...
```ts
import { extractArchive, resolveArchiveKind } from "@testclaw/fs-safe/archive";
...
```ts
import { pathScope } from "@testclaw/fs-safe/advanced";
