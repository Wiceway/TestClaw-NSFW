...
`@testclaw/fs-safe/path` is the lower-level lexical and canonical path surface — useful when you have your own logic that needs to know "is this path inside that directory" without going through `root()` or `pathScope()`.
...
```ts
import { isPathInsideWithRealpath, isNotFoundPathError } from "@testclaw/fs-safe/path";
