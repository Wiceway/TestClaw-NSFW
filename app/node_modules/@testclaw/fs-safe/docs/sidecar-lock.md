...
```ts
import { acquireFileLockSync } from "@testclaw/fs-safe/file-lock";
...
- `createAsyncLock` from `@testclaw/fs-safe/advanced` — in-process serialization for a single Node process.
