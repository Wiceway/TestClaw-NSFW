...
`@testclaw/fs-safe` declares seven exact-version optional packages for Linux
...
The equivalent environment variables are `FS_SAFE_NATIVE_MODE` and `TESTCLAW_FS_SAFE_NATIVE_MODE`. Accepted values are `auto`, `off`, `require`, `true`, `false`, `on`, `never`, `required`, `1`, and `0`.
...
| `TESTCLAW_FS_SAFE_PYTHON_MODE` | `TESTCLAW_FS_SAFE_NATIVE_MODE` |
| `pythonPath`, `FS_SAFE_PYTHON`, and the Testclaw interpreter-path aliases | Remove; prebuilt bindings do not use an interpreter path |
