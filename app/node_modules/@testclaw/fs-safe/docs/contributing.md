...
The `fs-safe` repo lives at [github.com/testclaw/fs-safe](https://github.com/testclaw/fs-safe). Contributions welcome — issues, bug reports, focused PRs.

## Local setup

```bash
git clone https://github.com/testclaw/fs-safe.git
cd fs-safe
pnpm install
```
...
[benchmark guide](https://github.com/testclaw/fs-safe/tree/main/benchmarks) for native/fallback runs, per-call
...
platform packages before publishing `@testclaw/fs-safe`, verifies every registry
...
`testclaw/fs-safe` and `release.yml`. A new platform package must be created and
...
Suspected security issues belong in private disclosure first. See [`SECURITY.md`](https://github.com/testclaw/fs-safe/blob/main/SECURITY.md) in the repo for the current contact path. Don't open a public issue for a credential-stealing or sandbox-escape bug — coordinate the disclosure first.

## License

By contributing you agree that your contributions are licensed under the project's [MIT license](https://github.com/testclaw/fs-safe/blob/main/LICENSE).
