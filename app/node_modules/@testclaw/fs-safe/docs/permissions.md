...
`@testclaw/fs-safe/permissions` contains the curated mode and permission inspection helpers used by secure file reads and by applications that want to report actionable permission problems.
...
} from "@testclaw/fs-safe/permissions";
...
The low-level Windows ACL parser and `icacls` command builders live in `@testclaw/fs-safe/advanced`:
...
`@testclaw/fs-safe/permissions` and `@testclaw/fs-safe/advanced`. Built-in
...
```ts
import { readOwnerAndDacl } from "@testclaw/fs-safe/permissions";
...
import { createPrivateDirectory } from "@testclaw/fs-safe/permissions";

const sqliteDirectory =
  "C:\\Users\\me\\AppData\\Local\\Testclaw\\private-databases";
