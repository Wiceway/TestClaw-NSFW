...
`@testclaw/fs-safe/copy` materializes independent, caller-owned directory trees. `copyTree` prefers native copy-on-write operations by default, can require cloning, or can copy regular file bytes without cloning or copy offload.

```ts
import { copyTree, createCloneSource, probeTreeClone } from "@testclaw/fs-safe/copy";
...
`copyFileHandle` from `@testclaw/fs-safe/advanced` copies bytes between two
...
import { copyFileHandle } from "@testclaw/fs-safe/advanced";
...
```ts
import { copyFileDescriptorSync } from "@testclaw/fs-safe/advanced";
