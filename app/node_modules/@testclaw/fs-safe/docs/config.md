...
`TESTCLAW_FS_SAFE_NATIVE_MODE` is accepted as an alias. Programmatic overrides via `configureFsSafeNative` always win.

### Python-helper migration bridge

Version 0.5 detects the former `FS_SAFE_PYTHON_MODE`, `FS_SAFE_PYTHON`,
`TESTCLAW_FS_SAFE_PYTHON_MODE`, `TESTCLAW_FS_SAFE_PYTHON`,
`TESTCLAW_PINNED_PYTHON`, and `TESTCLAW_PINNED_WRITE_PYTHON` names. It emits one
