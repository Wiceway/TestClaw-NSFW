...
The creation-policy work is tracked in [#482](https://github.com/testclaw/fs-safe/issues/482).

Use `createDirectory()`, `createDirectorySync()`, and `createFileSync()` from
`@testclaw/fs-safe/advanced` when an existing, trusted parent should receive
