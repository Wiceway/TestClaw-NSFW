...
`@testclaw/fs-safe/json` is the standalone JSON surface: strict and lenient read variants plus atomic JSON writes.
...
const result = readRootJsonObjectSync({
  ...
  relativePath: "plugin/testclaw.plugin.json",
...
For atomic text writes, use [`writeTextAtomic`](atomic.md) from `@testclaw/fs-safe/atomic`. For in-process serialization, use `createAsyncLock` from the advanced surface, or prefer [`jsonStore`](json-store.md) when you want a JSON-specific read-modify-write helper.
