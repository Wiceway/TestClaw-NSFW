...
`@testclaw/fs-safe/guest` exports Python 3 source for filesystem operations in
...
This is the filesystem engine extracted from Testclaw's sandbox bridge. The
...
definition. Testclaw's workspace bootstrap consumes this same fragment when
...
import { GUEST_FILESYSTEM_PYTHON } from "@testclaw/fs-safe/guest";
...
file from a private staging directory. Staging names retain the `.testclaw-*`
