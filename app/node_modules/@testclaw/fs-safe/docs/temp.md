...
`@testclaw/fs-safe/temp` is the stable temp surface: private temp **workspaces** with auto-cleanup plus the secure per-user temp root the helpers default to.
...
```ts
import { tempWorkspace } from "@testclaw/fs-safe/temp";
...
and sibling-temp helpers live behind `@testclaw/fs-safe/advanced`. They are
...
```ts
import { withTempFile } from "@testclaw/fs-safe/advanced";
...
```ts
import { writeViaSiblingTempPath } from "@testclaw/fs-safe/advanced";
...
```ts
import { resolveSecureTempRoot } from "@testclaw/fs-safe/temp";
...
} from "@testclaw/fs-safe/secure-temp-root";
```
