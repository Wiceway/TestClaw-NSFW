---
title: Advanced
description: "Lower-level composition helpers under @testclaw/fs-safe/advanced. Less stable than focused public subpaths."
---

# `@testclaw/fs-safe/advanced`

Composition primitives that Testclaw uses to build higher-level APIs. They are public — semver applies — but treated as a less stable surface than the focused subpaths (`root`, `json`, `store`, `temp`, `archive`, `durability`, `errors`). Reach for them only when you are building a primitive of your own and the focused subpaths do not cover it.
...
import {
  ...
  // …
} from "@testclaw/fs-safe/advanced";
```
...
import { readFileDescriptorBoundedSync } from "@testclaw/fs-safe/advanced";
...
import { writeFileWindowFully } from "@testclaw/fs-safe/advanced";
...
| `tempFile`, `withTempFile`, `TempFile`, `buildRandomTempFilePath`, `sanitizeTempFileName` | [temp.md](temp.md) | One-file temp primitive; prefer `tempWorkspace` from `@testclaw/fs-safe/temp` for the stable surface. |
...
Items in this surface can change shape between minor versions if a higher-level primitive needs them to. Pin to a minor version if you depend on a specific helper, or open an issue at the [GitHub repo](https://github.com/testclaw/fs-safe) and we will discuss promoting it to a focused subpath.
