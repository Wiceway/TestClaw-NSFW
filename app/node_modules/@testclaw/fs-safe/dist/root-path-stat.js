...
async function statObservedPathInRoot(root, resolvedPath, proof, receipt) {
    ...
    try {
        if (receipt) {
            ...
            try {
                observed = comparison.run(() => assertStatObservationSync(bigint => {
                ...
                if (isNotFoundPathError(error)) {
                    throw await missingObservedFileError(error, resolvedPath, "stat" in receipt.target ? receipt.target.stat : undefined, () => assertRootPathObservationReceiptCurrent(root, receipt), proof);
                }
                if (comparison.has(error, "identity"))
                    await captureChangedObservedFile(error, resolvedPath, "stat" in receipt.target ? receipt.target.stat : undefined, samples === 1 ? sample : undefined, () => assertRootPathObservationReceiptCurrent(root, receipt), proof);
                throw error;
            }
        ...
        try {
            ...
            if (isNotFoundPathError(error)) {
                throw await missingObservedFileError(error, resolvedPath, expected, () => assertRootDirectoryObservationGuard(root, parentGuard), proof);
            }
            if (comparison.has(error, "identity"))
                await captureChangedObservedFile(error, resolvedPath, expected, samples === 1 ? sample : undefined, () => assertRootDirectoryObservationGuard(root, parentGuard), proof);
            throw error;
        }
        if (observed.isSymbolicLink()) {
            throw new FsSafeError("path-mismatch", "file changed during operation");
