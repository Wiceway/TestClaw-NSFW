...
function assertParentCurrent(parent) {
...
function closeAfterFailure(closeFd, fd, failure, report = false) {
    if (fd === undefined)
        return;
    try {
        closeFd(fd);
...
export async function capturePolicyAwareWindowsParent(binding, params, rootAdmission) {
    const closeFd = captureNativeFdClose(binding);
    let session;
    function parentObservation(fd, guard) {
    ...
    async function openParent(fd, parentPath, relativePath) {
        try {
    ...
    async function authorize(targetPath, mutationPath = targetPath, phase = "parent") {
    ...
    const rootStat = inspectDirectoryIdentitySync(params.rootPath, inspectFileIdentitySync(() => fsSync.fstatSync(rootAdmission.root.fd, { bigint: true })));
    let current = {
    ...
    try {
        for (let index = 0; index < segments.length; index += 1) {
            ...
            try {
                ...
                const request = Object.freeze({ targetPath, mutationPath, phase: "parent-create" });
