# `@testclaw/gateway-protocol`

Typed schemas, inferred TypeScript types, and runtime validators for the Testclaw
...
See the [Gateway protocol specification]((документация продукта)/gateway/protocol)
for transport, authentication, roles, scopes, and complete frame examples.

## Versioning

Package versions follow the Testclaw calendar release train:
`YYYY.M.PATCH`, with the same prerelease suffix when applicable. A package version
therefore identifies the Testclaw source release that produced the schemas; it is
not the wire protocol number.

The wire protocol integer is versioned separately. Its current value is exported
as `PROTOCOL_VERSION` from `@testclaw/gateway-protocol/version`. Gateway protocol
...
```bash
npm install --save-exact @testclaw/gateway-protocol@2026.8.1
```

This release declares Node.js `>=22.19.0`. See the canonical
[installation guide]((документация продукта)/gateway/clients#install-the-packages)
...
`testclaw` CLI has its own package versions and dist-tags.

## Entry points

- `@testclaw/gateway-protocol` exports runtime validators, selected schemas, error
  formatting, and their TypeScript types. This is the main TypeBox-backed entry.
- `@testclaw/gateway-protocol/schema` exports the TypeBox schema graph, including
  the `ProtocolSchemas` registry used by generators.
- `@testclaw/gateway-protocol/frame-guards` exports dependency-free structural
  guards for gateway event and response envelopes.
- `@testclaw/gateway-protocol/client-info` exports client ID, mode, and capability
  registries plus normalization helpers.
- `@testclaw/gateway-protocol/connect-error-details` exports structured connect
  error readers and recovery metadata.
- `@testclaw/gateway-protocol/gateway-error-details` exports helpers for reading
  structured details from general gateway errors.
- `@testclaw/gateway-protocol/startup-unavailable` exports startup retry constants
  and helpers.
- `@testclaw/gateway-protocol/version` exports the current and minimum accepted
  protocol versions.
...
```ts
import { formatValidationErrors, validateRequestFrame } from "@testclaw/gateway-protocol";
...
```ts
import { isGatewayEventFrame } from "@testclaw/gateway-protocol/frame-guards";
...
```ts
import { GATEWAY_CLIENT_CAPS } from "@testclaw/gateway-protocol/client-info";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@testclaw/gateway-protocol/version";
...
[`protocol.schema.json`](https://unpkg.com/@testclaw/gateway-protocol@2026.8.1/protocol.schema.json)
...
Download it as a file; `@testclaw/gateway-protocol/protocol.schema.json` is not an
