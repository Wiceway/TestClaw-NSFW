import type { AssistantMessage, ToolResultMessage } from "@testclaw/llm-core";
import { coerceErrorMessage } from "@testclaw/normalization-core/error-coercion";
...
const TOOL_LOOP_RECOVERY_TERMINATED_MESSAGE =
  "Testclaw stopped this run because tool-loop recovery encountered another critical loop. No blocked tool action was executed.";
...
function withAssistantTurnTaint(message: AssistantMessage, tainted: boolean): AssistantMessage {
  ...
  const taintedMessage = {
    ...message,
    __testclaw: { ...readTurnTaintMetadata(message), turnTainted: true },
  } satisfies AssistantMessage & { __testclaw: TurnTaintMetadata };
...
function withToolResultContentSource(
  ...
  return {
    ...message,
    __testclaw: { ...readTurnTaintMetadata(message), resultContentSource: source },
