...
const ensureSupportedRuntimeVersion = async () => {
  if (process.versions.bun) {
    ...
    // rejecting Bun outright so capable Bun builds can run Testclaw.
    let hasNodeSqlite;
    try {
    ...
    process.stderr.write(
      "testclaw: this Bun runtime is unsupported because it does not provide node:sqlite.\n" +
  ...
  const canRunDiagnostics = canRunTestclawNodeDiagnostics(process.versions.node, probe.available);
  ...
  if (!diagnosticExemption) {
    process.stderr.write(`testclaw: ${failure}\n`);
  }
  if (diagnosticExemption) {
    return false;
  }
  ...
  if (unsupportedCommand === "update" && canRunDiagnostics) {
    ...
    process.env.TESTCLAW_NODE_UPDATE_RESPAWNED = "1";
    return false;
  }
  return process.exit(1);
};
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.TESTCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    TESTCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, resolveCompileCacheRespawnLauncher(), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["testclaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:testclaw/testclaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g testclaw@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.TESTCLAW_CONTAINER)) {
    return true;
  }
...
const isHelpFastPathDisabled = () =>
  process.env.TESTCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.TESTCLAW_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.TESTCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.TESTCLAW_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "testclaw.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".testclaw", "testclaw.json"),
    path.join(homeDir, ".testclaw", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "testclaw.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.TESTCLAW_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.TESTCLAW_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Testclaw ${version} (${commit})\n` : `Testclaw ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.TESTCLAW_BUNDLED_VERSION) ?? "0.0.0";
}
...
if (isBrowserNativeHostInvocation) {
  ...
  if (!waitingForNodeUpdateRespawn) {
    ...
    if (
      ...
      (existsSync(new URL("./.testclaw-lifecycle-pending", import.meta.url)) ||
        existsSync(new URL("./dist/testclaw-install-guard", import.meta.url)))
    ) {
      try {
        ...
        process.stderr.write(
          `testclaw: package lifecycle is incomplete. Reinstall with package scripts enabled, then retry. ${error instanceof Error ? error.message : String(error)}\n`,
        );
        process.exit(1);
      }
    }
  ...
  if (
    ...
    try {
      ...
      if (enabled !== undefined && result?.status === enabled) {
        ...
        const key = Symbol.for("testclaw.nodeCompileCacheBase");
